...
public interface IReportingService
{
    ...
    Task<VehicleBillingHistoryReport> GenerateVehicleBillingHistoryAsync(int vmfCode, int financialYear);
    ...
    Task<MaintenanceScheduleReport> GenerateMaintenanceScheduleReportAsync(DateTime startDate, DateTime endDate);
    Task<MaintenanceCostReport> GenerateMaintenanceCostReportAsync(int? vmfCode, DateTime startDate, DateTime endDate);

    // Trip Reports
    Task<TripSummaryReport> GenerateTripSummaryReportAsync(int? vmfCode, DateTime startDate, DateTime endDate);
    ...
    Task<ContractBillingReport> GenerateContractBillingReportAsync(int contractId, DateTime startDate, DateTime endDate);

    // PDF Generation
    Task<byte[]> GenerateVehicleReportPdfAsync(int vmfCode);
    Task<byte[]> GenerateCustomReportPdfAsync(string reportType, Dictionary<string, object> parameters);
