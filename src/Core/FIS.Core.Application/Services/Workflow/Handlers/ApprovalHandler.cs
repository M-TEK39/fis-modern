...
public class ApprovalHandler : StepHandlerBase
{
    ...
    public ApprovalHandler(ILogger<ApprovalHandler> logger)
        : base(logger) { }
    ...
    )
    {
        ...
        var hasApproverUserId =
            parameters.ContainsKey("approverUserId") && parameters["approverUserId"] != null;
        var hasApproverRole =
            parameters.ContainsKey("approverRole")
            && !string.IsNullOrWhiteSpace(parameters["approverRole"]?.ToString());

        if (!hasApproverUserId && !hasApproverRole)
            result.AddError(
                "At least one approval method must be specified: approverUserId or approverRole"
            );
