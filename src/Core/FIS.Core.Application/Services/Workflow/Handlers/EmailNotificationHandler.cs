...
public class EmailNotificationHandler : StepHandlerBase
{
    ...
    public EmailNotificationHandler(ILogger<EmailNotificationHandler> logger) : base(logger) { }
    ...
    public override Task<Interfaces.Workflow.ValidationResult> ValidateParametersAsync(Dictionary<string, object> parameters)
    {
        ...
        if (!parameters.ContainsKey("to") || string.IsNullOrWhiteSpace(parameters["to"]?.ToString()))
            result.AddError("Parameter 'to' is required");
        if (!parameters.ContainsKey("subject") || string.IsNullOrWhiteSpace(parameters["subject"]?.ToString()))
            result.AddError("Parameter 'subject' is required");
        if (!parameters.ContainsKey("body") || string.IsNullOrWhiteSpace(parameters["body"]?.ToString()))
