...
public class DataValidationHandler : StepHandlerBase
{
    ...
    public DataValidationHandler(ILogger<DataValidationHandler> logger)
        : base(logger) { }
    ...
    )
    {
        ...
        if (!context.WorkflowData.TryGetValue(fieldName, out var fieldValue))
        {
            return Task.FromResult(
                StepExecutionResult.FailureResult($"Field '{fieldName}' not found")
            );
        }
        ...
        return Task.FromResult(
            StepExecutionResult.SuccessResult($"Validation passed for '{fieldName}'")
        );
    }
    ...
    )
    {
        ...
        if (
            ...
            || string.IsNullOrWhiteSpace(parameters["fieldName"]?.ToString())
        )
            result.AddError("Parameter 'fieldName' is required");
        if (
            !parameters.ContainsKey("validationType")
            || string.IsNullOrWhiteSpace(parameters["validationType"]?.ToString())
        )
