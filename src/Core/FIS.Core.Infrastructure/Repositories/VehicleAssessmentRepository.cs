...
public class VehicleAssessmentRepository : IVehicleAssessmentRepository
{
    ...
    public async Task<VehicleAssessment?> GetByIdAsync(int assessmentCode)
    {
        return await _context
            .Set<VehicleAssessment>()
            .Include(v => v.Vehicle)
            .FirstOrDefaultAsync(v => v.vehicle_assessment_code == assessmentCode);
    }

    public async Task<IEnumerable<VehicleAssessment>> GetAllAsync()
    {
        return await _context.Set<VehicleAssessment>().Include(v => v.Vehicle).ToListAsync();
    }
    ...
    public async Task<VehicleAssessment?> GetByVehicleAsync(int vmfCode)
    {
        return await _context
            .Set<VehicleAssessment>()
            .Where(v => v.vmf_code == vmfCode)
            .Include(v => v.Vehicle)
            .OrderByDescending(v => v.assessment_date)
            .FirstOrDefaultAsync();
    }
    ...
    public async Task<IEnumerable<VehicleAssessment>> GetRecentAssessmentsAsync(int days)
    {
        ...
        return await _context
            .Set<VehicleAssessment>()
            .Where(v => v.assessment_date >= cutoffDate)
            .Include(v => v.Vehicle)
            .ToListAsync();
    }
    ...
    {
        _context.Set<VehicleAssessment>().Add(assessment);
    ...
    )
    {
        ...
        var existing = await _context
            .Set<VehicleAssessment>()
            .FindAsync(assessment.vehicle_assessment_code);
        if (existing == null)
            throw new InvalidOperationException(
                $"VehicleAssessment with vehicle_assessment_code {assessment.vehicle_assessment_code} not found"
            );
    ...
    public async Task DeleteAsync(int assessmentCode, int currentUserId)
    {
        var assessment = await GetByIdAsync(assessmentCode);
        if (assessment != null)
        {
            _context.Set<VehicleAssessment>().Remove(assessment);
            await _context.SaveChangesAsync();
        }
    }
}
