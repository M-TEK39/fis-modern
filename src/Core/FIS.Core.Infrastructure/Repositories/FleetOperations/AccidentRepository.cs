...
public sealed class AccidentRepository : IAccidentRepository
{
    ...
    )
    {
        ...
        try
        {
            ...
            var vehicleRegistrationAvailable =
                vehicleJoinAvailable && vehicleColumns.Contains("registration_number");
            var vehicleFleetAvailable =
                vehicleJoinAvailable && vehicleColumns.Contains("fleet_number");
            var siteDepartmentAvailable =
    ...
    public async Task<AccidentOutstandingDocumentReport?> GetOutstandingDocumentReportAsync(
        int accidentCode
    )
    {
        if (accidentCode <= 0)
        ...
        var documentStatusTrackingAvailable = OutstandingDocumentDefinitions.All(document =>
            accidentColumns.Contains(document.Column)
        );
        ...
        try
        {
            ...
            projection.AddRange(
                OutstandingDocumentDefinitions.Select(document =>
            ...
            return new AccidentOutstandingDocumentReport
            {
                ...
                outstanding_documents = OutstandingDocumentDefinitions
                    ...
                    .Where(document =>
                        !string.Equals(
                            ReadString(reader, document.Column),
                            "Y",
                            StringComparison.OrdinalIgnoreCase
                        )
                    )
    ...
    )
    {
        ...
        var requiredAccidentColumns = PeriodReportColumns
            .Where(column =>
                column is "accident_code" or "vmf_code" or "occurence_date" or "file_close_date"
            )
            .ToArray();
        ...
        var typeJoinAvailable =
            vehicleColumns.Contains("type_code") && typeColumns.Contains("type_code");
        var accidentTypeJoinAvailable =
            accidentColumns.Contains("acc_type_code")
            && accidentTypeColumns.Contains("acc_type_code");
        var connection = _context.Database.GetDbConnection();
        var shouldClose = connection.State != ConnectionState.Open;
        if (shouldClose)
        {
        ...
        try
        {
            ...
            {
                joins.Add(
                    "LEFT JOIN [dbo].[acc_type] AS [at] ON [at].[acc_type_code] = [a].[acc_type_code]"
                );
            }
                ...
                results.Add(
                    new AccidentPeriodReportRow
                    {
                        ...
                        accident_description =
                            ReadString(reader, "accident_description") ?? string.Empty,
                        driver_name = ReadString(reader, "driver_name") ?? string.Empty,
    ...
    )
    {
        ...
        var hasDepartmentPeriodFilter =
            departmentNumber is not null
            || periodStartDate.HasValue
            || periodEndDate.HasValue
            || hasCalendarPeriodFilter
            || hasFinancialYearFilter;
        if (
            flagMode is null
            && dateRangeMode is null
            && garageMode is null
            && !hasDepartmentPeriodFilter
        ...
        var periodAvailable =
            periodStartDate.HasValue
            && periodEndDate.HasValue
            && accidentColumns.Contains("occurence_date");
        var calendarPeriodAvailable =
            hasCalendarPeriodFilter && accidentColumns.Contains("occurence_date");
        ...
        if (
            hasDepartmentPeriodFilter
            && (
                (!periodAvailable && !calendarPeriodAvailable && !financialYearAvailable)
                || (normalizedDepartmentNumber.Length > 0 && !siteDepartmentAvailable)
            )
        )
        {
            return Array.Empty<AccidentVehicleReportRow>();
        ...
        var hasFilter =
            ...
            || flagAvailable
            || dateAvailable
            || garageMode is not null
            || periodAvailable
            || calendarPeriodAvailable
        ...
        try
        {
            ...
            var projection = VehicleReportColumns
                ...
                .Concat(
                    [
                        ...
                        accidentTypeJoinAvailable
                        && accidentTypeColumns.Contains("acc_type_description")
            ...
            {
                joins.Add(
                    "LEFT JOIN [dbo].[location] AS [l] ON [l].[location_code] = [v].[location_code]"
                );
            }
            ...
            {
                conditions.Add(
                    "[a].[occurence_date] >= @periodStartDate AND [a].[occurence_date] <= @periodEndDate"
                );
            }
            ...
            var primaryOrderColumn =
                orderByDepartment && siteDepartmentAvailable
                    ? "[s].[Department_number]"
                    : $"[v].[{orderColumn}]";
                ...
                results.Add(
                    new AccidentVehicleReportRow
                    {
                        ...
                        location_description =
                            ReadString(reader, "location_description") ?? string.Empty,
                        occurence_date = ReadDateTime(reader, "occurence_date"),
                        ...
                        accident_type_description =
                            ReadString(reader, "accident_type_description") ?? string.Empty,
                        ...
                        cost_of_repair = ReadDecimal(reader, "cost_of_repair"),
                        damage_description =
                            ReadString(reader, "damage_description") ?? string.Empty,
                        ...
                        th_claim_accept_reject =
                            ReadString(reader, "th_claim_accept_reject") ?? string.Empty,
                        th_claim_reject_reason =
                            ReadString(reader, "th_claim_reject_reason") ?? string.Empty,
    ...
    public async Task<IEnumerable<Accident>> GetByDateRangeAsync(
        DateTime startDate,
        DateTime endDate
    ) =>
        await QueryAsync(
    ...
    public async Task<AccidentClaimsSummary> GetClaimsSummaryAsync()
    {
        ...
        var closedClaims = accidents.Count(accident =>
            accident.claim_amount == 0 || !accident.claim_amount.HasValue
        );
        ...
        return (await GetAllAsync())
            ...
            .Select(accident => new AccidentOutstandingClaim
            {
                ...
                days_outstanding = (int)
                    (today - (accident.occurence_date ?? DateTime.MinValue)).TotalDays,
            });
    }

    public async Task<AccidentStatistics> GetStatisticsAsync(
    ...
    )
    {
        ...
        var accidentsThisYear = accidentList.Count(accident =>
            accident.occurence_date?.Year == thisYear
        );
        ...
        return new AccidentStatistics
        {
            ...
            total_third_party_claims = accidentList.Sum(accident =>
                accident.third_party_claim ?? 0
            ),
            total_department_claims = accidentList.Sum(accident =>
                accident.claim_against_dept ?? 0
            ),
    ...
    )
    {
        ...
        var existing =
            await GetByIdAsync(accident.accident_code)
        ...
        var values = BuildWriteValues(accident, availableColumns)
            .Where(value =>
                !string.Equals(value.Column, "accident_code", StringComparison.OrdinalIgnoreCase)
            )
            .ToList();
        AddOptionalValue(
            values,
        ...
        accident.date_updated = now;
        accident.modified_by_user_code =
            currentUserId > 0 ? currentUserId : existing.modified_by_user_code;
        return accident;
    }
    ...
    private static Accident MapAccident(DbDataReader reader)
    {
        var accident = new Accident
        {
            ...
            trip_author =
                ReadString(reader, "trip_author") ?? ReadString(reader, "Flag_trip_author"),
    ...
    )
    {
        var values = new List<WriteValue>
        {
            ...
            new(
                ...
                "@thirdPartyClaim",
                DbType.Decimal,
                accident.third_party_claim
            ),
            new(
                "SecondThirdPartyRegNo",
                "@secondThirdPartyRegno",
                DbType.String,
                accident.SecondThirdPartyRegNo
            ),
            ...
            new(
                ...
                "@flagTripAuthor",
                DbType.String,
                accident.Flag_trip_author ?? accident.trip_author
            ),
            new(
                "flag_trip_auth_date",
                "@flagTripAuthDate",
                DbType.DateTime2,
                accident.Flag_trip_auth_date
            ),
            ...
            new(
                ...
                DbType.Date,
                accident.priv_dampay_date?.Date
            ),
            new(
                "th_claim_accept_reject",
                "@thirdPartyClaimDecision",
                DbType.String,
                accident.th_claim_accept_reject
            ),
            new(
                "th_claim_reject_reason",
                "@thirdPartyClaimRejectReason",
                DbType.String,
                accident.th_claim_reject_reason
            ),
        ...
        values.Add(
            new WriteValue(
                ...
                "@claimAmount",
                DbType.Decimal,
                accident.claim_amount ?? accident.claim_against_dept
            )
        );
    ...
    private static string GetPeriodVehicleProjection(IReadOnlySet<string> columns, string column) =>
        columns.Contains(column)
            ? $"[v].[{column}] AS [{column}]"
            : $"CAST(NULL AS {GetSqlType(column)}) AS [{column}]";
    ...
    private static string GetAllAccidentDateFilter(string mode) =>
        mode switch
        {
            ...
            "1999-2001" =>
                "[a].[occurence_date] >= @middleStartDate AND [a].[occurence_date] < @middleEndDate",
    ...
    private static string GetCalendarPeriodFilter(string mode) =>
        ...
        {
            "month" =>
                "MONTH([a].[occurence_date]) = @accidentMonth AND YEAR([a].[occurence_date]) = @accidentYear",
            "year" => "YEAR([a].[occurence_date]) = @accidentYear",
            "02/03" =>
                "[a].[occurence_date] >= @financialPeriodStartDate AND [a].[occurence_date] < @financialPeriodEndDate",
            "01/02" =>
                "[a].[occurence_date] >= @financialPeriodStartDate AND [a].[occurence_date] < @financialPeriodEndDate",
            _ => "1 = 0",
        };
    ...
    private static string GetSqlType(string column) =>
        column switch
        {
            ...
            or "accident_km"
            or "cost_of_repair"
            or "third_party_claim"
            or "claim_against_dept" => "decimal(18, 2)",
            "occurence_date"
            or "reported_date"
            or "file_close_date"
            or "priv_dampay_date"
            or "write_off_date" => "date",
            "occurence_time" => "datetime2",
            "Flag_gg_hq_date" or "flag_trip_auth_date" or "date_created" or "date_updated" =>
                "datetime2",
