...
public class MonitorRepository : IMonitorRepository
{
    ...
    public async Task<IEnumerable<MonitorEntity>> GetByDateRangeAsync(
        DateTime startDate,
        DateTime endDate
    ) =>
        await QueryAsync(
    ...
    public async Task<MonitorEntity> CreateAsync(MonitorEntity monitor, int currentUserId)
    {
        ...
        command.CommandText =
            $"INSERT INTO [dbo].[{TableName}] ({string.Join(", ", values.Select(value => $"[{value.Column}]"))}) OUTPUT INSERTED.[monitor_code] VALUES ({string.Join(", ", values.Select(value => value.Parameter))})";
        AddParameters(command, values);
        var id = Convert.ToInt16(await command.ExecuteScalarAsync());
        return await GetByIdAsync(id)
            ?? throw new InvalidOperationException("Created monitor inquiry could not be read.");
    }
    ...
    public async Task<MonitorEntity> UpdateAsync(MonitorEntity monitor, int currentUserId)
    {
        ...
        command.Transaction = CurrentTransaction;
        command.CommandText =
            $"UPDATE [dbo].[{TableName}] SET {string.Join(", ", values.Select(value => $"[{value.Column}] = {value.Parameter}"))} WHERE [monitor_code] = @monitorCode";
        AddParameters(command, values);
        AddParameter(command, "@monitorCode", DbType.Int16, monitor.monitor_code);
        if (await command.ExecuteNonQueryAsync() == 0)
            throw new KeyNotFoundException(
                $"Monitor inquiry not found with code: {monitor.monitor_code}"
            );
    ...
    public async Task DeleteAsync(short monitorCode, int currentUserId)
    {
        ...
        if (columns.ContainsKey("is_deleted"))
        {
            ...
            command.CommandText =
                $"UPDATE [dbo].[{TableName}] SET {string.Join(", ", assignments)} WHERE [monitor_code] = @monitorCode";
        }
        else
        {
            command.CommandText =
                $"DELETE FROM [dbo].[{TableName}] WHERE [monitor_code] = @monitorCode";
        }
    ...
    private async Task<List<MonitorEntity>> QueryAsync(
    ...
    private async Task<Dictionary<string, ColumnInfo>> GetAvailableColumnsAsync()
    {
        ...
        if (RequiredColumns.Any(column => !columns.ContainsKey(column)))
            throw new InvalidOperationException(
                "The Monitor compatibility table is missing its required legacy columns."
            );
        return columns;
    }
        ...
        string.Join(
            ...
            [
                ...
                OptionalExpression(columns, "User_access_code", "smallint", "m")
                    + " AS [User_access_code]",
                OptionalExpression(columns, "Inquiry_type", "nvarchar(100)", "m")
                    + " AS [Inquiry_type]",
                OptionalExpression(columns, "Inquiry_Desc", "nvarchar(max)", "m")
                    + " AS [Inquiry_Desc]",
                OptionalExpression(columns, "Driver_name", "nvarchar(100)", "m")
                    + " AS [Driver_name]",
                OptionalExpression(columns, "Driver_persalno", "nvarchar(50)", "m")
                    + " AS [Driver_persalno]",
                OptionalExpression(columns, "Driver_Site", "smallint", "m") + " AS [Driver_Site]",
                DateCreatedExpression(columns) + " AS [date_created]",
                OptionalExpression(columns, "date_updated", "datetime2", "m")
                    + " AS [date_updated]",
                CreatedByExpression(columns) + " AS [created_by_user_code]",
                OptionalExpression(columns, "modified_by_user_code", "int", "m")
