...
public sealed class ClassRepository : IClassRepository
{
    ...
    public async Task<IEnumerable<Class>> SearchAsync(string searchTerm)
    {
        ...
        var searchColumns = new[]
        {
            ...
            "class_number",
            "bank_number"
        }.Where(availableColumns.Contains).ToArray();
    ...
    public async Task<Class> UpdateAsync(Class classEntity, int currentUserId)
    {
        ...
        var existing = await GetByIdAsync(classEntity.class_code)
        ...
        classEntity.date_updated = now;
        classEntity.modified_by_user_code = currentUserId > 0
            ? currentUserId
    ...
    public async Task DeleteAsync(short classCode, int currentUserId)
    {
        ...
        try
        {
            ...
            if (availableColumns.Contains("is_deleted"))
            {
                ...
                command.CommandText = "UPDATE [dbo].[class] SET [is_deleted] = @isDeleted, " +
        ...
        bool includeCreateAudit = true)
    {
        var values = new List<WriteValue>
        {
            ...
            new("modified_by_user_code", "@modifiedByUserCode", DbType.Int32, classEntity.modified_by_user_code)
        };

        AddOptionalValue(values, availableColumns, "class_number", "@classNumber", DbType.String, classEntity.class_number);
        AddOptionalValue(values, availableColumns, "bank_number", "@bankNumber", DbType.String, classEntity.bank_number);
        AddOptionalValue(values, availableColumns, "months_life", "@monthsLife", DbType.Int16, classEntity.months_life);
        AddOptionalValue(values, availableColumns, "depreciation_percent", "@depreciationPercent", DbType.Decimal, classEntity.depreciation_percent);
        AddOptionalValue(values, availableColumns, "odometer_life", "@odometerLife", DbType.Decimal, classEntity.odometer_life);
        AddOptionalValue(values, availableColumns, "appreciate_percent", "@appreciatePercent", DbType.Int16, classEntity.appreciate_percent);
        AddOptionalValue(values, availableColumns, "replacement_cost", "@replacementCost", DbType.Decimal, classEntity.replacement_cost);

        if (includeCreateAudit)
        {
            AddOptionalValue(values, availableColumns, "date_created", "@dateCreated", DbType.DateTime2, classEntity.date_created);
            AddOptionalValue(values, availableColumns, "created_by_user_code", "@createdByUserCode", DbType.Int32, classEntity.created_by_user_code);
    ...
    private async Task<List<Class>> QueryAsync(string? predicate = null, Action<DbCommand>? configure = null)
    {
        var availableColumns = await GetAvailableColumnsAsync();
        var connection = _context.Database.GetDbConnection();
        var shouldClose = connection.State != ConnectionState.Open;
        ...
        try
        {
            ...
            command.CommandText = $"SELECT {string.Join(", ", projection)} FROM [dbo].[{TableName}] WHERE {string.Join(" AND ", conditions)} ORDER BY [description], [class_code]";
    ...
    private async Task<short> ExecuteInsertAsync(IReadOnlyList<WriteValue> values)
    {
        ...
        try
        {
            await using var command = connection.CreateCommand();
            command.Transaction = _context.Database.CurrentTransaction?.GetDbTransaction();
            command.CommandText = $"INSERT INTO [dbo].[{TableName}] ({string.Join(", ", values.Select(value => $"[{value.Column}]"))}) OUTPUT INSERTED.[class_code] VALUES ({string.Join(", ", values.Select(value => value.Parameter))})";
    ...
    private async Task ExecuteUpdateAsync(short classCode, IReadOnlyList<WriteValue> values)
    {
        ...
        try
        {
            await using var command = connection.CreateCommand();
            command.Transaction = _context.Database.CurrentTransaction?.GetDbTransaction();
            command.CommandText = $"UPDATE [dbo].[{TableName}] SET {string.Join(", ", values.Select(value => $"[{value.Column}] = {value.Parameter}"))} WHERE [class_code] = @classCode";
            AddParameters(command, values);
            AddParameter(command, "@classCode", DbType.Int16, classCode);
            await command.ExecuteNonQueryAsync();
        }
    ...
    private async Task<HashSet<string>> GetAvailableColumnsAsync()
    {
        ...
        try
        {
            ...
            if (missing.Length > 0)
            {
                throw new InvalidOperationException($"The legacy class table is missing required columns: {string.Join(", ", missing)}");
            }
