...
public sealed class LeaseContractTermsRepository : ILeaseContractTermsRepository
{
    ...
    public async Task<IEnumerable<LeaseContractTerms>> GetAllAsync()
        => await QueryAsync();
    ...
    public async Task<LeaseContractTerms> CreateAsync(LeaseContractTerms terms, int currentUserId)
    {
        ...
        AddValue(values, availableColumns, "CreatedBy", "@createdBy", DbType.Int32, currentUserId > 0 ? currentUserId : terms.CreatedBy);
        ...
        AddValue(values, availableColumns, "created_by_user_code", "@createdByUserCode", DbType.Int32, currentUserId > 0 ? currentUserId : terms.created_by_user_code);
    ...
    public async Task<LeaseContractTerms> UpdateAsync(LeaseContractTerms terms, int currentUserId)
    {
        ...
        _ = await GetByIdAsync(terms.VehicleContractTermID)
        ...
        AddValue(values, availableColumns, "modified_by_user_code", "@modifiedByUserCode", DbType.Int32, currentUserId > 0 ? currentUserId : terms.modified_by_user_code);

        var modifiedByColumn = FirstAvailable(availableColumns, "ModifiedBy", "UpdatedBy");
        if (modifiedByColumn is not null)
        {
            AddValue(values, availableColumns, modifiedByColumn, "@modifiedBy", DbType.Int32, currentUserId > 0 ? currentUserId : terms.ModifiedBy);
        }

        var modifiedDateColumn = FirstAvailable(availableColumns, "ModifiedDate", "UpdatedDate");
        if (modifiedDateColumn is not null)
        {
            AddValue(values, availableColumns, modifiedDateColumn, "@modifiedDate", DbType.DateTime2, now);
        }
    ...
    public async Task DeleteAsync(int termId, int currentUserId)
    {
        ...
        try
        {
            ...
            if (availableColumns.Contains("is_deleted"))
            {
                ...
                AddAssignment(assignments, command, availableColumns, "date_updated", "@dateUpdated", DbType.DateTime2, DateTime.UtcNow);
                AddAssignment(assignments, command, availableColumns, "modified_by_user_code", "@modifiedByUserCode", DbType.Int32, currentUserId > 0 ? currentUserId : null);
                command.CommandText = $"""
        ...
        Action<DbCommand>? configure = null)
    {
        ...
        var vehicleDateFallbackAvailable = vehicleColumns.Contains("vmf_code") && vehicleColumns.Contains("take_on_date");
        var connection = _context.Database.GetDbConnection();
        var shouldClose = connection.State != ConnectionState.Open;
        if (shouldClose)
        {
        ...
        try
        {
            ...
            var projection = new[]
            {
                ...
                GetProjection(availableColumns, "AppliedInterest", "AppliedInterest", "decimal(18,2)"),
                GetProjection(availableColumns, "FixedMonthlyAmount", "FixedMonthlyAmount", "decimal(18,2)"),
                GetAuthorityStatusProjection(availableColumns),
                GetProjection(availableColumns, "CreatedBy", "CreatedBy", "int"),
                GetDateProjection(availableColumns, "CreatedDate", "CreatedDate", "CreatedDate", "date_created"),
                GetPreferredProjection(availableColumns, "ModifiedBy", ["ModifiedBy", "UpdatedBy"], "int"),
                GetPreferredDateProjection(availableColumns, "ModifiedDate", ["ModifiedDate", "UpdatedDate"]),
                ...
                GetPreferredDateProjection(availableColumns, "date_updated", ["date_updated", "ModifiedDate", "UpdatedDate"]),
                GetPreferredProjection(availableColumns, "created_by_user_code", ["created_by_user_code", "CreatedBy"], "int"),
                GetPreferredProjection(availableColumns, "modified_by_user_code", ["modified_by_user_code", "ModifiedBy", "UpdatedBy"], "int"),
                ...
                GetProjection(availableColumns, "ExcessKilosTarrif", "ExcessKilosTarrif", "decimal(18,2)"),
                ...
                GetPreferredProjection(availableColumns, "authority_comment", ["authority_comment", "Comments"], "varchar(250)"),
                GetPreferredProjection(availableColumns, "lease_notes", ["lease_notes", "Comments"], "varchar(250)"),
    ...
    private async Task ExecuteUpdateAsync(int termId, IReadOnlyList<WriteValue> values, IReadOnlySet<string> availableColumns)
    {
        if (values.Count == 0)
        {
            return;
    ...
    private static List<WriteValue> BuildBaseWriteValues(LeaseContractTerms terms, IReadOnlySet<string> availableColumns)
    {
        var values = new List<WriteValue>();
        AddValue(values, availableColumns, "vmf_Code", "@vmfCode", DbType.Int32, terms.vmf_Code);
        AddValue(values, availableColumns, "AgreedTerms", "@agreedTerms", DbType.Int32, terms.AgreedTerms);
        AddValue(values, availableColumns, "AgreedKilos", "@agreedKilos", DbType.Int64, terms.AgreedKilos);
        AddValue(values, availableColumns, "AppliedInterest", "@appliedInterest", DbType.Decimal, terms.AppliedInterest);
        AddValue(values, availableColumns, "FixedMonthlyAmount", "@fixedMonthlyAmount", DbType.Decimal, terms.FixedMonthlyAmount);
        AddValue(values, availableColumns, "StartDate", "@startDate", DbType.DateTime2, terms.StartDate);
        AddValue(values, availableColumns, "EndDate", "@endDate", DbType.DateTime2, terms.EndDate);
        AddValue(values, availableColumns, "AgreedOverallKilo", "@agreedOverallKilo", DbType.Int32, terms.AgreedOverallKilo);
        AddValue(values, availableColumns, "ExcessKilosTarrif", "@excessKilosTarrif", DbType.Decimal, terms.ExcessKilosTarrif);
        AddValue(values, availableColumns, "RelieveVehicle", "@relieveVehicle", DbType.Boolean, terms.RelieveVehicle);
        AddValue(values, availableColumns, "lease_site_code", "@leaseSiteCode", DbType.Int16, terms.lease_site_code);
        ...
        AddValue(values, availableColumns, "authority_comment", "@authorityComment", DbType.String, terms.authority_comment);
        AddValue(values, availableColumns, "rejection_reason", "@rejectionReason", DbType.String, terms.rejection_reason);
        return values;
    }
        ...
        DateTime now)
    {
        ...
        AddValue(values, availableColumns, "AuthorityStatus", "@authorityStatus", DbType.Int32, legacyStatus);
        AddValue(values, availableColumns, "Rejected", "@rejected", DbType.Int32, legacyRejected);
        AddValue(values, availableColumns, "AuthorisedBy", "@authorisedBy", DbType.Int32, status == 2 && currentUserId > 0 ? currentUserId : terms.AuthorisedBy);
        AddValue(values, availableColumns, "AuthorisedDate", "@authorisedDate", DbType.DateTime2, status == 2 ? now : terms.AuthorisedDate);
    }
    ...
    private static string GetEffectiveStartProjection(IReadOnlySet<string> columns, bool vehicleDateFallbackAvailable)
    {
        ...
        return vehicleDateFallbackAvailable ? "[v].[take_on_date] AS [StartDate]" : "CAST(NULL AS datetime2) AS [StartDate]";
    }

    private static string GetEffectiveEndProjection(IReadOnlySet<string> columns, bool vehicleDateFallbackAvailable)
    {
        var fallback = columns.Contains("AgreedTerms") && vehicleDateFallbackAvailable
    ...
    private static string GetDateProjection(IReadOnlySet<string> columns, string alias, params string[] candidates)
    {
    ...
    private static string GetPreferredDateProjection(IReadOnlySet<string> columns, string alias, IReadOnlyList<string> candidates)
    {
        var available = candidates.Where(columns.Contains).Select(column => $"[l].[{column}]").ToArray();
        return available.Length == 0
            ? $"CAST(NULL AS datetime2) AS [{alias}]"
            : $"COALESCE({string.Join(", ", available)}) AS [{alias}]";
    }

    private static string GetPreferredProjection(IReadOnlySet<string> columns, string alias, IReadOnlyList<string> candidates, string sqlType)
    {
        var available = candidates.Where(columns.Contains).Select(column => $"[l].[{column}]").ToArray();
        return available.Length == 0
    ...
    private static string GetProjection(IReadOnlySet<string> columns, string alias, string column, string sqlType)
        => columns.Contains(column)
            ? $"[l].[{column}] AS [{alias}]"
            : $"CAST(NULL AS {sqlType}) AS [{alias}]";

    private static string GetOptionalProjection(IReadOnlySet<string> columns, string column, string sqlType, string fallback)
        => columns.Contains(column)
            ? $"[l].[{column}] AS [{column}]"
    ...
    private static string? FirstAvailable(IReadOnlySet<string> columns, params string[] candidates)
    ...
    private static void AddValue(ICollection<WriteValue> values, IReadOnlySet<string> availableColumns, string column, string parameter, DbType type, object? value)
    {
    ...
    private static void AddAssignment(ICollection<string> assignments, DbCommand command, IReadOnlySet<string> availableColumns, string column, string parameter, DbType type, object? value)
    {
        if (!availableColumns.Contains(column))
        {
            return;
