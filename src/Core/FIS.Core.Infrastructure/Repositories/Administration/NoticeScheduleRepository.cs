...
public sealed class NoticeScheduleRepository : INoticeScheduleRepository
{
        ...
        await WithSchemaAsync(
            async (connection, transaction, schema) =>
                schema.IsModern
                    ...
                    : await ReadStoredScheduleAsync(
                        connection,
                        transaction,
                        "DEV_SEL_NoticeSchedule",
                        noticeScheduleId
                    )
        );
            ...
            async (connection, transaction, schema) =>
                schema.IsModern
                    ...
                    : (
                        ...
                        .Where(schedule =>
                            schedule.start_date <= DateTime.Today
                            && (
                                !schedule.end_date.HasValue
                                || schedule.end_date.Value >= DateTime.Today
                            )
                        )
                        .ToList()
        );

    public async Task<IEnumerable<NoticeSchedule>> GetSchedulesByDateRangeAsync(
        DateTime startDate,
        DateTime endDate
    ) =>
        await WithSchemaAsync(
                ...
                schema.IsModern
                    ...
                    : (
                        ...
                        .Where(schedule =>
                            ...
                            && (!schedule.end_date.HasValue || schedule.end_date.Value >= startDate)
                        )
                        .ToList()
        );
    ...
    public async Task<NoticeSchedule> CreateAsync(NoticeSchedule noticeSchedule, int currentUserId)
    {
        ...
        return await WithSchemaAsync(
            async (connection, transaction, schema) =>
            {
                if (!schema.IsModern)
                {
                    await ExecuteLegacyScheduleWriteAsync(
                        connection,
                        transaction,
                        noticeSchedule,
                        currentUserId
                ...
                command.CommandText = $"""
                INSERT INTO [dbo].[{ScheduleTableName}] ({string.Join(
                    ", ",
                ...
                noticeSchedule.notice_schedule_id = Convert.ToInt32(
                ...
                if (!schema.IsModern)
                {
                    _ =
                        await ReadStoredScheduleAsync(
                            ...
                            transaction,
                            "DEV_SEL_NoticeSchedule",
                            noticeSchedule.notice_schedule_id
                        )
                        ?? throw new InvalidOperationException(
                            $"NoticeSchedule with notice_schedule_id {noticeSchedule.notice_schedule_id} not found"
                        );
                    await ExecuteLegacyScheduleWriteAsync(
                        connection,
                        transaction,
    ...
    public async Task DeleteAsync(int noticeScheduleId, int currentUserId) =>
        await WithSchemaAsync<int>(
            async (connection, transaction, schema) =>
            {
                if (!schema.IsModern)
                {
                    await ExecuteLegacyScheduleDeleteAsync(
                        connection,
                        transaction,
                        noticeScheduleId
                    );
                ...
                if (columns.Contains("is_deleted"))
                {
                    ...
                    command.CommandText =
                        $"UPDATE [dbo].[{ScheduleTableName}] SET {string.Join(", ", assignments)} WHERE [notice_schedule_id] = @scheduleId AND {ActiveFilter(string.Empty, schema.ScheduleColumns.Contains("is_deleted"))}";
                }
                else
                {
                    command.CommandText =
                        $"DELETE FROM [dbo].[{ScheduleTableName}] WHERE [notice_schedule_id] = @scheduleId";
                }
    ...
    )
    {
        ...
        var projection = ScheduleColumns
            ...
            .Concat(
                OptionalScheduleColumns.Select(column =>
                    OptionalProjection("s", column, scheduleColumns)
                )
            )
            .Append(
                hasUser
    ...
    {
        await using var command = CreateStoredProcedure(
            connection,
            transaction,
            "DEV_INS_Schedule"
        );
    ...
    private static NoticeSchedule MapModernSchedule(DbDataReader reader)
    {
        ...
        {
            notice_schedule_id =
                NoticeRepositorySupport.ReadInt32(reader, "notice_schedule_id") ?? 0,
            ...
            CreatedByUser = string.IsNullOrWhiteSpace(createdBy)
                ? null
    ...
    {
        var createdBy = NoticeRepositorySupport.ReadString(
            reader,
            "created_by",
            "CreatedBy",
            "createBy"
        ...
        {
            notice_schedule_id =
                NoticeRepositorySupport.ReadInt32(
                    reader,
                    "notice_schedule_id",
                    "NoticeScheduleId",
                    "ScheduleID"
            ...
            notice_id =
                NoticeRepositorySupport.ReadInt32(reader, "notice_id", "NoticeId", "NoticeID") ?? 0,
    ...
    private static string ActiveFilter(string alias, bool hasDeletedColumn) =>
        !hasDeletedColumn ? "1 = 1"
        : string.IsNullOrWhiteSpace(alias) ? "ISNULL([is_deleted], 0) = 0"
