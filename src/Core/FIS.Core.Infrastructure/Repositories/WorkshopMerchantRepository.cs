...
public sealed class WorkshopMerchantRepository : IWorkshopMerchantRepository
{
    ...
    private static readonly string[] BusinessColumns = ["wwmerch_code", "wwmerch_name", "wwmerch_tel", "wwmerch_fax", "wwmerch_email"];
    private static readonly string[] OptionalAuditColumns = ["date_created", "date_updated", "created_by_user_code", "modified_by_user_code", "is_deleted"];
    ...
    public async Task<IEnumerable<WwMerchant>> GetAllAsync()
        => await QueryAsync();
    ...
    public async Task<WwMerchant> CreateAsync(WwMerchant merchant, int currentUserId)
    {
        ...
        AddValue(values, columns, "wwmerch_name", "@merchantName", DbType.String, merchant.wwmerch_name);
        AddValue(values, columns, "wwmerch_tel", "@merchantTel", DbType.String, merchant.wwmerch_tel);
        AddValue(values, columns, "wwmerch_fax", "@merchantFax", DbType.String, merchant.wwmerch_fax);
        AddValue(values, columns, "wwmerch_email", "@merchantEmail", DbType.String, merchant.wwmerch_email);
        ...
        return await GetByIdAsync(code)
            ?? throw new InvalidOperationException($"Workshop merchant {code} could not be read after creation.");
    }

    public async Task<WwMerchant> UpdateAsync(WwMerchant merchant, int currentUserId)
    {
        ArgumentNullException.ThrowIfNull(merchant);
        var existing = await GetByIdAsync(merchant.wwmerch_code)
            ?? throw new KeyNotFoundException($"Workshop merchant {merchant.wwmerch_code} not found");
        var columns = await GetAvailableColumnsAsync();
        var values = new List<WriteValue>();
        AddValue(values, columns, "wwmerch_name", "@merchantName", DbType.String, merchant.wwmerch_name, includeNull: true);
        AddValue(values, columns, "wwmerch_tel", "@merchantTel", DbType.String, merchant.wwmerch_tel, includeNull: true);
        AddValue(values, columns, "wwmerch_fax", "@merchantFax", DbType.String, merchant.wwmerch_fax, includeNull: true);
        AddValue(values, columns, "wwmerch_email", "@merchantEmail", DbType.String, merchant.wwmerch_email, includeNull: true);
    ...
    private async Task<List<WwMerchant>> QueryAsync(string? predicate = null, Action<DbCommand>? configure = null)
    {
        var columns = await GetAvailableColumnsAsync();
        var connection = _context.Database.GetDbConnection();
        var shouldClose = connection.State != ConnectionState.Open;
        ...
        try
        {
            ...
            while (await reader.ReadAsync()) results.Add(MapMerchant(reader, columns));
            return results;
        }
        finally
        {
    ...
    private async Task ExecuteUpdateAsync(int merchantCode, IReadOnlyList<WriteValue> values, IReadOnlyDictionary<string, ColumnInfo> columns)
    {
        var connection = _context.Database.GetDbConnection();
        var shouldClose = connection.State != ConnectionState.Open;
        if (shouldClose) await connection.OpenAsync();
    ...
    private async Task<Dictionary<string, ColumnInfo>> GetAvailableColumnsAsync()
    {
        ...
        try
        {
            ...
            foreach (var required in BusinessColumns.Where(column => !columns.ContainsKey(column)))
                throw new InvalidOperationException($"The required workshop merchant column {required} is not available.");
            return columns;
        }
        finally
        {
    ...
    private static WwMerchant MapMerchant(DbDataReader reader, IReadOnlyDictionary<string, ColumnInfo> columns)
        => new()
        {
    ...
    private static void AddValue(ICollection<WriteValue> values, IReadOnlyDictionary<string, ColumnInfo> columns, string column, string parameter, DbType type, object? value, bool includeNull = false)
    {
