...
public sealed class TaxiRepository : ITaxiRepository
{
    ...
    [
        "rek_num", "contractor_id", "vmf_code", "department_code", "site_code", "date_required",
        "time_required", "vehicle_type_code", "official", "rank", "address_1", "address_2", "address_3",
        "flight", "instructions", "destination_1", "destination_2", "destination_3", "user_access_code",
        "request_date", "confirmed", "sub_contractor_id", "cancelled", "resp_code", "object_code", "fms_code",
        "date_required_2", "time_required_2", "address_12", "address_22", "address_32", "destination_12",
        "destination_22", "destination_32", "trans_man_name", "trans_man_date", "trans_man_rank", "trans_man_tel",
        "booking_by", "driver", "arrival_time", "reg_num", "project", "driver_available", "persal", "JIA_pickup",
        "official_tel_num", "parent_taxi_code", "fund_code"
    ];

    private static readonly string[] RequiredColumns =
    ["request_id", "rek_num", "site_code", "date_required", "time_required", "official"];
    ...
    {
        "date_required", "time_required", "request_date", "date_required_2", "time_required_2",
        "trans_man_date", "arrival_time", "date_created", "date_updated"
    };
    ...
    public Task<IEnumerable<Taxi>> GetAllAsync()
    ...
    public async Task<Taxi> CreateAsync(Taxi taxi, int currentUserId)
    {
        ...
        return await GetByIdAsync(requestId)
            ?? throw new InvalidOperationException($"Taxi request {requestId} could not be read after creation.");
    }

    public async Task<Taxi> UpdateAsync(Taxi taxi, int currentUserId)
    {
        ArgumentNullException.ThrowIfNull(taxi);
        var existing = await GetByIdAsync(taxi.request_id)
        ...
        values.RemoveAll(value => value.Column.Equals("request_id", StringComparison.OrdinalIgnoreCase));
        ...
        return await GetByIdAsync(taxi.request_id)
            ?? throw new InvalidOperationException($"Taxi request {taxi.request_id} could not be read after update.");
    }
    ...
    public async Task DeleteAsync(int requestId, int currentUserId)
    {
        ...
        if (columns.ContainsKey("is_deleted"))
        {
            ...
            AddOptionalAssignment(assignments, command, columns, "date_updated", "@dateUpdated", DbType.DateTime2, DateTime.UtcNow);
            AddOptionalAssignment(assignments, command, columns, "modified_by_user_code", "@modifiedByUserCode", DbType.Int32, UserIdOrNull(currentUserId));
            command.CommandText = $"UPDATE [dbo].[{TableName}] SET {string.Join(", ", assignments)} WHERE [request_id] = @requestId AND {GetActiveFilter(columns)}";
        }
        else
        {
            command.CommandText = $"DELETE FROM [dbo].[{TableName}] WHERE [request_id] = @requestId";
        }
    ...
    private async Task<IEnumerable<Taxi>> QueryAsEnumerableAsync(string? predicate = null, Action<DbCommand>? configure = null)
        => await QueryAsync(predicate, configure);

    private async Task<List<Taxi>> QueryAsync(string? predicate = null, Action<DbCommand>? configure = null)
    {
        var columns = await GetAvailableColumnsAsync(RequiredColumns);
        await using var scope = await OpenConnectionAsync();
        await using var command = scope.Connection.CreateCommand();
        ...
        command.CommandText = $"""
            SELECT {string.Join(", ", BusinessColumns.Concat(AuditColumns).Select(column => GetProjection(columns, column, "t")))},
        ...
        while (await reader.ReadAsync()) results.Add(MapTaxi(reader));
        return results;
    }
    ...
    private async Task ExecuteUpdateAsync(int requestId, IReadOnlyList<WriteValue> values, IReadOnlyDictionary<string, ColumnInfo> columns)
    {
    ...
    private static List<WriteValue> BuildValues(Taxi taxi, IReadOnlyDictionary<string, ColumnInfo> columns, int currentUserId, bool isCreate)
    {
        var values = new List<WriteValue>();
        AddRequiredValue(values, columns, "rek_num", "@rekNum", DbType.String, taxi.rek_num?.Trim());
        AddValue(values, columns, "contractor_id", "@contractorId", DbType.Int16, taxi.contractor_id);
        ...
        AddRequiredValue(values, columns, "date_required", "@dateRequired", DbType.DateTime, taxi.date_required);
        AddRequiredValue(values, columns, "time_required", "@timeRequired", DbType.DateTime, taxi.time_required);
        AddValue(values, columns, "vehicle_type_code", "@vehicleTypeCode", DbType.Int16, taxi.vehicle_type_code);
        ...
        short? legacyUserId = columns.ContainsKey("user_access_code")
            ...
            : null;
        AddValue(values, columns, "user_access_code", "@userAccessCode", DbType.Int16, legacyUserId);
        AddValue(values, columns, "request_date", "@requestDate", DbType.DateTime, taxi.request_date ?? DateTime.Now);
        AddValue(values, columns, "confirmed", "@confirmed", DbType.Int16, taxi.confirmed);
        AddValue(values, columns, "sub_contractor_id", "@subContractorId", DbType.Int16, taxi.sub_contractor_id);
        ...
        AddValue(values, columns, "date_required_2", "@dateRequired2", DbType.DateTime, taxi.date_required_2);
        AddValue(values, columns, "time_required_2", "@timeRequired2", DbType.DateTime, taxi.time_required_2);
        ...
        AddValue(values, columns, "destination_12", "@destination12", DbType.String, taxi.destination_12);
        AddValue(values, columns, "destination_22", "@destination22", DbType.String, taxi.destination_22);
        AddValue(values, columns, "destination_32", "@destination32", DbType.String, taxi.destination_32);
        ...
        AddValue(values, columns, "driver_available", "@driverAvailable", DbType.Boolean, taxi.driver_available);
        ...
        AddValue(values, columns, "parent_taxi_code", "@parentTaxiCode", DbType.Int32, taxi.parent_taxi_code);
    ...
    private static void MergeTaxi(Taxi target, Taxi source)
    {
        ...
        if (target.date_required == default) target.date_required = source.date_required;
        if (target.time_required == default) target.time_required = source.time_required;
        target.official = string.IsNullOrWhiteSpace(target.official) ? source.official : target.official;
    ...
    {
        if (string.IsNullOrWhiteSpace(taxi.rek_num)) throw new ArgumentException("Requisition number is required.", nameof(taxi));
        if (taxi.site_code == 0) throw new ArgumentException("Site code is required.", nameof(taxi));
        if (taxi.date_required == default) throw new ArgumentException("Required date is required.", nameof(taxi));
        if (taxi.time_required == default) throw new ArgumentException("Required time is required.", nameof(taxi));
    }
    ...
    private static string GetSqlType(string column)
        => DateColumns.Contains(column) ? "datetime2" : column switch
        {
            "request_id" or "parent_taxi_code" => "int",
            "contractor_id" or "department_code" or "site_code" or "vehicle_type_code" or "user_access_code" or "confirmed" or "sub_contractor_id" => "smallint",
    ...
    private static void AddOptionalAssignment(List<string> assignments, DbCommand command, IReadOnlyDictionary<string, ColumnInfo> columns, string column, string parameter, DbType type, object? value)
    {
        if (!columns.ContainsKey(column)) return;
    ...
    private static short ToLegacyShortUserId(int userId)
    {
        if (userId > short.MaxValue || userId < short.MinValue)
            throw new InvalidOperationException($"User id {userId} cannot be stored in legacy taxi user_access_code column.");
        return (short)userId;
    }
