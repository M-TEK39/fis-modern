...
public sealed class WorkshopRepository : IWorkshopRepository
{
    ...
    [
        "ww_code", "vmf_code", "receive_time", "receive_date", "complete_time", "complete_date",
        "fetch_time", "fetch_date", "driver_name", "contact_name", "contact_tel", "ww_remarks",
        "ww_reason", "accid_mech", "ww_site", "contact_fax", "contact_email", "call_refer", "ww_km",
        "recover_cost", "rem_other_repair", "tow_comp", "tow_amount", "spare_wheel", "jack",
        "wheel_spanner", "radio", "2way", "gear_lock", "keys", "fuel", "inter_exter", "merch_code",
        "date_to_merch", "km_to_merch", "date_from_merch", "km_from_merch", "cost_repair", "points",
        "fa_auth_num", "test_name", "inform_admin", "date_from_ww", "time_from_ww", "fetch_name",
        "job_close", "garage", "blue_light", "monitor_refer"
    ];
    ...
    {
        "receive_time", "complete_time", "fetch_time", "time_from_ww"
    };

    private static readonly HashSet<string> DateColumns = new(StringComparer.OrdinalIgnoreCase)
    {
        "receive_date", "complete_date", "fetch_date", "date_to_merch", "date_from_merch", "date_from_ww",
    ...
    public async Task<Workshop> CreateAsync(Workshop item, int currentUserId)
    {
        ...
        AddValue(values, columns, "date_created", "@dateCreated", DbType.DateTime2, DateTime.UtcNow, includeNull: false);
        AddValue(
            values,
            columns,
            ...
            includeNull: false);
        AddValue(values, columns, "is_deleted", "@isDeleted", DbType.Boolean, false, includeNull: false);

        var code = await ExecuteInsertAsync(values);
        return await GetByIdAsync(code)
            ?? throw new InvalidOperationException($"Workshop with ww_code {code} could not be read after creation.");
    }
    ...
    public async Task<Workshop> UpdateAsync(Workshop item, int currentUserId)
    {
        ...
        var existing = await GetByIdAsync(item.ww_code)
            ?? throw new InvalidOperationException($"Workshop with ww_code {item.ww_code} not found");
        var columns = await GetAvailableColumnsAsync();
        ...
        AddLegacyValues(values, item, columns, includeNulls: false);
        AddValue(values, columns, "date_updated", "@dateUpdated", DbType.DateTime2, DateTime.UtcNow, includeNull: false);
        AddValue(
            values,
            columns,
        ...
        IReadOnlyDictionary<string, ColumnInfo>? suppliedColumns = null)
    {
        ...
        try
        {
            ...
            var order = columns.ContainsKey("receive_date") ? "[receive_date] DESC, " : string.Empty;
            command.CommandText = $"""
        ...
        bool includeNulls)
    {
        ...
        AddValue(values, columns, "vmf_code", "@vmfCode", DbType.Int32, item.vmf_code, includeNulls);
        AddTimeValue(values, columns, "receive_time", "@receiveTime", item.receive_time, includeNulls);
        AddValue(values, columns, "receive_date", "@receiveDate", DbType.DateTime2, item.receive_date, includeNulls);
        AddTimeValue(values, columns, "complete_time", "@completeTime", item.complete_time, includeNulls);
        AddValue(values, columns, "complete_date", "@completeDate", DbType.DateTime2, item.complete_date, includeNulls);
        return values;
    }
        ...
        bool includeNulls)
    {
        ...
        AddValue(values, columns, "fetch_date", "@fetchDate", DbType.DateTime2, item.fetch_date, includeNulls);
        AddValue(values, columns, "driver_name", "@driverName", DbType.String, item.driver_name, includeNulls);
        AddValue(values, columns, "contact_name", "@contactName", DbType.String, item.contact_name, includeNulls);
        AddValue(values, columns, "contact_tel", "@contactTel", DbType.String, item.contact_tel, includeNulls);
        AddValue(values, columns, "ww_remarks", "@wwRemarks", DbType.String, item.ww_remarks, includeNulls);
        AddValue(values, columns, "ww_reason", "@wwReason", DbType.String, item.ww_reason, includeNulls);
        AddValue(values, columns, "accid_mech", "@accidMech", DbType.String, item.accid_mech, includeNulls);
        AddValue(values, columns, "ww_site", "@wwSite", DbType.Int16, item.ww_site, includeNulls);
        AddValue(values, columns, "contact_fax", "@contactFax", DbType.String, item.contact_fax, includeNulls);
        AddValue(values, columns, "contact_email", "@contactEmail", DbType.String, item.contact_email, includeNulls);
        AddValue(values, columns, "call_refer", "@callRefer", DbType.Decimal, item.call_refer, includeNulls);
        AddValue(values, columns, "ww_km", "@wwKm", DbType.Decimal, item.ww_km, includeNulls);
        AddValue(values, columns, "recover_cost", "@recoverCost", DbType.Decimal, item.recover_cost, includeNulls);
        AddValue(values, columns, "rem_other_repair", "@remOtherRepair", DbType.String, item.rem_other_repair, includeNulls);
        AddValue(values, columns, "tow_comp", "@towComp", DbType.Int32, item.tow_comp, includeNulls);
        AddValue(values, columns, "tow_amount", "@towAmount", DbType.Decimal, item.tow_amount, includeNulls);
        AddValue(values, columns, "spare_wheel", "@spareWheel", DbType.String, item.spare_wheel, includeNulls);
        AddValue(values, columns, "jack", "@jack", DbType.String, item.jack, includeNulls);
        AddValue(values, columns, "wheel_spanner", "@wheelSpanner", DbType.String, item.wheel_spanner, includeNulls);
        ...
        AddValue(values, columns, "gear_lock", "@gearLock", DbType.String, item.gear_lock, includeNulls);
        ...
        AddValue(values, columns, "inter_exter", "@interExter", DbType.String, item.inter_exter, includeNulls);
        AddValue(values, columns, "merch_code", "@merchCode", DbType.Int32, item.merch_code, includeNulls);
        AddValue(values, columns, "date_to_merch", "@dateToMerch", DbType.DateTime2, item.date_to_merch, includeNulls);
        AddValue(values, columns, "km_to_merch", "@kmToMerch", DbType.Decimal, item.km_to_merch, includeNulls);
        AddValue(values, columns, "date_from_merch", "@dateFromMerch", DbType.DateTime2, item.date_from_merch, includeNulls);
        AddValue(values, columns, "km_from_merch", "@kmFromMerch", DbType.Decimal, item.km_from_merch, includeNulls);
        AddValue(values, columns, "cost_repair", "@costRepair", DbType.Decimal, item.cost_repair, includeNulls);
        AddValue(values, columns, "points", "@points", DbType.Decimal, item.points, includeNulls);
        AddValue(values, columns, "fa_auth_num", "@faAuthNum", DbType.String, item.fa_auth_num, includeNulls);
        AddValue(values, columns, "test_name", "@testName", DbType.String, item.test_name, includeNulls);
        AddValue(values, columns, "inform_admin", "@informAdmin", DbType.String, item.inform_admin, includeNulls);
        AddValue(values, columns, "date_from_ww", "@dateFromWw", DbType.DateTime2, item.date_from_ww, includeNulls);
        AddTimeValue(values, columns, "time_from_ww", "@timeFromWw", item.time_from_ww, includeNulls);
        AddValue(values, columns, "fetch_name", "@fetchName", DbType.String, item.fetch_name, includeNulls);
        AddValue(values, columns, "job_close", "@jobClose", DbType.String, item.job_close, includeNulls);
        AddValue(values, columns, "garage", "@garage", DbType.String, item.garage, includeNulls);
        AddValue(values, columns, "blue_light", "@blueLight", DbType.String, item.blue_light, includeNulls);
        AddValue(values, columns, "monitor_refer", "@monitorRefer", DbType.Int16, item.monitor_refer, includeNulls);
    }
    ...
    private async Task<short> ExecuteInsertAsync(IReadOnlyList<WriteValue> values)
    {
        ...
        try
        {
            await using var command = connection.CreateCommand();
            command.Transaction = _context.Database.CurrentTransaction?.GetDbTransaction();
            command.CommandText = values.Count == 0
    ...
    private async Task<Dictionary<string, ColumnInfo>> GetAvailableColumnsAsync()
    {
        ...
        try
        {
            ...
            {
                throw new InvalidOperationException("The required workshop compatibility column ww_code is not available.");
            }
        ...
        IReadOnlyDictionary<string, ColumnInfo> columns)
    {
        ...
        if (!completeDate.HasValue && columns.TryGetValue("complete_time", out var completeTimeColumn)
            && !string.Equals(completeTimeColumn.DataType, "time", StringComparison.OrdinalIgnoreCase))
        {
    ...
    private static string GetSqlType(string column)
    {
        ...
        return column switch
        {
            ...
            "vmf_code" or "tow_comp" or "merch_code" or "created_by_user_code" or "modified_by_user_code" => "int",
            "call_refer" or "ww_km" or "recover_cost" or "tow_amount" or "km_to_merch" or "km_from_merch" or "cost_repair" or "points" => "decimal(18, 2)",
            _ => "varchar(1)"
        };
    }
