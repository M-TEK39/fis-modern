...
public sealed class MakeRepository : IMakeRepository
{
    ...
    public async Task<MakePage> GetPageAsync(int page = 1, int pageSize = 24)
    {
        ...
        try
        {
            ...
            command.CommandText =
                $"SELECT {string.Join(", ", projection)} FROM [dbo].[{TableName}] WHERE {notDeletedFilter} ORDER BY [make_description], [make_code] OFFSET @skip ROWS FETCH NEXT @pageSize ROWS ONLY";
            ...
            {
                items.Add(MapMake(reader, availableColumns));
            }

            return new MakePage(items, page, pageSize, total);
        }
        finally
        {
            if (shouldClose)
