...
public sealed class DepartmentRepository : IDepartmentRepository
{
    ...
    public async Task<DepartmentPage> GetPageAsync(int page = 1, int pageSize = 24)
    {
        ...
        try
        {
            ...
            command.CommandText =
                $"SELECT {string.Join(", ", projection)} FROM [dbo].[{TableName}] WHERE {notDeletedFilter} ORDER BY [description], [department_code] OFFSET @skip ROWS FETCH NEXT @pageSize ROWS ONLY";
            ...
            {
                items.Add(MapDepartment(reader, availableColumns));
            }

            return new DepartmentPage(items, page, pageSize, total);
        }
        finally
        {
            if (shouldClose)
