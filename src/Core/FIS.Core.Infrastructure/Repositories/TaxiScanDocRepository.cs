...
public sealed class TaxiScanDocRepository : ITaxiScanDocRepository
{
    ...
    private static readonly string[] RequiredColumns = ["taxi_scandoc_code", "vmf_code", "image", "period_begin", "period_end"];
    private static readonly string[] AuditColumns = ["date_created", "date_updated", "created_by_user_code", "modified_by_user_code", "is_deleted"];
    ...
    public async Task<TaxiScanDoc> CreateAsync(TaxiScanDoc document, int currentUserId)
    {
        ...
        if (document.vmf_code <= 0) throw new ArgumentException("Vehicle code is required.", nameof(document));
        if (string.IsNullOrWhiteSpace(document.image)) throw new ArgumentException("Stored image name is required.", nameof(document));
        if (document.period_begin.HasValue && document.period_end.HasValue && document.period_end < document.period_begin)
            throw new ArgumentException("The certificate end date must be on or after the begin date.", nameof(document));
        ...
        var values = new List<WriteValue>
        {
            ...
            RequiredValue(schema.Columns, "period_begin", "@periodBegin", DbType.DateTime, document.period_begin),
            RequiredValue(schema.Columns, "period_end", "@periodEnd", DbType.DateTime, document.period_end),
        };

        AddValue(values, schema.Columns, "date_created", "@dateCreated", DbType.DateTime2, DateTime.UtcNow);
        AddValue(values, schema.Columns, "created_by_user_code", "@createdByUserCode", DbType.Int32, UserIdOrNull(currentUserId));
        ...
        command.CommandText = $"""
            INSERT INTO [dbo].[{schema.TableName}] ({string.Join(", ", values.Select(value => $"[{value.Column}]"))})
        ...
        return await GetByIdAsync(document.taxi_scandoc_code)
            ?? throw new InvalidOperationException($"Taxi scan document {document.taxi_scandoc_code} could not be read after creation.");
    }
    ...
    public async Task DeleteAsync(int scanDocCode, int currentUserId)
    {
        ...
        if (schema.Columns.ContainsKey("is_deleted"))
        {
            ...
            AddOptionalAssignment(assignments, command, schema.Columns, "date_updated", "@dateUpdated", DbType.DateTime2, DateTime.UtcNow);
            AddOptionalAssignment(assignments, command, schema.Columns, "modified_by_user_code", "@modifiedByUserCode", DbType.Int32, UserIdOrNull(currentUserId));
            command.CommandText = $"UPDATE [dbo].[{schema.TableName}] SET {string.Join(", ", assignments)} WHERE [taxi_scandoc_code] = @scanDocCode AND {GetActiveFilter(schema.Columns)}";
        }
        else
        {
            command.CommandText = $"DELETE FROM [dbo].[{schema.TableName}] WHERE [taxi_scandoc_code] = @scanDocCode";
        }
    ...
    private async Task<List<TaxiScanDoc>> QueryAsync(string? predicate = null, Action<DbCommand>? configure = null)
    {
        var schema = await GetSchemaAsync();
        await using var scope = await OpenConnectionAsync();
        await using var command = scope.Connection.CreateCommand();
        ...
        command.CommandText = $"""
            SELECT {string.Join(", ", RequiredColumns.Concat(AuditColumns).Select(column => GetProjection(schema.Columns, column, "scan")))}
        ...
        while (await reader.ReadAsync()) results.Add(MapDocument(reader));
        return results;
    }
    ...
    private async Task<TableSchema> GetSchemaAsync()
    {
        ...
        var tables = new Dictionary<string, Dictionary<string, ColumnInfo>>(StringComparer.OrdinalIgnoreCase);
        await using var reader = await command.ExecuteReaderAsync();
        ...
        if (table is null) throw new InvalidOperationException("Neither Taxi_ScanDocs nor TaxiScanDocs is available in the connected database.");
        var schema = new TableSchema(table, tables[table]);
        foreach (var required in RequiredColumns.Where(column => !schema.Columns.ContainsKey(column)))
            throw new InvalidOperationException($"The required taxi scan compatibility column {required} is not available.");
        return schema;
    }
    ...
    private static string GetSqlType(string column)
        => column switch
        {
            "taxi_scandoc_code" or "vmf_code" or "created_by_user_code" or "modified_by_user_code" => "int",
            "period_begin" or "period_end" or "date_created" or "date_updated" => "datetime2",
            "is_deleted" => "bit",
            _ => "varchar(1)",
        };

    private static WriteValue RequiredValue(IReadOnlyDictionary<string, ColumnInfo> columns, string column, string parameter, DbType type, object? value)
    {
        if (!columns.ContainsKey(column)) throw new InvalidOperationException($"The required taxi scan compatibility column {column} is not available.");
    ...
    {
        if (columns.ContainsKey(column)) values.Add(new WriteValue(column, parameter, type, value));
    }
