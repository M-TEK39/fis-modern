...
public sealed class TaxiWhiteLogRepository : ITaxiWhiteLogRepository
{
    ...
    ["vmf_code", "start_odo", "end_odo", "start_date", "end_date", "driver", "user_access_code"];

    private static readonly string[] RequiredColumns =
    ["Log_id", "vmf_code", "start_odo", "end_odo", "start_date", "end_date", "driver", "user_access_code"];
    ...
    public async Task<TaxiWhiteLog> CreateAsync(TaxiWhiteLog log, int currentUserId)
    {
        ...
        log.user_access_code = log.user_access_code == 0 ? ToLegacyShortUserId(currentUserId) : log.user_access_code;
        ...
        return await GetByIdAsync(log.Log_id)
            ?? throw new InvalidOperationException($"Taxi white log {log.Log_id} could not be read after creation.");
    }
    ...
    public async Task DeleteAsync(int logId, int currentUserId)
    {
        ...
        if (columns.ContainsKey("is_deleted"))
        {
            ...
            command.CommandText = $"UPDATE [dbo].[{TableName}] SET {string.Join(", ", assignments)} WHERE [Log_id] = @logId AND {GetActiveFilter(columns)}";
        }
        else
        {
    ...
    private async Task<List<TaxiWhiteLog>> QueryAsync(string? predicate = null, Action<DbCommand>? configure = null)
    {
        var columns = await GetAvailableColumnsAsync(RequiredColumns);
        await using var scope = await OpenConnectionAsync();
        await using var command = scope.Connection.CreateCommand();
        ...
        command.CommandText = $"""
            SELECT {string.Join(", ", BusinessColumns.Concat(AuditColumns).Select(column => GetProjection(columns, column, "whiteLog")))},
    ...
    private static List<WriteValue> BuildValues(TaxiWhiteLog log, IReadOnlyDictionary<string, ColumnInfo> columns, bool includeAudit)
    {
        ...
        AddRequiredValue(values, columns, "start_date", "@startDate", DbType.DateTime, log.start_date);
        AddRequiredValue(values, columns, "end_date", "@endDate", DbType.DateTime, log.end_date);
        AddRequiredValue(values, columns, "driver", "@driver", DbType.String, log.driver?.Trim() ?? string.Empty);
        AddRequiredValue(values, columns, "user_access_code", "@userAccessCode", DbType.Int16, log.user_access_code);
        if (includeAudit)
        {
            AddValue(values, columns, "date_created", "@dateCreated", DbType.DateTime2, DateTime.UtcNow);
            AddValue(values, columns, "created_by_user_code", "@createdByUserCode", DbType.Int32, log.created_by_user_code);
    ...
    {
        if (log.vmf_code <= 0) throw new ArgumentException("Vehicle code is required.", nameof(log));
        if (log.end_odo <= log.start_odo) throw new ArgumentException("End odometer must be greater than start odometer.", nameof(log));
        if (log.end_date < log.start_date) throw new ArgumentException("End date must be on or after start date.", nameof(log));
    }
    ...
    private static short ToLegacyShortUserId(int userId)
    {
        if (userId > short.MaxValue || userId < short.MinValue)
            throw new InvalidOperationException($"User id {userId} cannot be stored in legacy taxi white log user_access_code column.");
        return (short)userId;
    }
