...
public sealed class LeaseTariffRepository : ILeaseTariffRepository
{
    ...
    public async Task<LeaseTariff> UpdateAsync(LeaseTariff leaseTariff, int currentUserId)
    {
        ...
        _ = await GetByIdAsync(leaseTariff.lease_tariff_code)
    ...
    public async Task DeleteAsync(int leaseTariffCode, int currentUserId)
    {
        ...
        try
        {
            ...
            if (columns.Contains("is_deleted"))
            {
                ...
                AddAssignment(assignments, command, columns, "date_updated", "@dateUpdated", DbType.DateTime2, DateTime.UtcNow);
                AddAssignment(assignments, command, columns, "modified_by_user_code", "@modifiedByUserCode", DbType.Int32, currentUserId > 0 ? currentUserId : null);
                command.CommandText = $"""
    ...
    private async Task<List<LeaseTariff>> QueryAsync(string? predicate = null, Action<DbCommand>? configure = null)
    {
        ...
        try
        {
            ...
            var where = string.IsNullOrWhiteSpace(predicate) ? GetActiveFilter(columns) : $"{predicate} AND {GetActiveFilter(columns)}";
            command.CommandText = $"""
    ...
    private static List<WriteValue> BuildWriteValues(LeaseTariff tariff, IReadOnlySet<string> columns)
    {
        ...
        AddValue(values, columns, "fixed_tariff", "@fixedTariff", DbType.Decimal, tariff.fixed_tariff);
        AddValue(values, columns, "active", "@active", DbType.Boolean, tariff.active);
        AddValue(values, columns, "excess_kilo_tariff", "@excessKiloTariff", DbType.Decimal, tariff.excess_kilo_tariff);
        return values;
    }
    ...
    private async Task ExecuteUpdateAsync(int code, IReadOnlyList<WriteValue> values, IReadOnlySet<string> columns)
    {
    ...
    {
        if (tariff.vmf_code <= 0) throw new ArgumentException("A valid vehicle code is required.", nameof(tariff));
        if (tariff.end_date < tariff.start_date) throw new ArgumentException("The tariff end date cannot be earlier than its start date.", nameof(tariff));
    }
    ...
    private static string GetProjection(IReadOnlySet<string> columns, string alias, string column, string sqlType)
        => columns.Contains(column) ? $"[l].[{column}] AS [{alias}]" : $"CAST(NULL AS {sqlType}) AS [{alias}]";

    private static string GetOptionalProjection(IReadOnlySet<string> columns, string column, string sqlType, string fallback)
        => columns.Contains(column) ? $"[l].[{column}] AS [{column}]" : $"CAST({fallback} AS {sqlType}) AS [{column}]";

    private static void AddValue(ICollection<WriteValue> values, IReadOnlySet<string> columns, string column, string parameter, DbType type, object? value)
    {
        if (columns.Contains(column)) values.Add(new WriteValue(column, parameter, type, value));
    }

    private static void AddAssignment(ICollection<string> assignments, DbCommand command, IReadOnlySet<string> columns, string column, string parameter, DbType type, object? value)
    {
