...
public sealed class NoticeRepository : INoticeRepository
{
    ...
    public async Task<Notice?> GetByIdAsync(int noticeId)
        => await WithSchemaAsync(async (connection, transaction, schema) =>
            schema.IsModern
                ? await ReadModernSingleAsync(connection, transaction, schema.NoticeColumns, noticeId)
                : await ReadStoredNoticeAsync(connection, transaction, "DEV_SEL_Notice", noticeId));
    ...
    public async Task<IEnumerable<Notice>> GetByDateRangeAsync(DateTime startDate, DateTime endDate)
        => await WithSchemaAsync<IEnumerable<Notice>>(async (connection, transaction, schema) =>
            schema.IsModern
                ? await ReadModernAsync(
                    connection,
                    transaction,
                    ...
                    single: false)
                : await ReadStoredNoticesInDateRangeAsync(connection, transaction, startDate, endDate));
    ...
    public async Task<Notice> CreateAsync(Notice notice, int currentUserId)
    {
        ...
        return await WithSchemaAsync(async (connection, transaction, schema) =>
        {
            ...
            var values = new List<WriteValue>
            {
                ...
                new("notice_person_title", "@noticePersonTitle", DbType.String, notice.notice_person_title)
            };
            ...
            command.CommandText = $"""
                INSERT INTO [dbo].[{NoticeTableName}] ({string.Join(", ", values.Select(value => $"[{value.Column}]"))})
    ...
    public async Task<Notice> UpdateAsync(Notice notice, int currentUserId)
    {
        ...
        return await WithSchemaAsync(async (connection, transaction, schema) =>
        {
            if (!schema.IsModern)
            {
                _ = await ReadStoredNoticeAsync(connection, transaction, "DEV_SEL_Notice", notice.notice_id)
                    ?? throw new InvalidOperationException($"Notice with notice_id {notice.notice_id} not found");
                await ExecuteLegacyNoticeWriteAsync(connection, transaction, notice, notice.notice_id, notice.notice_date ?? DateTime.UtcNow);
                return notice;
            }
            ...
            command.CommandText = $"""
                ...
                """;
            AddParameters(command, values);
            NoticeRepositorySupport.AddParameter(command, "@noticeId", DbType.Int32, notice.notice_id);
    ...
    public async Task DeleteAsync(int noticeId, int currentUserId)
        => await WithSchemaAsync<int>(async (connection, transaction, schema) =>
        {
            ...
            if (columns.Contains("is_deleted"))
            {
                ...
                command.CommandText = $"UPDATE [dbo].[{NoticeTableName}] SET {string.Join(", ", assignments)} WHERE [notice_id] = @noticeId AND {ActiveFilter(string.Empty, schema.NoticeColumns.Contains("is_deleted"))}";
            }
            else
            {
                command.CommandText = $"DELETE FROM [dbo].[{NoticeTableName}] WHERE [notice_id] = @noticeId";
            }
        ...
        bool includeActiveSchedule = false)
    {
        var projection = NoticeColumns
            ...
            .Concat(OptionalNoticeColumns.Select(column => OptionalProjection("n", column, noticeColumns)))
            .ToArray();
        ...
        DateTime noticeDate)
    {
        ...
        NoticeRepositorySupport.AddStringParameter(command, "@newResPersonTItle", notice.notice_person_title);
    ...
    private static Notice MapStoredNotice(DbDataReader reader)
        => new()
        {
            notice_id = NoticeRepositorySupport.ReadInt32(reader, "notice_id", "NoticeID", "ID") ?? 0,
            notice_date = NoticeRepositorySupport.ReadDateTime(reader, "notice_date", "Date"),
            notice_from = NoticeRepositorySupport.ReadString(reader, "notice_from", "From", "FromField"),
            notice_title = NoticeRepositorySupport.ReadString(reader, "notice_title", "Title", "TitleField"),
            notice_body = NoticeRepositorySupport.ReadString(reader, "notice_body", "Body", "BodyText"),
            notice_person = NoticeRepositorySupport.ReadString(reader, "notice_person", "Person", "ResponsiblePerson"),
            notice_person_title = NoticeRepositorySupport.ReadString(reader, "notice_person_title", "PersonTitle", "ResponsiblePersonTitle"),
