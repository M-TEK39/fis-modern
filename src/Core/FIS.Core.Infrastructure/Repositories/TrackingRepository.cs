...
public class TrackingRepository : ITrackingRepository
{
    ...
    public TrackingRepository(FisDbContext context) => _context = context ?? throw new ArgumentNullException(nameof(context));

    public async Task<Tracking?> GetByIdAsync(short trackCode)
        => (await QueryAsync("t.[track_code] = @trackCode", command => AddParameter(command, "@trackCode", DbType.Int16, trackCode))).SingleOrDefault();
    ...
    public async Task<IEnumerable<Tracking>> GetByVehicleAsync(int vmfCode)
        => await QueryAsync("t.[vmf_code] = @vmfCode", command => AddParameter(command, "@vmfCode", DbType.Int32, vmfCode));
    ...
    public async Task<Tracking> CreateAsync(Tracking tracking, int currentUserId)
    {
        ...
        AddValue(values, columns, "track_num", "@trackNum", DbType.String, tracking.track_num, true);
        AddValue(values, columns, "gg_previous", "@ggPrevious", DbType.String, tracking.gg_previous, true);
        AddValue(values, columns, "gg_follow", "@ggFollow", DbType.String, tracking.gg_follow, true);
        AddValue(values, columns, "install_date", "@installDate", DbType.DateTime, tracking.install_date, true);
        AddValue(values, columns, "remove_date", "@removeDate", DbType.DateTime, tracking.remove_date, true);
        AddValue(values, columns, "track_status", "@trackStatus", DbType.String, tracking.track_status, true);
        AddValue(values, columns, "track_type", "@trackType", DbType.String, tracking.track_type, true);
        AddValue(values, columns, "track_note", "@trackNote", DbType.String, tracking.track_note, true);
        ...
        command.CommandText = $"INSERT INTO [dbo].[{TableName}] ({string.Join(", ", values.Select(value => $"[{value.Column}]"))}) OUTPUT INSERTED.[track_code] VALUES ({string.Join(", ", values.Select(value => value.Parameter))})";
        AddParameters(command, values);
        var id = Convert.ToInt16(await command.ExecuteScalarAsync());
        return await GetByIdAsync(id) ?? throw new InvalidOperationException("Created tracking record could not be read.");
    }
    ...
    public async Task<Tracking> UpdateAsync(Tracking tracking, int currentUserId)
    {
        ...
        await using var command = scope.Connection.CreateCommand();
        command.Transaction = CurrentTransaction;
        command.CommandText = $"UPDATE [dbo].[{TableName}] SET {string.Join(", ", values.Select(value => $"[{value.Column}] = {value.Parameter}"))} WHERE [track_code] = @trackCode";
        AddParameters(command, values);
        AddParameter(command, "@trackCode", DbType.Int16, tracking.track_code);
        if (await command.ExecuteNonQueryAsync() == 0) throw new KeyNotFoundException($"Tracking record not found with code: {tracking.track_code}");
        return await GetByIdAsync(tracking.track_code) ?? throw new KeyNotFoundException($"Tracking record not found with code: {tracking.track_code}");
    }
    ...
    public async Task DeleteAsync(short trackCode, int currentUserId)
    {
        ...
        if (columns.ContainsKey("is_deleted"))
        {
            ...
            command.CommandText = $"UPDATE [dbo].[{TableName}] SET {string.Join(", ", assignments)} WHERE [track_code] = @trackCode";
        }
        else command.CommandText = $"DELETE FROM [dbo].[{TableName}] WHERE [track_code] = @trackCode";
        AddParameter(command, "@trackCode", DbType.Int16, trackCode);
        if (await command.ExecuteNonQueryAsync() == 0) throw new KeyNotFoundException($"Tracking record not found with code: {trackCode}");
    }

    private async Task<List<Tracking>> QueryAsync(string? predicate = null, Action<DbCommand>? configure = null)
    {
        var columns = await GetAvailableColumnsAsync();
        await using var scope = await OpenConnectionAsync();
        await using var command = scope.Connection.CreateCommand();
    ...
    private async Task<Dictionary<string, ColumnInfo>> GetAvailableColumnsAsync()
    {
        ...
        if (RequiredColumns.Any(column => !columns.ContainsKey(column))) throw new InvalidOperationException("The Tracking compatibility table is missing its required legacy columns.");
        return columns;
    }
    ...
    private static string DateCreatedExpression(IReadOnlyDictionary<string, ColumnInfo> columns)
        => columns.ContainsKey("date_created") ? "COALESCE(t.[date_created], t.[install_date])" : OptionalExpression(columns, "install_date", "datetime2");

    private static string CreatedByExpression(IReadOnlyDictionary<string, ColumnInfo> columns)
        => columns.ContainsKey("created_by_user_code") ? "t.[created_by_user_code]" : "CAST(NULL AS int)";
    ...
    private static short? ReadShort(DbDataReader reader, string name) => reader.IsDBNull(reader.GetOrdinal(name)) ? null : Convert.ToInt16(reader[name]);
    ...
    private sealed class ConnectionScope(DbConnection connection, bool shouldClose) : IAsyncDisposable
    {
        ...
        public async ValueTask DisposeAsync() { if (shouldClose) await Connection.CloseAsync(); }
    }
}
