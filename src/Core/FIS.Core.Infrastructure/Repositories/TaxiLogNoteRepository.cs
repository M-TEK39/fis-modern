...
public sealed class TaxiLogNoteRepository : ITaxiLogNoteRepository
{
    ...
    private static readonly string[] RequiredColumns = ["taxi_log_note_code", "taxi_log_note_description"];
    ...
    public async Task<IEnumerable<TaxiLogNote>> GetAllAsync()
    {
        ...
        command.CommandText = $"""
            SELECT [taxi_log_note_code], [taxi_log_note_description],
                   {string.Join(", ", AuditColumns.Select(column => GetProjection(columns, column)))}
            FROM [dbo].[{TableName}]
