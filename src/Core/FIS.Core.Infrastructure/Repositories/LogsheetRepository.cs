...
public class LogsheetRepository : ILogsheetRepository
{
    ...
    public async Task<Logsheet> CreateAsync(Logsheet logsheet, int currentUserId)
    {
        ...
        if (logsheet.trans_date == default) logsheet.trans_date = now;
        if (logsheet.trans_time == default) logsheet.trans_time = now.TimeOfDay;
        if (logsheet.department_code <= 0) logsheet.department_code = logsheet.site_code;
    ...
    public async Task<Logsheet> UpdateAsync(Logsheet logsheet, int currentUserId)
    {
        ...
        if (logsheet.trans_date == default) logsheet.trans_date = existing.trans_date;
        if (logsheet.trans_time == default) logsheet.trans_time = existing.trans_time;
        if (logsheet.department_code <= 0) logsheet.department_code = existing.department_code;
        _context.Entry(existing).CurrentValues.SetValues(logsheet);
        existing.date_updated = DateTime.UtcNow;
        existing.modified_by_user_code = currentUserId > 0 ? currentUserId : existing.modified_by_user_code;
        await _context.SaveChangesAsync();
        return existing;
    }
    ...
    public async Task DeleteAsync(int logCode, int currentUserId)
    {
        ...
        if (logsheet != null)
        {
            ...
            logsheet.modified_by_user_code = currentUserId > 0 ? currentUserId : logsheet.modified_by_user_code;
            await _context.SaveChangesAsync();
        }
    }
