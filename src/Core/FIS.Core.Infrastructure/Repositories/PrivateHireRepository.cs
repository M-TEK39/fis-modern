...
public sealed class PrivateHireRepository : IPrivateHireRepository
{
    ...
    [
        "PHV_code", "registration_number", "model_code", "site_code", "contracted_to",
        "engine_number", "chassis_number", "year_manufactured", "bank_code", "colour",
        "tank_capacity", "contractor_id", "fuel_card", "fuel_card_receiver", "take_on_date",
        "take_on_odo", "return_date", "return_odo", "km_tariff", "daily_tariff", "hourly_tariff",
        "model_desc"
    ];

    private static readonly string[] ContractorBusinessColumns =
    [
        "contractor_id", "contractor_name", "physical_address", "postal_address", "tel_number",
        "fax_number", "email_address", "contact_person", "active", "type", "quotations",
        "project_name", "project_begdat", "project_enddat"
    ];

    private static readonly string[] ContractorRequiredColumns = ["contractor_id", "contractor_name"];
    ...
    {
        "take_on_date", "return_date", "date_created", "date_updated", "project_begdat", "project_enddat"
    };
    ...
    public async Task<PrivateHire?> GetByIdAsync(int privateHireCode)
        => (await QueryPrivateHiresAsync(
            ...
            command => AddParameter(command, "@privateHireCode", DbType.Int16, checked((short)privateHireCode))))
            .SingleOrDefault();
    ...
    public async Task<IEnumerable<PrivateHire>> GetActiveHiresAsync()
        => await QueryPrivateHiresAsync(
            ...
            command => AddParameter(command, "@recentReturnDate", DbType.DateTime, DateTime.Now.AddDays(-30)));
    ...
    public async Task<PrivateHire> CreateAsync(PrivateHire privateHire, int currentUserId)
    {
        ...
        var columns = await GetAvailableColumnsAsync(PrivateHireTableName, PrivateHireBusinessColumns);
        ...
        privateHire.PHV_code = checked((short)await ExecuteInsertAsync(PrivateHireTableName, "PHV_code", values));
        return await GetByIdAsync(privateHire.PHV_code)
            ?? throw new InvalidOperationException($"Private hire {privateHire.PHV_code} could not be read after creation.");
    }

    public async Task UpdateAsync(PrivateHire privateHire, int currentUserId)
    {
        ArgumentNullException.ThrowIfNull(privateHire);
        var existing = await GetByIdAsync(privateHire.PHV_code)
            ?? throw new InvalidOperationException($"Private hire {privateHire.PHV_code} not found");
        ...
        await ExecuteUpdateAsync(PrivateHireTableName, "PHV_code", privateHire.PHV_code, values, columns);
    ...
    {
        if (string.IsNullOrWhiteSpace(searchTerm)) return await QueryPrivateHiresAsync();
    ...
    public async Task<PrivateHireContractorRecord?> GetContractorByIdAsync(int contractorId)
        => (await QueryContractorsAsync(
            ...
            command => AddParameter(command, "@contractorId", DbType.Int16, checked((short)contractorId))))
            .SingleOrDefault();

    public async Task<PrivateHireContractorRecord> CreateContractorAsync(PrivateHireContractorRecord contractor, int currentUserId)
    {
        ...
        var columns = await GetAvailableColumnsAsync(ContractorsTableName, ContractorRequiredColumns);
        ...
        return await GetContractorByIdAsync(contractor.contractor_id)
            ?? throw new InvalidOperationException($"Contractor {contractor.contractor_id} could not be read after creation.");
    }

    public async Task UpdateContractorAsync(PrivateHireContractorRecord contractor, int currentUserId)
    {
        ArgumentNullException.ThrowIfNull(contractor);
        var existing = await GetContractorByIdAsync(contractor.contractor_id)
            ?? throw new InvalidOperationException($"Contractor {contractor.contractor_id} not found");
        ...
        AddValue(values, columns, "date_updated", "@dateUpdated", DbType.DateTime2, now);
        AddValue(values, columns, "modified_by_user_code", "@modifiedByUserCode", DbType.Int32, UserIdOrNull(currentUserId));
        await ExecuteUpdateAsync(ContractorsTableName, "contractor_id", contractor.contractor_id, values, columns);
    ...
    private async Task<List<PrivateHire>> QueryPrivateHiresAsync(string? predicate = null, Action<DbCommand>? configure = null)
    {
        var columns = await GetAvailableColumnsAsync(PrivateHireTableName, PrivateHireBusinessColumns);
        ...
        command.CommandText = $"""
            SELECT {string.Join(", ", PrivateHireBusinessColumns.Concat(OptionalAuditColumns).Select(column => GetProjection(columns, column)))}
        ...
        while (await reader.ReadAsync()) results.Add(MapPrivateHire(reader, columns));
        return results;
    }

    private async Task<List<PrivateHireContractorRecord>> QueryContractorsAsync(string? predicate = null, Action<DbCommand>? configure = null)
    {
        var columns = await GetAvailableColumnsAsync(ContractorsTableName, ContractorRequiredColumns);
        await using var scope = await OpenConnectionAsync();
        await using var command = scope.Connection.CreateCommand();
        ...
        if (!string.IsNullOrWhiteSpace(predicate)) conditions.Add($"({predicate})");
        command.CommandText = $"""
            SELECT {string.Join(", ", ContractorBusinessColumns.Concat(OptionalAuditColumns).Select(column => GetProjection(columns, column)))}
        ...
        await using var reader = await command.ExecuteReaderAsync();
        while (await reader.ReadAsync()) results.Add(MapContractor(reader, columns));
        return results;
    }

    private async Task<Dictionary<string, ColumnInfo>> GetAvailableColumnsAsync(string tableName, IReadOnlyCollection<string> requiredColumns)
    {
        await using var scope = await OpenConnectionAsync();
        await using var command = scope.Connection.CreateCommand();
        command.Transaction = _context.Database.CurrentTransaction?.GetDbTransaction();
    ...
    private async Task<int> ExecuteInsertAsync(string tableName, string keyColumn, IReadOnlyList<WriteValue> values)
    {
        await using var scope = await OpenConnectionAsync();
        await using var command = scope.Connection.CreateCommand();
        command.Transaction = _context.Database.CurrentTransaction?.GetDbTransaction();
    ...
    private async Task ExecuteUpdateAsync(string tableName, string keyColumn, int key, IReadOnlyList<WriteValue> values, IReadOnlyDictionary<string, ColumnInfo> columns)
    {
    ...
    private async Task DeleteAsync(string tableName, string keyColumn, int key, int currentUserId)
    {
        ...
        if (columns.ContainsKey("is_deleted"))
        {
            ...
            if (columns.ContainsKey("modified_by_user_code"))
            {
                ...
                AddParameter(command, "@modifiedByUserCode", DbType.Int32, UserIdOrNull(currentUserId));
            }
            command.CommandText = $"UPDATE [dbo].[{tableName}] SET {string.Join(", ", assignments)} WHERE [{keyColumn}] = @key AND {GetActiveFilter(columns)}";
        }
        else
        {
    ...
    private static List<WriteValue> BuildPrivateHireValues(PrivateHire privateHire, IReadOnlyDictionary<string, ColumnInfo> columns)
    {
        var values = new List<WriteValue>();
        AddRequiredValue(values, columns, "registration_number", "@registrationNumber", DbType.String, privateHire.registration_number);
        AddRequiredValue(values, columns, "model_code", "@modelCode", DbType.Int16, privateHire.model_code);
        AddRequiredValue(values, columns, "site_code", "@siteCode", DbType.Int32, privateHire.site_code);
        AddValue(values, columns, "contracted_to", "@contractedTo", DbType.Int32, privateHire.contracted_to);
        AddValue(values, columns, "engine_number", "@engineNumber", DbType.String, privateHire.engine_number);
        AddValue(values, columns, "chassis_number", "@chassisNumber", DbType.String, privateHire.chassis_number);
        AddValue(values, columns, "year_manufactured", "@yearManufactured", DbType.String, privateHire.year_manufactured);
        ...
        AddValue(values, columns, "tank_capacity", "@tankCapacity", DbType.Int32, privateHire.tank_capacity);
        AddRequiredValue(values, columns, "contractor_id", "@contractorId", DbType.Int16, privateHire.contractor_id);
        AddValue(values, columns, "fuel_card", "@fuelCard", DbType.String, privateHire.fuel_card);
        AddValue(values, columns, "fuel_card_receiver", "@fuelCardReceiver", DbType.String, privateHire.fuel_card_receiver);
        AddRequiredValue(values, columns, "take_on_date", "@takeOnDate", DbType.DateTime, privateHire.take_on_date);
        AddRequiredValue(values, columns, "take_on_odo", "@takeOnOdo", DbType.Int32, privateHire.take_on_odo);
        AddValue(values, columns, "return_date", "@returnDate", DbType.DateTime, privateHire.return_date);
        AddRequiredValue(values, columns, "return_odo", "@returnOdo", DbType.Int32, privateHire.return_odo);
        AddValue(values, columns, "km_tariff", "@kmTariff", DbType.Decimal, privateHire.km_tariff);
        AddValue(values, columns, "daily_tariff", "@dailyTariff", DbType.Decimal, privateHire.daily_tariff);
        AddValue(values, columns, "hourly_tariff", "@hourlyTariff", DbType.Decimal, privateHire.hourly_tariff);
        AddValue(values, columns, "model_desc", "@modelDesc", DbType.String, privateHire.model_desc);
        return values;
    }

    private static List<WriteValue> BuildContractorValues(PrivateHireContractorRecord contractor, IReadOnlyDictionary<string, ColumnInfo> columns)
    {
        var values = new List<WriteValue>();
        AddRequiredValue(values, columns, "contractor_name", "@contractorName", DbType.String, contractor.contractor_name?.Trim());
        AddValue(values, columns, "physical_address", "@physicalAddress", DbType.String, contractor.physical_address?.Trim());
        AddValue(values, columns, "postal_address", "@postalAddress", DbType.String, contractor.postal_address?.Trim());
        AddValue(values, columns, "tel_number", "@telNumber", DbType.String, contractor.tel_number?.Trim());
        AddValue(values, columns, "fax_number", "@faxNumber", DbType.String, contractor.fax_number?.Trim());
        AddValue(values, columns, "email_address", "@emailAddress", DbType.String, contractor.email_address?.Trim());
        AddValue(values, columns, "contact_person", "@contactPerson", DbType.String, contractor.contact_person?.Trim());
        ...
        AddValue(values, columns, "quotations", "@quotations", DbType.Boolean, contractor.quotations);
        AddValue(values, columns, "project_name", "@projectName", DbType.String, contractor.project_name?.Trim());
        AddValue(values, columns, "project_begdat", "@projectBegdat", DbType.Date, contractor.project_begdat);
        AddValue(values, columns, "project_enddat", "@projectEnddat", DbType.Date, contractor.project_enddat);
        return values;
    }
    ...
    private static void MergePrivateHire(PrivateHire target, PrivateHire source)
    {
        ...
        if (target.model_code == 0) target.model_code = source.model_code;
        if (target.site_code == 0) target.site_code = source.site_code;
        ...
        if (target.contractor_id == 0) target.contractor_id = source.contractor_id;
        target.fuel_card ??= source.fuel_card;
        target.fuel_card_receiver ??= source.fuel_card_receiver;
        if (target.take_on_date == default) target.take_on_date = source.take_on_date;
        if (target.take_on_odo == 0) target.take_on_odo = source.take_on_odo;
        target.return_date ??= source.return_date;
        if (target.return_odo == 0) target.return_odo = source.return_odo;
    ...
    private static void MergeContractor(PrivateHireContractorRecord target, PrivateHireContractorRecord source)
    {
        target.contractor_name = string.IsNullOrWhiteSpace(target.contractor_name) ? source.contractor_name : target.contractor_name;
    ...
    {
        if (string.IsNullOrWhiteSpace(privateHire.registration_number)) throw new ArgumentException("Registration number is required.", nameof(privateHire));
        if (privateHire.take_on_date == default) throw new ArgumentException("Take-on date is required.", nameof(privateHire));
    }

    private static PrivateHire MapPrivateHire(DbDataReader reader, IReadOnlyDictionary<string, ColumnInfo> columns)
        => new()
        {
    ...
    private static PrivateHireContractorRecord MapContractor(DbDataReader reader, IReadOnlyDictionary<string, ColumnInfo> columns)
        => new()
        {
            contractor_id = ReadInt16(reader, "contractor_id") ?? 0,
    ...
    private static string GetSqlType(string column)
        => DateColumns.Contains(column)
            ? column.EndsWith("dat", StringComparison.OrdinalIgnoreCase) ? "date" : "datetime2"
            : column switch
            {
                "PHV_code" or "contractor_id" or "model_code" or "active" => "smallint",
                "site_code" or "contracted_to" or "tank_capacity" or "take_on_odo" or "return_odo" => "int",
