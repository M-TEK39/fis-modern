...
public sealed class ExtraCodeRepository : IExtraCodeRepository
{
    ...
    public async Task UpdateAsync(ExtraCode extraCode, int currentUserId)
    {
        ...
        var existing = await FindByIdAsync(extraCode.extra_code)
        ...
        extraCode.date_updated = now;
        extraCode.modified_by_user_code = currentUserId > 0
            ? currentUserId
            : existing.Code.modified_by_user_code;
    ...
    public async Task DeleteAsync(short extraCode, int currentUserId)
    {
        ...
        try
        {
            ...
            if (availableColumns.Contains("is_deleted"))
            {
                ...
                command.CommandText = $"UPDATE [dbo].[{TableName}] SET {string.Join(", ", updates)} WHERE [extra_code] = @extraCode";
            }
            else
            {
                command.CommandText = $"DELETE FROM [dbo].[{TableName}] WHERE [extra_code] = @extraCode";
            }
        ...
        bool includeCreateAudit = true)
    {
        var values = new List<WriteValue>
        {
            ...
            new("category_type_code", "@categoryTypeCode", DbType.Int32, extraCode.category_type_code),
            ...
            new("modified_by_user_code", "@modifiedByUserCode", DbType.Int32, extraCode.modified_by_user_code)
        };

        if (includeCreateAudit)
        {
            values.Add(new("date_created", "@dateCreated", DbType.DateTime2, extraCode.date_created));
            values.Add(new("created_by_user_code", "@createdByUserCode", DbType.Int32, extraCode.created_by_user_code));
        ...
        Action<DbCommand>? configure = null)
    {
        ...
        try
        {
            ...
            command.CommandText = $"SELECT {string.Join(", ", projection)} FROM [dbo].[{TableName}] WHERE {string.Join(" AND ", conditions)} ORDER BY [extra_description], [extra_code]";
    ...
    private async Task<short> ExecuteInsertAsync(IReadOnlyList<WriteValue> values)
    {
        ...
        try
        {
            await using var command = connection.CreateCommand();
            command.Transaction = _context.Database.CurrentTransaction?.GetDbTransaction();
            command.CommandText = $"INSERT INTO [dbo].[{TableName}] ({string.Join(", ", values.Select(value => $"[{value.Column}]"))}) OUTPUT INSERTED.[extra_code] VALUES ({string.Join(", ", values.Select(value => value.Parameter))})";
        ...
        IReadOnlyList<WriteValue> values)
    {
        ...
        try
        {
            await using var command = connection.CreateCommand();
            command.Transaction = _context.Database.CurrentTransaction?.GetDbTransaction();
            command.CommandText = $"UPDATE [dbo].[{TableName}] SET {string.Join(", ", values.Select(value => $"[{value.Column}] = {value.Parameter}"))} WHERE [extra_code] = @extraCode";
            AddParameters(command, values);
            AddParameter(command, "@extraCode", DbType.Int16, extraCode);
            await command.ExecuteNonQueryAsync();
        }
    ...
    private async Task<HashSet<string>> GetAvailableColumnsAsync()
    {
        ...
        try
        {
            ...
            if (missing.Length > 0)
            {
                throw new InvalidOperationException($"The extra_codes table is missing required columns: {string.Join(", ", missing)}");
            }
