...
const ResponsiveContainer = React.lazy(async () => {
  ...
  return {
    default: recharts.ResponsiveContainer as React.ComponentType<{ children: React.ReactNode }>,
  };
});

const ChartTooltip = React.lazy(async () => {
  const recharts = await import("recharts");
  return { default: recharts.Tooltip as React.ComponentType<unknown> };
});

const ChartLegend = React.lazy(async () => {
  const recharts = await import("recharts");
  return { default: recharts.Legend as React.ComponentType<unknown> };
});
...
function ChartTooltipContent({
  ...
  const labelConfig = getPayloadConfigFromPayload(config, labelItem, labelConfigKey);
  const labelValue =
