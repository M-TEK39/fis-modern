...
export const DEFAULT_MAKE_PAGE_SIZE = 24;
...
export async function getMakesPage(
  ...
  const payload = await readJson(await requestApi(`api/make/page?${params.toString()}`));

  if (!isRecord(payload) || !Array.isArray(payload.items)) {
    throw new MakeApiError("invalid-response", "The FIS API returned an invalid make page.");
  }
  ...
  if (!metadata) {
    throw new MakeApiError(
      "invalid-response",
      "The FIS API returned incomplete make pagination metadata.",
    );
  }

  return {
    items: payload.items.map(mapMake).filter((make): make is MakeRecord => make !== null),
    ...metadata,
  };
}
