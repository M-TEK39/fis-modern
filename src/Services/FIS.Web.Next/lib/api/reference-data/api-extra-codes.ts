...
export const DEFAULT_EXTRA_CODE_PAGE_SIZE = 24;
...
export type ExtraCodePage = {
  items: ExtraCodeRecord[];
  page: number;
  pageSize: number;
  total: number;
  totalPages: number;
...
function normalizePageSize(value: number | undefined) {
  const pageSize =
    Number.isInteger(value) && (value ?? 0) > 0
      ? (value ?? DEFAULT_EXTRA_CODE_PAGE_SIZE)
...
export async function getExtraCodesPage(
  options: { page?: number; pageSize?: number; searchTerm?: string } = {},
): Promise<ExtraCodePage> {
  ...
  const payload = await readJson(await requestApi(`api/extracode/page?${params.toString()}`));
  if (!isRecord(payload) || !Array.isArray(payload.items)) {
    throw new ExtraCodeApiError(
      "invalid-response",
      "The FIS API returned an invalid extra code page.",
    );
  }
  ...
  if (!metadata) {
    throw new ExtraCodeApiError(
      "invalid-response",
      "The FIS API returned incomplete extra code pagination metadata.",
    );
  }

  return {
    items: payload.items.map(mapExtraCode).filter((item): item is ExtraCodeRecord => item !== null),
    ...metadata,
  };
}
