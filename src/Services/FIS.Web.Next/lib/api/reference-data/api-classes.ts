...
export const DEFAULT_CLASS_PAGE_SIZE = 24;
...
export async function getClassesPage(
  ...
  const payload = await readJson(await requestApi(`api/class/page?${params.toString()}`));

  if (!isRecord(payload) || !Array.isArray(payload.items)) {
    throw new ClassApiError("invalid-response", "The FIS API returned an invalid class page.");
  }
  ...
  if (!metadata) {
    throw new ClassApiError(
      "invalid-response",
      "The FIS API returned incomplete class pagination metadata.",
    );
  }

  return {
    items: payload.items.map(mapClass).filter((item): item is ClassRecord => item !== null),
    ...metadata,
  };
}
