...
import { FinanceApiError, getFinanceJson, runFinanceAction, type FinanceRow } from "@/lib/api-finance";
...
function rowValue(item: unknown): FinanceRow | null {
  ...
  for (const [key, itemValue] of Object.entries(item)) {
    row[key] = typeof itemValue === "string" || typeof itemValue === "number" || typeof itemValue === "boolean" || itemValue === null
      ? itemValue
...
function rowsFrom(valueToMap: unknown) {
  if (Array.isArray(valueToMap)) return valueToMap.map(rowValue).filter((row): row is FinanceRow => row !== null);
  if (!isRecord(valueToMap)) return [];
  const rows = value(valueToMap, "dataRows", "DataRows", "rows", "Rows", "items", "Items");
  return Array.isArray(rows) ? rows.map(rowValue).filter((row): row is FinanceRow => row !== null) : [];
}

export function mapFinanceReport(payload: unknown, fallbackTitle: string): FinanceReport {
  const rows = rowsFrom(payload);
  const record = isRecord(payload) ? payload : null;
  const title = record && typeof value(record, "title", "Title") === "string"
...
export async function getBillingHistory(vmfCode: number, financialYear: number) {
  const payload = await getFinanceJson(`api/report/billing/history/${vmfCode}?financialYear=${financialYear}`);
  return mapFinanceReport(payload, "Vehicle billing history");
}

export async function getReversalTree(journalNumber: string) {
  const payload = await getFinanceJson(`api/finance/reports/reversals-tree/${encodeURIComponent(journalNumber)}`);
...
export function dedicatedFinanceReportPath(action: string, params: DedicatedFinanceReportParams) {
  ...
  if (!report) throw new FinanceApiError("invalid-response", "The requested Finance report is not available.");
  const query = new URLSearchParams({ id: String(params.id), postingMonthCode: String(params.postingMonthCode), filterBy: params.filterBy, format: params.format });
  return `api/finance/reports/${report.endpoint}?${query.toString()}`;
}

export async function getDedicatedFinanceReportData(action: string, params: Omit<DedicatedFinanceReportParams, "format">) {
...
export const FINANCE_REPORT_ACTIONS = [
  "summary-by-cost-type", "summary-by-cost-type-journal", "summary-html", "summary-pdf",
  "detailed-html", "detailed-pdf", "detailed-table", "detailed-excel", "detailed-vip-taxi",
  "detailed-vip-taxi-pdf", "detailed-fuel-pdf", "detailed-fuel-excel", "detailed-toll-oil-pdf",
  "detailed-toll-oil-excel", "detailed-surcharge-pdf", "detailed-surcharge-excel",
] as const;
