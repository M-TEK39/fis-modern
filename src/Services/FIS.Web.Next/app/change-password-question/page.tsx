...
function getMessage(result: string | undefined) {
  switch (result) {
    case "success":
      return { tone: "success", text: "Password and security question changed successfully." } as const;
    case "missing-username":
      return { tone: "error", text: "Please select a username." } as const;
    case "missing-password":
      return { tone: "error", text: "Complete the old, new, and confirmation password fields." } as const;
    case "password-mismatch":
      return { tone: "error", text: "The Confirm New Password must match the New Password entry." } as const;
    ...
    case "unavailable":
      return { tone: "error", text: "The password/question service is unavailable. Retry when the FIS API is available." } as const;
    case "invalid-response":
      return { tone: "error", text: "The password/question update failed. Check the old password and try again." } as const;
    default:
      return result ? { tone: "error", text: "The password/question update could not be completed." } as const : null;
  }
}

export default async function ChangePasswordQuestionPage({ searchParams }: Readonly<{ searchParams: SearchParams }>) {
  await connection();
  const session = await getSession();

  if (session.status === "anonymous") redirect("/login");
  if (session.status === "expired") {
    return <main className="page-shell vehicle-page-shell"><SessionRecovery returnPath="/change-password-question" /></main>;
  }
  if (session.status === "unavailable") {
    return <main className="page-shell vehicle-page-shell"><section className="vehicle-status-card" role="alert"><p className="eyebrow">API unavailable</p><h2>Change Password and Question could not be opened.</h2><p className="muted-copy">Retry when the FIS API is available.</p></section></main>;
  }
  ...
  const username = canManageOthers ? requestedUsername : session.userAccessCode?.trim() || session.email?.trim() || "";
  ...
  if (canManageOthers) {
    try {
      ...
      if (error instanceof UserAdminApiError && error.reason === "unauthorized") {
        return <main className="page-shell vehicle-page-shell"><section className="vehicle-status-card" role="alert"><p className="eyebrow">Access restricted</p><h2>You do not have permission to load user accounts.</h2></section></main>;
      }

      console.error("FIS password/question user lookup failed", error instanceof Error ? error.message : "unknown error");
      return <main className="page-shell vehicle-page-shell"><section className="vehicle-status-card" role="alert"><p className="eyebrow">API unavailable</p><h2>Users could not be loaded.</h2><p className="muted-copy">Retry when the FIS API is available.</p></section></main>;
    }
  }
    ...
    <main className="page-shell vehicle-page-shell">
      <section className="vehicle-card" aria-labelledby="change-password-question-title">
        <header className="vehicle-page-header">
          ...
          <Link className="button button-secondary" href={canManageOthers ? "/UserAdmin/UserAdminMenu.aspx" : "/home"}>
            {canManageOthers ? "Menu" : "Home"}
          </Link>
        </header>

        <p className="muted-copy">The security answer is case sensitive. Keep it safe so the password recovery process remains available.</p>
