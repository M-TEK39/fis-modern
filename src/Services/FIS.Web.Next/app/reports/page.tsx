...
const QUICK_LINKS = [
  ...
  ["6) Vehicles with high distances in all departments (Per Department)", "/reports/high-distance-all"],
  ...
  ["9.6) VIP and Pool utilization including income - current month", "/reports/vip-pool-income-current"],
  ["9.6.1) VIP and Pool utilization including income - previous months", "/reports/vip-pool-income-previous"],
...
export default async function ReportsPage() {
  ...
  if (session.status !== "authenticated") return <ReportsFrame title="Reports Maintenance Menu" description="The reports workspace is temporarily unavailable."><p className="notice notice-error">The FIS API could not be reached. Retry when it is available.</p></ReportsFrame>;
  if (!hasReportsRole(session.roles)) return <ReportsFrame title="Reports Maintenance Menu" description="Legacy report access is enforced on the server."><AccessRestricted /></ReportsFrame>;

  return (
    <ReportsFrame title="Reports Maintenance Menu" description="Reports maintenance and frequently used report shortcuts.">
      <div className="vehicle-menu-tiles">
        <section className="vehicle-menu-tile">
          <h2 className="vehicle-menu-header">Reports Maintenance Menu</h2>
          <div className="vehicle-menu-body"><Link className="vehicle-menu-link" href="/reports/help">Reports Maintenance Information / Help</Link></div>
        </section>
        <section className="vehicle-menu-tile">
          <h2 className="vehicle-menu-header">Available Reports</h2>
          <div className="vehicle-menu-body"><Link className="vehicle-menu-link" href="/reports/trip-authority">Trip Authority Reports</Link><Link className="vehicle-menu-link" href="/reports/fis-report">FIS Reports</Link></div>
        </section>
        <section className="vehicle-menu-tile">
          <h2 className="vehicle-menu-header">Quick Links - Frequently Used Reports</h2>
          <div className="vehicle-menu-body">
            {QUICK_LINKS.map(([label, href]) => <Link className="vehicle-menu-link" href={href} key={href}>{label}</Link>)}
          </div>
        </section>
        <div className="vehicle-footer-actions"><Link className="button button-secondary" href="/home">Home</Link><Link className="button button-secondary" href="/reports/fis-report">FIS Report Menu</Link></div>
      </div>
    </ReportsFrame>
  );
}
