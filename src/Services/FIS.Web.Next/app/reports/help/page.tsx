...
export default async function ReportsHelpPage() {
  ...
  return (
    <ReportsFrame
      title="Reports Maintenance Information / Help"
      description="Reference guidance for report flows and usage."
    >
      <div className="vehicle-menu-tiles">
        {sections.map((section) => (
          <section className="vehicle-status-maintenance-panel" key={section.title}>
            <h2>{section.title}</h2>
            <p className="muted-copy">{section.content}</p>
          </section>
        ))}
      </div>
