...
function ErrorCard({ message }: Readonly<{ message: string }>) {
  return (
    <section className="vehicle-status-card" role="alert">
      <p className="eyebrow">Department deletion</p>
      <h2>{message}</h2>
...
export default async function DepartmentDeleteCheckPage({
  searchParams,
}: DepartmentDeleteCheckPageProps) {
  await connection();
  ...
  if (session.status === "expired")
    return (
      <main className="page-shell vehicle-page-shell">
        <SessionRecovery returnPath="/Validation/MNT_Department_Del_Check.aspx" />
      </main>
    );
  ...
  const departmentCode = parseCode(
    getQueryValue(query.code) ?? getQueryValue(query.cmbdep) ?? getQueryValue(query.departmentCode),
  );
  const queryError = getQueryValue(query.error);
  if (!departmentCode)
    return (
      <main className="page-shell vehicle-page-shell">
        <ErrorCard message="Select a department before attempting deletion." />
      </main>
    );
  if (queryError)
    return (
      <main className="page-shell vehicle-page-shell">
        <ErrorCard message={queryError} />
      </main>
    );
  ...
  try {
    ...
    if (!department)
      return (
        <main className="page-shell vehicle-page-shell">
          <ErrorCard message={`Department ${departmentCode} was not found.`} />
        </main>
      );
        ...
        <section className="vehicle-card" aria-labelledby="department-delete-title">
          <header className="vehicle-page-header">
            <div>
              ...
              <h1 id="department-delete-title">Delete Department</h1>
              <p>Check the same site and logsheet dependencies used by the legacy delete flow.</p>
            </div>
            <Link className="button button-secondary" href="/Validation/MNT_department.aspx">
              Department Maintenance
            </Link>
          ...
          <section className="vehicle-status-card" role={blocked ? "alert" : "note"}>
            ...
            <dl className="status-maintenance-details">
              <div>
                <dt>Sites assigned</dt>
                <dd>{dependencies.siteCount}</dd>
              </div>
              <div>
                <dt>Logsheets using those sites</dt>
                <dd>{dependencies.logsheetCount}</dd>
              </div>
            </dl>
            {blocked ? (
              <>
                <p className="muted-copy">
                  This department cannot be deleted until its sites are removed and its logsheets
                  are rectified.
                </p>
                <Link className="button button-secondary" href="/Validation/MNT_department.aspx">
                  Return to Department Maintenance
                </Link>
              </>
            ) : (
    ...
    if (error instanceof DepartmentApiError && error.reason === "unauthorized")
      return (
        <main className="page-shell vehicle-page-shell">
          <SessionRecovery
            returnPath={`/Validation/MNT_Department_Del_Check.aspx?code=${departmentCode}`}
          />
        </main>
      );
    ...
    console.error(
      "FIS department delete check failed",
      error instanceof Error ? error.message : "unknown error",
    );
    return (
      <main className="page-shell vehicle-page-shell">
        <ErrorCard message="Department dependencies could not be checked." />
      </main>
    );
  }
}
