...
function integer(formData: FormData, key: string, label: string, required = true) {
  ...
  if (!value || !Number.isSafeInteger(parsed) || parsed < 0) throw new Error(`${label} must be a whole number.`);
  return parsed;
}
...
async function authorizeTroubleshooting() {
  ...
  if (!hasTroubleshootingRole(session.roles)) return { ok: false as const, message: "You do not have permission to maintain Troubleshoot records." };
...
function apiMessage(error: unknown, fallback: string) {
  if (error instanceof TroubleshootApiError) {
    ...
    if (error.reason === "unavailable") return "The Troubleshoot service is temporarily unavailable. Please try again.";
...
export async function updateTroubleshootLogsAction(formData: FormData) {
  ...
  if (!userAccessCode || userAccessCode <= 0) pathWithMessage("/troubleshoot/log", "Select a user/site before updating logs.");
  ...
  try {
    ...
    pathWithMessage(`/troubleshoot/log?userAccessCode=${userAccessCode}`, apiMessage(error, "Troubleshoot logs could not be updated."));
  }
}
...
export async function saveApproverRanksAction(formData: FormData) {
  ...
  try {
    ...
    const ranks = names.map((_, index) => {
      ...
      if (!Number.isSafeInteger(id) || id < 0 || !rankName || !description) throw new Error("Every approver rank needs a name and description.");
      if (rankName.length > 200 || description.length > 500) throw new Error("Approver rank text is too long.");
    ...
    pathWithMessage("/troubleshoot/approver-ranks", apiMessage(error, "Approver ranks could not be saved."));
  }
}
...
export async function removeTripsWithoutRoutesAction(formData: FormData) {
  ...
  if (text(formData, "confirm") !== "yes") pathWithMessage("/troubleshoot/remove-trips-no-routes", "Confirm that you want to remove trips without routes.");

  try {
    const fromDate = date(formData, "fromDate", "From date");
    const toDate = date(formData, "toDate", "To date");
    if (fromDate && toDate && fromDate > toDate) throw new Error("The To date must be on or after the From date.");
    const removed = await removeTripsWithoutRoutes({ fromDate: fromDate ?? undefined, toDate: toDate ?? undefined });
    revalidatePath("/troubleshoot/remove-trips-no-routes");
    redirect(`/troubleshoot/remove-trips-no-routes?saved=${removed}`);
  } catch (error) {
    pathWithMessage("/troubleshoot/remove-trips-no-routes", apiMessage(error, "Trips without routes could not be removed."));
  }
}
