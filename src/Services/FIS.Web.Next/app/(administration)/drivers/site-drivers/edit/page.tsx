...
function resultMessage(result: string | undefined, detail: string | undefined) {
  ...
  if (result === "invalid")
    return {
      tone: "error",
      text: detail || "Check the site-driver fields and try again.",
    } as const;
  if (result === "forbidden")
    return { tone: "error", text: "You do not have permission to maintain site drivers." } as const;
  if (result === "not-found")
  ...
  if (result === "rejected")
    return { tone: "error", text: "The site-driver change was rejected by the database." } as const;
  return result
...
async function renderSiteDriverEdit({ searchParams }: Readonly<{ searchParams: SearchParams }>) {
  ...
  if (session.status === "expired")
    return (
      <main className="page-shell vehicle-page-shell">
        <SessionRecovery returnPath="/drivers/site-drivers/edit" />
      </main>
    );
    ...
    return (
      <main className="page-shell vehicle-page-shell">
        <section className="vehicle-status-card" role="alert">
          ...
          <h2>Site-driver details could not be loaded.</h2>
        </section>
      </main>
    );
    ...
    return (
      <main className="page-shell vehicle-page-shell">
        <section className="vehicle-status-card" role="alert">
          ...
          <h2>Select a department and site before editing site drivers.</h2>
          <Link className="button button-secondary" href="/drivers">
  ...
  try {
    ...
    if (error instanceof DriverManagementApiError && error.reason === "unauthorized")
      return (
        <main className="page-shell vehicle-page-shell">
          <SessionRecovery
            returnPath={contextPath("/drivers/site-drivers/edit", departmentCode, siteCode)}
          />
        </main>
      );
    console.error(
      "FIS site-driver edit request failed",
      ...
      <main className="page-shell vehicle-page-shell">
        <section className="vehicle-status-card" role="alert">
          ...
          <Link
            ...
            href={contextPath("/drivers/site-drivers/edit", departmentCode, siteCode)}
          >
            Try again
          </Link>
        </section>
