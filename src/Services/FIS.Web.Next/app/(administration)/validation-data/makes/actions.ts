...
function apiErrorMessage(error: unknown, operation: string) {
  if (error instanceof MakeApiError) {
    ...
    if (error.reason === "unavailable")
      return `The vehicle make ${operation} service is temporarily unavailable. Please try again.`;
...
export async function createMakeAction(
  ...
  try {
    ...
    return {
      status: "error",
      message:
        error instanceof MakeValidationError ? error.message : apiErrorMessage(error, "created"),
    };
  }
...
export async function updateMakeAction(
  ...
  try {
    ...
    return {
      status: "error",
      message:
        error instanceof MakeValidationError ? error.message : apiErrorMessage(error, "updated"),
    };
  }
...
export async function deleteMakeAction(formData: FormData) {
  ...
  try {
    ...
    redirect(
      `/validation-data/makes?error=${encodeURIComponent(error instanceof Error ? error.message : "Make code is invalid.")}`,
    );
  }
  if (!access.ok)
    redirect(
      `/Validation/MNT_Make_Del_Check.aspx?code=${encodeURIComponent(String(makeCode!))}&error=${encodeURIComponent(access.message)}`,
    );
  ...
  try {
    ...
    redirect(
      `/Validation/MNT_Make_Del_Check.aspx?code=${makeCode!}&error=${encodeURIComponent(apiErrorMessage(error, "deleted"))}`,
    );
  }
  if (!dependencies!.canDelete) {
    redirect(
      `/Validation/MNT_Make_Del_Check.aspx?code=${makeCode}&error=${encodeURIComponent("This make cannot be deleted while models are linked to it.")}`,
    );
  }
