...
import { updateModelAction } from "@/app/(administration)/validation-data/models/actions";
...
async function ModelEditPageContent({ searchParams }: ModelEditPageProps) {
  ...
  if (session.status === "expired")
    return (
      <main className="page-shell vehicle-page-shell">
        <SessionRecovery returnPath="/Validation/MNT_Model_Edit.aspx" />
      </main>
    );
  ...
  if (!hasVehicleManagementPermission(session.accessLevel))
    return (
      <main className="page-shell vehicle-page-shell">
        <ErrorCard message="You do not have permission to edit vehicle models." />
      </main>
    );

  const query = await searchParams;
  const modelCode = parseCode(
    getQueryValue(query.cmbmodel) ?? getQueryValue(query.modelCode) ?? getQueryValue(query.code),
  );
  if (!modelCode)
    return (
      <main className="page-shell vehicle-page-shell">
        <ErrorCard message="Select a model before opening edit." />
      </main>
    );

  try {
    const [model, referenceData] = await Promise.all([
      getModel(modelCode),
      getModelReferenceData(),
    ]);
    if (!model)
    ...
    if (error instanceof ModelApiError && error.reason === "unauthorized")
      return (
        <main className="page-shell vehicle-page-shell">
          <SessionRecovery returnPath={`/Validation/MNT_Model_Edit.aspx?cmbmodel=${modelCode}`} />
        </main>
      );
    ...
    console.error(
      "FIS model edit request failed",
      error instanceof Error ? error.message : "unknown error",
    );
    return (
      <main className="page-shell vehicle-page-shell">
        <ErrorCard message="Model details could not be loaded." />
      </main>
    );
  }
}
