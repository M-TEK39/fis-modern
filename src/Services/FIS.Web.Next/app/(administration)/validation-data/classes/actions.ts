...
function getRequiredInteger(
  ...
  if (!/^\d+$/.test(value) || value.length > maxLength) {
    throw new ClassValidationError(
      `${label} must be a whole number with no more than ${maxLength} digits.`,
    );
  }
...
function getClassInput(formData: FormData): ClassWriteInput {
  ...
  return {
    ...
    depreciationPercent: getRequiredDecimal(
      formData,
      "depreciationPercent",
    ...
    appreciatePercent: getRequiredInteger(
      formData,
      "appreciatePercent",
      "Appreciate percent",
      2,
    ...
    replacementCost: getRequiredInteger(
      formData,
      "replacementCost",
      "Replacement cost",
      8,
...
async function authorizeClassMaintenance() {
  ...
  if (!hasLegacyRole(session.roles, "Validation")) {
    return {
      ok: false as const,
      message: "You do not have permission to maintain vehicle classes.",
    };
  }
...
function apiErrorMessage(error: unknown, operation: string) {
  if (error instanceof ClassApiError) {
    ...
    if (error.reason === "unavailable")
      return `The vehicle class ${operation} service is temporarily unavailable. Please try again.`;
...
export async function createClassAction(
  ...
  try {
    ...
    if (!created)
      throw new ClassApiError("invalid-response", "The FIS API did not return the created class.");
  } catch (error) {
    return {
      status: "error",
      message:
        error instanceof ClassValidationError ? error.message : apiErrorMessage(error, "created"),
    };
  }
...
export async function updateClassAction(
  ...
  try {
    ...
    if (!updated)
      throw new ClassApiError("invalid-response", "The FIS API did not return the updated class.");
  } catch (error) {
    return {
      status: "error",
      message:
        error instanceof ClassValidationError ? error.message : apiErrorMessage(error, "updated"),
    };
  }

  revalidateClassRoutes();
  redirect(
    `/validation-data/classes?saved=updated&classCode=${encodeURIComponent(String(classCode))}`,
  );
}
...
export async function deleteClassAction(formData: FormData) {
  ...
  try {
    ...
    redirect(
      `/validation-data/classes?error=${encodeURIComponent(error instanceof Error ? error.message : "Class code is invalid.")}`,
    );
  }
  if (!access.ok) {
    redirect(
      `/Validation/MNT_Class_Del_Check.aspx?code=${encodeURIComponent(String(classCode))}&error=${encodeURIComponent(access.message)}`,
    );
  }
  ...
  try {
    ...
    redirect(
      `/Validation/MNT_Class_Del_Check.aspx?code=${classCode}&error=${encodeURIComponent(apiErrorMessage(error, "deleted"))}`,
    );
  }
  if (!dependencies.canDelete || dependencies.modelCount > 0 || dependencies.vehicleCount > 0) {
    redirect(
      `/Validation/MNT_Class_Del_Check.aspx?code=${classCode}&error=${encodeURIComponent("This class cannot be deleted while models or vehicles are linked to it.")}`,
    );
  }
