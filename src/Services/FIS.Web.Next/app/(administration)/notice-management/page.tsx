...
export default async function NoticeManagementPage({
  ...
  const filtered = schedules
    .filter((schedule) => matchesSearch(schedule, search))
    .filter((schedule) => matchesStatus(schedule, status, today));
      ...
      <section className="vehicle-card" aria-labelledby="notice-management-title">
        ...
        {schedules.length === 0 ? (
          <div className="vehicle-empty-state">
            <p className="eyebrow">No notices to display</p>
            <p>Please select Create new Notice to create one.</p>
          </div>
        ) : filtered.length === 0 ? (
          <div className="vehicle-empty-state">
            <p className="eyebrow">No matching schedules</p>
            ...
            <nav className="pagination-controls" aria-label="Notice schedule pagination">
              {page <= 1 ? (
                ...
                <Link
                  ...
                  href={listHref(routePath, search, status, page - 1)}
                >
                  Previous
                </Link>
              )}
