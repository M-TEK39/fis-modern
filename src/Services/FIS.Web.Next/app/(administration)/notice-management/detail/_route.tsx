...
import { saveNoticeAction } from "@/app/(administration)/notice-management/actions";
import RouteLoading from "@/components/app-shell/route-loading";
import NoticeDetailForm from "@/app/(administration)/notice-management/notice-detail-form";
  ...
  return (
    <section className="vehicle-status-card" role="alert">
      ...
      <div className="button-row">
        ...
        <Link className="button button-secondary" href="/notice-management">
          Notice Management
        </Link>
      </div>
    </section>
...
function errorMessage(value: string | undefined) {
  switch (value) {
    ...
    case "not-found":
      return "The selected notice no longer exists.";
    default:
...
async function NoticeDetailContent({
    ...
    return (
      <main className="page-shell vehicle-page-shell">
        <StatusCard
          ...
          message="Notice details could not be loaded."
          routePath={routePath}
        />
      </main>
    );
  ...
  const requestedNoticeId = positiveInteger(
    getQueryValue(query.noticeid) ?? getQueryValue(query.noticeId),
  );
  const requestedScheduleId = positiveInteger(
    getQueryValue(query.scheduleid) ?? getQueryValue(query.scheduleId),
  );
  ...
  try {
        ...
        return (
          <main className="page-shell vehicle-page-shell">
            <StatusCard
              ...
              message="The selected notice schedule could not be found."
              routePath={routePath}
            />
          </main>
        );
      ...
      if (!notice)
        return (
          <main className="page-shell vehicle-page-shell">
            <StatusCard
              title="Notice not found"
              message="The selected notice could not be found."
              routePath={routePath}
            />
          </main>
        );
    ...
    if (error instanceof NoticeApiError && error.reason === "not-found")
      return (
        <main className="page-shell vehicle-page-shell">
          <StatusCard
            title="Notice not found"
    ...
    console.error(
      "FIS notice detail request failed",
  ...
  const createdDate = dateInputValue(
    schedule?.createdDate,
    getQueryValue(query.createdate)?.replaceAll("/", "-") || today,
  );
  const backPath =
    routePath === "/Admin/NoticeDetailManagement.aspx"
      ...
      <section className="vehicle-card" aria-labelledby="notice-detail-title">
        <header className="vehicle-page-header">
          <div>
            ...
            <p>
              {notice
                ? "Update the notice schedule and client communication."
                : "Create a notice schedule and client communication."}
            </p>
          </div>
...
type NoticeDetailRouteProps = Pick<NoticeDetailPageProps, "searchParams">;

export default function NoticeDetailRoute({ searchParams }: Readonly<NoticeDetailRouteProps>) {
  return (
