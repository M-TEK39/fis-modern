...
function DetailActions({ contract, canApprove }: Readonly<{ contract: ContractRecord; canApprove: boolean }>) {
  ...
  return (
    <section className="vehicle-status-maintenance-panel" aria-labelledby="contract-actions-title">
      ...
      <div className="button-row">
        ...
        {status === 3 && canApprove ? <Link className="button button-secondary" href={`${returnPath}#extend-contract`}>Extend</Link> : null}
        {status === 3 && canApprove ? <Link className="button button-secondary" href={`${returnPath}#close-contract`}>Close and return home</Link> : null}
        {status === 3 && canApprove ? <form action={runContractAction}><input name="contractId" type="hidden" value={contract.contractCode} /><input name="returnPath" type="hidden" value={returnPath} /><input name="action" type="hidden" value="cancel" /><button className="button button-danger" type="submit">Cancel</button></form> : null}
      </div>
