...
export default async function ContractPrintoutPage({ searchParams }: ContractPrintoutPageProps) {
  ...
  if (session.status === "expired")
    return (
      <main className="page-shell vehicle-page-shell">
        <SessionRecovery returnPath="/contracts/printout" />
      </main>
    );
    ...
    return (
      <main className="page-shell vehicle-page-shell">
        <section className="vehicle-status-card" role="alert">
          ...
          <h2>Contract printout could not be loaded.</h2>
        </section>
      </main>
    );
  ...
  if (!contractId)
    return (
      <main className="page-shell vehicle-page-shell">
        <section className="vehicle-status-card" role="alert">
          <p className="eyebrow">Contract not selected</p>
          <h2>Choose a contract from the print menu.</h2>
  ...
  try {
    ...
    return (
      <main className="page-shell vehicle-page-shell">
        <article
          ...
          aria-labelledby="contract-printout-title"
        >
          <header className="vehicle-page-header">
            <div>
              <p className="eyebrow">Fleet Management</p>
              <h1 id="contract-printout-title">Vehicle Contract #{contract.contractCode}</h1>
              <p>
          ...
          <section className="vehicle-status-maintenance-panel">
            ...
            <dl className="contract-facts">
              {[
                ["Status", valueOrDash(contract.contractStatusCode)],
                ["Vehicle code", valueOrDash(contract.vmfCode)],
                ["Site", `${valueOrDash(contract.siteDescription)} (${contract.siteCode})`],
                ["Driver", valueOrDash(contract.driverName)],
                ...
                ["BAS fund", valueOrDash(contract.basFundCode)],
                ["BAS objective", valueOrDash(contract.basObjectiveCode)],
                ["BAS project", valueOrDash(contract.basProjectNumber)],
                ["BAS responsibility", valueOrDash(contract.basResponsibilityCode)],
              ].map(([label, value]) => (
                <div key={label}>
                  <dt>{label}</dt>
                  <dd>{value}</dd>
    ...
    if (error instanceof ContractApiError && error.reason === "unauthorized")
      return (
        <main className="page-shell vehicle-page-shell">
          <SessionRecovery returnPath={`/contracts/printout?contractId=${contractId}`} />
        </main>
      );
    return (
      <main className="page-shell vehicle-page-shell">
        <section className="vehicle-status-card" role="alert">
          ...
          <h2>Contract #{contractId} could not be loaded.</h2>
          <Link className="button button-secondary" href="/contracts/print-menu">
            Print menu
          </Link>
        </section>
