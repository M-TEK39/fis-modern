...
import { createMonitor, getMonitorDrivers, MonitorApiError, updateMonitor } from "@/lib/api-monitor";
...
function integer(formData: FormData, key: string, label: string, required = true): number | null {
  ...
  if (!value || !Number.isInteger(parsed) || parsed <= 0) throw new MonitorValidationError(`${label} must be a positive whole number.`);
  return parsed;
}

function date(formData: FormData) {
  const value = text(formData, "captureDate");
  if (!/^\d{4}-\d{2}-\d{2}$/.test(value)) throw new MonitorValidationError("Capture date is invalid.");
...
async function authorizeMonitor() {
  ...
  const allowed = session.roles.some((role) => role.toLocaleLowerCase().replace(/[^a-z0-9]/g, "") === "callcentre");
  return allowed ? { ok: true as const } : { ok: false as const, message: "You do not have permission to maintain Monitor inquiries." };
}
...
function apiErrorMessage(error: unknown) {
  if (error instanceof MonitorApiError) {
    ...
    if (error.reason === "unavailable") return "The Monitor service is temporarily unavailable. Please try again.";
...
export async function saveMonitorAction(formData: FormData) {
  ...
  const path = returnPath(formData, monitorCode ? `/monitor/edit?referenceNumber=${monitorCode}` : "/monitor/capture");
  ...
  try {
    ...
    if (!inquiryType || inquiryType.length > 100) throw new MonitorValidationError("Inquiry type is required and must be 100 characters or fewer.");
    ...
    if (driverName && driverName.length > 100) throw new MonitorValidationError("Driver name must be 100 characters or fewer.");
    if (driverPersalNo && driverPersalNo.length > 50) throw new MonitorValidationError("Driver Persal number must be 50 characters or fewer.");
    if (driverSite !== null && driverSite <= 0) throw new MonitorValidationError("Driver site must be a positive site code.");
    const input = { vmf_code: vmfCode, Capture_dat: date(formData), Inquiry_type: inquiryType, Inquiry_Desc: text(formData, "inquiryDescription") || null, Driver_name: driverName, Driver_persalno: driverPersalNo, Driver_Site: driverSite };
    if (monitorCode) {
    ...
    redirectWithMessage(path, "error", error instanceof MonitorValidationError ? error.message : apiErrorMessage(error));
  }
}
