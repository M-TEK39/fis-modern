...
function getRequiredText(formData: FormData, key: string, label: string, maxLength: number) {
  ...
  if (value.length > maxLength) throw new SiteValidationError(`${label} must be ${maxLength} characters or fewer.`);
  return value;
}
...
function getCode(formData: FormData, key: string, label: string, max = 32767) {
  ...
  if (!Number.isInteger(parsed) || parsed <= 0 || parsed > max) throw new SiteValidationError(`${label} is invalid.`);
  return parsed;
}
...
function getDecimal(formData: FormData, key: string, label: string) {
  ...
  if (!value) return 0;
  const parsed = Number(value);
  if (!Number.isFinite(parsed) || parsed < 0 || parsed > 999.999) throw new SiteValidationError(`${label} is invalid.`);
  return parsed;
}
...
function getNullableBoolean(formData: FormData, key: string) {
  ...
  if (!value) return null;
  if (value !== "true" && value !== "false") throw new SiteValidationError("A boolean setting is invalid.");
  return value === "true";
}
...
function buildInput(formData: FormData, mode: "create" | "update"): SiteWriteInput {
  ...
  if (!/^\d{7}$/.test(departmentNumber)) throw new SiteValidationError("Department number must contain exactly 7 numbers.");

  const postalCode = getOptionalText(formData, "postalCode", "Postal code", 10);
  if (postalCode && !/^\d+$/.test(postalCode)) throw new SiteValidationError("Postal code must contain numbers only.");
...
async function authorizeSiteMaintenance() {
  ...
  if (!hasVehicleManagementPermission(session.accessLevel)) return { ok: false as const, message: "You do not have permission to maintain sites." };
...
function apiErrorMessage(error: unknown, operation: string) {
  if (error instanceof SiteApiError) {
    ...
    if (error.reason === "unavailable") return `The site ${operation} service is temporarily unavailable. Please try again.`;
...
export async function createSiteAction(
  ...
  try {
    ...
    return { status: "error", message: error instanceof SiteValidationError ? error.message : apiErrorMessage(error, "created") };
  }
...
export async function updateSiteAction(
  ...
  try {
    ...
    return { status: "error", message: error instanceof SiteValidationError ? error.message : apiErrorMessage(error, "updated") };
  }
  revalidateSiteRoutes();
