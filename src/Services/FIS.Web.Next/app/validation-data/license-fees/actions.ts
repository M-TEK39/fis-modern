...
function getFee(formData: FormData): number | null {
  ...
  if (
    ...
    throw new LicenseFeeValidationError(
      "Yearly licence fee must be a non-negative number with no more than 2 decimal places.",
    );
  }
  return parsed;
}
...
function getInput(formData: FormData): LicenseFeeWriteInput {
  ...
  if (!description || description.length > 50) {
    throw new LicenseFeeValidationError(
      "Licence fee description is required and must be 50 characters or fewer.",
    );
  }
...
function apiErrorMessage(error: unknown, operation: string) {
  if (error instanceof LicenseFeeApiError) {
    ...
    if (error.reason === "unavailable")
      return `The licence fee ${operation} service is temporarily unavailable. Please try again.`;
...
export async function createLicenseFeeAction(
  _previousState: LicenseFeeActionState = initialState,
  formData: FormData,
  ...
  try {
    ...
    if (!created)
      throw new LicenseFeeApiError(
        "invalid-response",
        "The FIS API did not return the created licence fee.",
      );
  } catch (error) {
    return {
      status: "error",
      message:
        error instanceof LicenseFeeValidationError
          ? error.message
...
export async function updateLicenseFeeAction(
  _previousState: LicenseFeeActionState = initialState,
  formData: FormData,
): Promise<LicenseFeeActionState> {
  const access = await authorizeLicenseFeeMaintenance();
  ...
  try {
    ...
    if (!updated)
      throw new LicenseFeeApiError(
        "invalid-response",
        "The FIS API did not return the updated licence fee.",
      );
  } catch (error) {
    return {
      status: "error",
  ...
  revalidateLicenseFeeRoutes();
  redirect(
    `/validation-data/license-fees?saved=updated&licenceFeeCode=${encodeURIComponent(String(licenceFeeCode))}`,
  );
}
...
export async function deleteLicenseFeeAction(formData: FormData) {
  ...
  try {
    ...
    redirect(
      `/validation-data/license-fees?error=${encodeURIComponent(error instanceof Error ? error.message : "Licence fee code is invalid.")}`,
    );
  }
  if (!access.ok)
    redirect(
      `/Validation/MNT_Licence_Fee_Del_Check.aspx?code=${licenceFeeCode}&error=${encodeURIComponent(access.message)}`,
    );
  ...
  try {
    ...
    redirect(
      `/Validation/MNT_Licence_Fee_Del_Check.aspx?code=${licenceFeeCode}&error=${encodeURIComponent(apiErrorMessage(error, "checked"))}`,
    );
  }
  if (!dependencies.canDelete || dependencies.modelCount > 0) {
    redirect(
      `/Validation/MNT_Licence_Fee_Del_Check.aspx?code=${licenceFeeCode}&error=${encodeURIComponent("This licence fee cannot be deleted while models are linked to it.")}`,
    );
  }
  try {
    ...
    redirect(
      `/Validation/MNT_Licence_Fee_Del_Check.aspx?code=${licenceFeeCode}&error=${encodeURIComponent(apiErrorMessage(error, "deleted"))}`,
    );
  }
  revalidateLicenseFeeRoutes();
