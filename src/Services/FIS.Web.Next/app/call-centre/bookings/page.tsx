  ...
  return (
    <section className="vehicle-status-card" role="alert">
      ...
      <div className="button-row">
        <Link className="button button-primary" href="/call-centre/bookings">Try again</Link>
    ...
    <section className="vehicle-form-section" aria-labelledby="booking-lookup-title">
      ...
      <form method="get" className="form-stack">
        <div className="field">
          ...
          <input id="booking-lookup-id" name="bookingId" inputMode="numeric" defaultValue={bookingId} />
          {sourceGmt ? <input type="hidden" name="gmt" value={sourceGmt} /> : null}
        </div>
        <div className="button-row">
          <button className="button button-secondary" type="submit">Load Booking</button>
          <Link className="button button-primary" href={newBookingHref}>New Booking</Link>
          <Link className="button button-secondary" href="/call-centre">Menu</Link>
        </div>
      </form>
    </section>
  );
...
function BookingForm({ record, sourceGmt }: Readonly<{ record: BookingRecord; sourceGmt: string }>) {
    ...
    <form action={saveBookingAction} className="vehicle-form-stack">
      ...
      <section className="vehicle-form-section" aria-labelledby="booking-form-title">
        ...
        <div className="field-grid">
          <Field id="booking-name" label="Name" name="name" defaultValue={valueOrEmpty(record.name)} />
          <Field id="booking-site" label="Site Code" name="site_code" defaultValue={valueOrEmpty(record.siteCode)} />
          <Field id="booking-telephone" label="Telephone" name="telephone" defaultValue={valueOrEmpty(record.telephone)} />
          <Field id="booking-vmf" label="VMF Code" name="vmf_code" type="number" defaultValue={valueOrEmpty(record.vmfCode)} />
          <Field id="booking-class" label="Class Code" name="class_code" type="number" defaultValue={valueOrEmpty(record.classCode)} required />
          <Field id="booking-user" label="User ID" name="user_id" type="number" defaultValue={valueOrEmpty(record.userId)} required />
          <Field id="booking-location" label="Location Code" name="location_code" type="number" defaultValue={valueOrEmpty(record.locationCode)} required />
          <Field id="booking-start" label="Start Date" name="start_date" type="datetime-local" defaultValue={dateTimeInputValue(record.startDate)} required />
          <Field id="booking-end" label="End Date" name="end_date" type="datetime-local" defaultValue={dateTimeInputValue(record.endDate)} />
          <Field id="booking-status" label="Booking Status" name="booking_status" defaultValue={valueOrEmpty(record.bookingStatus)} />
          <Field id="booking-collected" label="Collected (0/1)" name="collected" type="number" defaultValue={valueOrEmpty(record.collected)} />
          <Field id="booking-date" label="Booking Date" name="booking_date" type="datetime-local" defaultValue={dateTimeInputValue(record.bookingDate)} required />
          <div className="field field-full">
            <label htmlFor="booking-notes">Notes</label>
            <textarea id="booking-notes" name="notes" rows={3} defaultValue={valueOrEmpty(record.notes)} />
          </div>
        </div>
        <div className="button-row">
          <button className="button button-primary" type="submit">Submit</button>
          <Link className="button button-secondary" href={sourceGmt ? `/call-centre/bookings?gmt=${encodeURIComponent(sourceGmt)}` : "/call-centre/bookings"}>Clear</Link>
        </div>
      </section>
      {isEdit ? (
        <section className="vehicle-form-section" aria-labelledby="booking-audit-title">
          ...
          <div className="field-grid">
            <p className="form-hint">Created: {valueOrEmpty(record.dateCreated) || "Not present on this legacy schema"}</p>
            <p className="form-hint">Updated: {valueOrEmpty(record.dateUpdated) || "Not present on this legacy schema"}</p>
            <p className="form-hint">Created by: {valueOrEmpty(record.createdByUserCode) || "Not present on this legacy schema"}</p>
            <p className="form-hint">Modified by: {valueOrEmpty(record.modifiedByUserCode) || "Not present on this legacy schema"}</p>
...
function errorMessage(error: unknown) {
  if (error instanceof BookingApiError || error instanceof CallCentreApiError) {
    ...
    if (error.reason === "not-found") return "The requested booking or GMT reference was not found.";
...
export default async function BookingPage({ searchParams }: BookingPageProps) {
  ...
  if (session.status === "expired") {
    return <main className="page-shell vehicle-page-shell"><SessionRecovery returnPath="/call-centre/bookings" /></main>;
  }
  ...
  return (
    <main className="page-shell vehicle-page-shell">
      <section className="vehicle-card" aria-labelledby="booking-page-title">
        ...
        {saved ? <div className="notice notice-success" role="status">Booking #{formRecord.bookingId || bookingId} saved successfully.</div> : null}
        {linkWarning ? <div className="notice notice-error" role="alert">The booking was saved, but the source GMT link could not be updated. The booking can still be opened by ID.</div> : null}
        {contextNotice ? <div className="notice notice-info" role="status">{contextNotice}</div> : null}
