...
function AccidentChildFields({ record, sites, tableAvailable }: Readonly<{ record: Record<string, unknown> | null; sites: CallCentreSiteOption[]; tableAvailable: boolean }>) {
  if (!record) return <section className="vehicle-form-section" aria-labelledby="accident-child-title"><div className="vehicle-form-section-header"><div><p className="eyebrow">Legacy Accident child table</p><h2 id="accident-child-title">Accident details</h2></div></div><p className="muted-copy">{tableAvailable ? "No linked Accident row was found. The Call_centre record remains editable." : "This database does not expose the legacy Accident columns. The Call_centre record remains editable."}</p></section>;
  return <section className="vehicle-form-section" aria-labelledby="accident-child-title"><div className="vehicle-form-section-header"><div><p className="eyebrow">Legacy Accident child table</p><h2 id="accident-child-title">Accident details</h2></div></div><div className="field-grid"><Field id="accident-occurrence-date" label="Accident Date" name="accident_occurence_date" type="date" defaultValue={childDate(record, "occurence_date")} /><Field id="accident-occurrence-time" label="Accident Time" name="accident_occurence_time" type="time" defaultValue={childTime(record, "occurence_time")} /><Field id="accident-description" label="Accident Description" name="accident_description" maxLength={60} defaultValue={childValue(record, "description")} /><Field id="accident-damage" label="GG Damage Description" name="accident_damage_description" maxLength={60} defaultValue={childValue(record, "damage_description")} /><Field id="accident-third-party-reg" label="Third Party Registration" name="accident_third_party_regno" maxLength={8} defaultValue={childValue(record, "third_party_regno")} /><Field id="accident-third-party-owner" label="Third Party Owner" name="accident_third_party_owner" maxLength={30} defaultValue={childValue(record, "third_party_owner")} /><Field id="accident-third-party-tel" label="Third Party Telephone" name="accident_third_party_tel" maxLength={30} defaultValue={childValue(record, "third_party_tel")} /><Field id="accident-notes" label="Accident Notes" name="accident_notes" maxLength={50} defaultValue={childValue(record, "notes")} /><Field id="accident-occurrence-place" label="Accident Place" name="accident_occurence_place" maxLength={50} defaultValue={childValue(record, "occurence_place")} /><SiteSelect id="accident-driver-site" label="Accident Driver Site" name="accident_driver_site_code" value={childNumber(record, "driver_site_code")} sites={sites} /><ChoiceSelect id="accident-death" label="Death" name="accident_death" value={childValue(record, "death")} choices={["?", "Y", "N"]} /><ChoiceSelect id="accident-injured" label="Injured" name="accident_injured" value={childValue(record, "injured")} choices={["?", "Y", "N"]} /><ChoiceSelect id="accident-tow-need" label="Need Tow Truck" name="accident_tow_need" value={childValue(record, "Tow_need", "tow_need")} choices={["Y", "N"]} /></div></section>;
}

function LossChildFields({ record, sites, lossTypes, tableAvailable }: Readonly<{ record: Record<string, unknown> | null; sites: CallCentreSiteOption[]; lossTypes: LossTypeOption[]; tableAvailable: boolean }>) {
  if (!record) return <section className="vehicle-form-section" aria-labelledby="loss-child-title"><div className="vehicle-form-section-header"><div><p className="eyebrow">Legacy Losses child table</p><h2 id="loss-child-title">Loss / theft details</h2></div></div><p className="muted-copy">{tableAvailable ? "No linked Losses row was found. The Call_centre record remains editable." : "This database does not expose the legacy Losses columns. The Call_centre record remains editable."}</p></section>;
  return <section className="vehicle-form-section" aria-labelledby="loss-child-title"><div className="vehicle-form-section-header"><div><p className="eyebrow">Legacy Losses child table</p><h2 id="loss-child-title">Loss / theft details</h2></div></div><div className="field-grid"><Field id="loss-date" label="Loss Date" name="loss_date" type="date" defaultValue={childDate(record, "loss_date")} /><div className="field"><label htmlFor="loss-type">Loss Type</label><select id="loss-type" name="loss_type_code" defaultValue={valueOrEmpty(childNumber(record, "loss_type_code"))}><option value="">Select loss type</option>{lossTypes.map((item) => <option key={item.code} value={item.code}>{item.description}</option>)}</select></div><SiteSelect id="loss-site" label="Loss Site" name="loss_site_code" value={childNumber(record, "site_code")} sites={sites} /><Field id="loss-department-contact" label="Department Contact" name="loss_department_contact" maxLength={100} defaultValue={childValue(record, "dept_contact")} /><Field id="loss-place" label="Place of Loss" name="loss_place_of_loss" maxLength={100} defaultValue={childValue(record, "place_of_loss")} /><Field id="loss-driver" label="Loss Driver" name="loss_driver_name" maxLength={100} defaultValue={childValue(record, "driver_name")} /><Field id="loss-remarks" label="Loss Remarks" name="loss_remarks" maxLength={100} defaultValue={childValue(record, "Remarks", "remarks")} /><ChoiceSelect id="loss-tow-need" label="Need Tow Truck" name="loss_tow_need" value={childValue(record, "Tow_need", "tow_need")} choices={["Y", "N"]} /></div></section>;
}

function TowingChildFields({ record, sites, towTrucks, tableAvailable }: Readonly<{ record: Record<string, unknown> | null; sites: CallCentreSiteOption[]; towTrucks: TowTruckOption[]; tableAvailable: boolean }>) {
  if (!record) return <section className="vehicle-form-section" aria-labelledby="towing-child-title"><div className="vehicle-form-section-header"><div><p className="eyebrow">Legacy Towing child table</p><h2 id="towing-child-title">Road assistance details</h2></div></div><p className="muted-copy">{tableAvailable ? "No linked Towing row was found. The Call_centre record remains editable." : "This database does not expose the legacy Towing columns. The Call_centre record remains editable."}</p></section>;
  return <section className="vehicle-form-section" aria-labelledby="towing-child-title"><div className="vehicle-form-section-header"><div><p className="eyebrow">Legacy Towing child table</p><h2 id="towing-child-title">Road assistance details</h2></div></div><div className="field-grid"><Field id="towing-location" label="Towing Location" name="towing_location" maxLength={50} defaultValue={childValue(record, "Tow_location_start")} /><Field id="towing-problem" label="Vehicle Problem" name="towing_vehicle_problem" maxLength={60} defaultValue={childValue(record, "Vehicle_problem")} /><SiteSelect id="towing-site" label="Towing Site" name="towing_site_code" value={childNumber(record, "Site_code")} sites={sites} /><div className="field"><label htmlFor="towing-truck">Assist Company</label><select id="towing-truck" name="towing_tow_truck_code" defaultValue={valueOrEmpty(childNumber(record, "Tow_Truck_code"))}><option value="">Select assist company</option>{towTrucks.map((item) => <option key={item.code} value={item.code}>{item.name ?? item.code}{item.telephone ? ` (${item.telephone})` : ""}</option>)}</select></div><Field id="towing-contact-name" label="Contact Person" name="towing_contact_person_name" maxLength={30} defaultValue={childValue(record, "Contact_person_name")} /><Field id="towing-contact-tel" label="Contact Telephone" name="towing_contact_person_tel" maxLength={30} defaultValue={childValue(record, "Contact_person_tel")} /><Field id="towing-remarks" label="Towing Remarks" name="towing_remarks" maxLength={50} defaultValue={childValue(record, "Remaks")} /></div></section>;
}

function IncidentEditor({ record, sites, notifyLists, details, lossTypes, towTrucks }: Readonly<{ record: CallCentreIncidentRecord; sites: CallCentreSiteOption[]; notifyLists: NotifyListRecord[]; details: CallCentreEditDetails; lossTypes: LossTypeOption[]; towTrucks: TowTruckOption[] }>) {
  return <form action={updateCallCentreIncidentAction} className="vehicle-form-stack">
    ...
    <input name="child-record-available" type="hidden" value={record.incidentType?.toLowerCase() === "accident" ? (details.accident ? "1" : "0") : record.incidentType?.toLowerCase() === "loss_theft" ? (details.loss ? "1" : "0") : record.incidentType?.toLowerCase() === "road_assistance" ? (details.towing ? "1" : "0") : "0"} />
    ...
    {record.incidentType?.toLowerCase() === "accident" ? <AccidentChildFields record={details.accident} sites={sites} tableAvailable={details.accidentTableAvailable} /> : null}
    {record.incidentType?.toLowerCase() === "loss_theft" ? <LossChildFields record={details.loss} sites={sites} lossTypes={lossTypes} tableAvailable={details.lossTableAvailable} /> : null}
    {record.incidentType?.toLowerCase() === "road_assistance" ? <TowingChildFields record={details.towing} sites={sites} towTrucks={towTrucks} tableAvailable={details.towingTableAvailable} /> : null}
