...
export default async function LegacyTaxiRoute({ params, searchParams }: LegacyTaxiRouteProps) {
  ...
  if (path.includes("scan") || path.includes("rekcert") || path.includes("scandoc") || path.includes("listall") || path.includes("list_one") || path.includes("delete_file") || path.includes("no_rekcert") || path.includes("chk_rek") || path.includes("rekfileexists")) return <TaxiScanRequisitionPage searchParams={searchParams} />;
  if (path.includes("mnt_menu")) return <TaxiMaintenanceInfoPage />;
  if (path.includes("rpt") || path.includes("report") || path.includes("no_log") || path.includes("invoiced")) return <TaxiReportsPage searchParams={searchParams} kind={reportKind(path)} />;
  if (path.includes("log") || path.includes("white_log")) return <TaxiLogsPage searchParams={searchParams} mode={path.includes("white_log") ? "white-log" : path.includes("edit") ? "edit" : path.includes("reprint") ? "reprint" : "enter"} />;
  return <TaxiRequestsPage searchParams={searchParams} mode={path.includes("cancel") ? "cancel" : path.includes("reprint") ? "reprint" : path.includes("edit") ? "edit" : "add"} />;
}
