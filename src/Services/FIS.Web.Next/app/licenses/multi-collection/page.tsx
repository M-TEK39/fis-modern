...
export default async function LicenseMultiCollectionPage({
  ...
  return (
    <LicenseShell
      title="Collection for TWO or MORE Licences"
      description="Collect several vehicle licences for one receiver. Complete the receiver details, then process each vehicle in turn."
    >
      ...
      <section className="vehicle-status-maintenance-panel" aria-labelledby="multi-receiver-title">
        <div className="vehicle-form-section-header">
          <div>
            <p className="eyebrow">Collection details</p>
            <h2 id="multi-receiver-title">Receiver Details</h2>
          </div>
        </div>
        <form method="get">
          <div className="form-grid">
            <div className="form-field">
              <label className="form-label" htmlFor="multi-receiver">
                Receiver Name
              </label>
              <input
            ...
            <div className="form-field">
              <label className="form-label" htmlFor="multi-receiver-id">
                ID Number
              </label>
              <input
                className="form-input"
                id="multi-receiver-id"
                name="receiverId"
            ...
            <div className="form-field">
              <label className="form-label" htmlFor="multi-receiver-tel">
                Tel Number
              </label>
              <input
                className="form-input"
                id="multi-receiver-tel"
                name="receiverTel"
            ...
            <div className="form-field">
              <label className="form-label" htmlFor="multi-date-collected">
                Date Collected
              </label>
              <input
                className="form-input"
                id="multi-date-collected"
                name="dateCollected"
                type="date"
                defaultValue={dateCollected}
                required
              />
            </div>
          </div>
          <input name="mode" type="hidden" value={mode} />
          {number ? <input name="number" type="hidden" value={number} /> : null}
          {lookup ? <input name="lookup" type="hidden" value="1" /> : null}
          <div className="button-row">
            <button className="button button-primary" type="submit">
              Save receiver details
            </button>
            <Link className="button button-secondary" href="/licenses/multi-collection">
              Clear
            </Link>
          </div>
        </form>
      ...
      <LicenseVehicleSearch
        mode={mode}
        number={number}
        clearHref="/licenses/multi-collection"
        hiddenFields={{ receiver, receiverId, receiverTel, dateCollected }}
      />
      {details ? (
        <>
          <LicenseDetailsForm
            ...
            workflow="multi-collection"
          />
          <LicenseHistoryTable entries={history?.history ?? []} />
        </>
      ) : (
        <p className="muted-copy">
          Enter receiver details and search a vehicle to load its licence record.
        </p>
      )}
