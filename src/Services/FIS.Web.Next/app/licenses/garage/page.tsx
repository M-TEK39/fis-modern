...
export default async function LicenseGaragePage({ searchParams }: Readonly<{ searchParams: Promise<Record<string, string | string[] | undefined>> }>) {
  ...
  return <LicenseShell title="Licence At Garage Maintenance" description="Capture a licence that is available at the garage."><LicenseNotice query={query} />{errorMessage ? <section className="vehicle-status-card" role="alert"><h2>{errorMessage}</h2><p className="muted-copy">The legacy record remains unchanged. Check the vehicle number and retry.</p></section> : null}<section className="vehicle-status-card"><p className="eyebrow">Legacy workflow rule</p><p className="muted-copy">The Exp Date is preloaded one year after the current licence date, matching the garage collection workflow. Review it before saving.</p></section><LicenseVehicleSearch mode={mode} number={number} clearHref="/licenses/garage" />{details ? <><LicenseDetailsForm details={details} sites={sites} returnPath={returnPath} workflow="garage" /><LicenseHistoryTable entries={history?.history ?? []} /></> : <p className="muted-copy">Search a vehicle to load its licence details.</p>}<div className="vehicle-footer-actions"><Link className="button button-secondary" href="/licenses">Licence Menu</Link><Link className="button button-secondary" href="/licenses/garage">Clear</Link></div></LicenseShell>;
}
