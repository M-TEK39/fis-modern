...
export default async function LicenseOneVehiclePage({
  ...
  return (
    <LicenseShell
      title="Licence Maintenance for ONE Vehicle"
      description="Capture and maintain the licence record for one vehicle."
    >
      ...
      <LicenseVehicleSearch mode={mode} number={number} />
      {details ? (
        <>
          <LicenseDetailsForm details={details} sites={sites} returnPath={returnPath} />
          <LicenseHistoryTable entries={history?.history ?? []} />
        </>
      ) : (
        <p className="muted-copy">
          Search a vehicle to load its licence details and renewal history.
        </p>
      )}
      <div className="vehicle-footer-actions">
        ...
        <Link className="button button-secondary" href="/licenses/one-vehicle">
          Clear
        </Link>
      </div>
    </LicenseShell>
