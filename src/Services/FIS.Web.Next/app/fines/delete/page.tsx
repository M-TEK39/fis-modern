...
import { FineApiError, getFines, searchFineVehicles, type FineRecord, type FineSearchType, type FineVehicleOption } from "@/lib/api-fines";
...
function SearchForm({ searchType, searchQuery }: Readonly<{ searchType: FineSearchType; searchQuery: string }>) {
  return (
    <form className="vehicle-status-maintenance-panel" method="get">
      ...
      <div className="vehicle-search-row">
        <label className="sr-only" htmlFor="fine-delete-search">{searchType === "GG" ? "GG number" : "GP number"}</label>
        <input
...
function Resolution({ searchType, vehicles }: Readonly<{ searchType: FineSearchType; vehicles: FineVehicleOption[] }>) {
  if (vehicles.length === 0) {
    return null;
  }

  return (
    <section className="vehicle-status-maintenance-panel" aria-labelledby="fine-delete-resolution-title">
      ...
      <ul className="form-hint fis-list-pad">
        {vehicles.slice(0, 5).map((vehicle) => (
          <li key={vehicle.vmfCode}>
            ...
            {searchType === "GP" && vehicle.isHistoricalMatch && vehicle.matchedRegistration ? ` — historical GP ${vehicle.matchedRegistration}` : ""}
          </li>
        ))}
      </ul>
    </section>
...
function FineRows({ fines }: Readonly<{ fines: FineRecord[] }>) {
  if (fines.length === 0) {
    return <div className="vehicle-empty-state"><p className="eyebrow">No records found</p><h2>No fines matched this vehicle.</h2><p className="muted-copy">Try another GG or GP number.</p></div>;
  }
  ...
  return (
    <div className="vehicle-table-wrapper" aria-live="polite">
      <table className="vehicle-table">
        ...
        <thead><tr><th scope="col">Offence Date</th><th scope="col">Document Type</th><th scope="col">Date at GMT</th><th scope="col">Action</th></tr></thead>
        <tbody>
          {fines.map((fine) => (
            <tr key={fine.fineCode}>
              ...
              <td><Link className="button button-danger button-small" href={buildDetailHref(fine.fineCode)}>DELETE</Link></td>
            </tr>
          ))}
        </tbody>
      </table>
...
function ApiUnavailable() {
  return (
    <section className="vehicle-status-card" role="alert">
      ...
      <Link className="button button-primary" href="/fines/delete">Try again</Link>
    </section>
  );
}

export default async function FineDeletePage({ searchParams, routePath = "/fines/delete" }: FineDeletePageProps) {
  await connection();
  ...
  try {
    ...
    const fines = matchingCodes === null ? allFines : allFines.filter((fine) => fine.vmfCode !== null && matchingCodes.has(fine.vmfCode));

    return (
      <main className="page-shell vehicle-page-shell">
        <section className="vehicle-card" aria-labelledby="fine-delete-title">
          <header className="vehicle-page-header">
            <div><p className="eyebrow">Fines maintenance</p><h1 id="fine-delete-title">Delete Fines</h1><p>Find a fine by GG or current/historical GP number, review it, then confirm deletion.</p></div>
            <Link className="button button-secondary" href="/fines">Fines Menu</Link>
          </header>
          {deleted ? <div className="notice notice-success" role="status">Fine deleted successfully.</div> : null}
          {error ? <div className="notice notice-error" role="alert">{error}</div> : null}
          <SearchForm searchType={searchType} searchQuery={searchQuery} />
          <Resolution searchType={searchType} vehicles={vehicles} />
          <section className="vehicle-status-maintenance-panel" aria-labelledby="fine-delete-results-title">
            <div className="vehicle-form-section-header"><div><p className="eyebrow">Fine maintenance</p><h2 id="fine-delete-results-title">Fines found</h2></div></div>
    ...
    console.error("FIS fine deletion lookup failed", requestError instanceof Error ? requestError.message : "unknown error");
    return <main className="page-shell vehicle-page-shell"><ApiUnavailable /></main>;
  }
}
