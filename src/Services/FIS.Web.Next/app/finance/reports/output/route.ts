...
function htmlFor(report: FinanceReport) {
  ...
  const headers = columns
    .map((column) => `<th>${escapeHtml(column.replaceAll("_", " "))}</th>`)
    .join("");
  ...
  try {
    ...
    if (kind === "dedicated") {
      ...
      const output = await getFinanceOutput(
        dedicatedFinanceReportPath(reportAction, { id, postingMonthCode, filterBy, format }),
      );
      return outputResponse(
        output,
    ...
    if (kind === "wesbank") {
      ...
      if (!vmfCode || !financialYear)
        return NextResponse.json(
          { message: "A vehicle and financial year are required." },
          { status: 400 },
        );
    ...
    return new NextResponse(htmlFor(report), {
      headers: { "content-type": "text/html; charset=utf-8", "content-disposition": "inline" },
    });
  } catch (error) {
    const status =
      error instanceof FinanceApiError && error.reason === "unauthorized"
        ? 401
        : error instanceof FinanceApiError && error.reason === "invalid-response"
          ? 502
          : 503;
    return NextResponse.json(
