...
function messageFor(value: unknown, action: BatchAction) {
  if (value && typeof value === "object" && !Array.isArray(value)) {
    ...
    if (action === "check-scoa") {
      ...
      if (compliant === true) return warnings instanceof Array && warnings.length > 0 ? `SCOA check passed with ${warnings.length} warning(s).` : "SCOA check passed.";
      if (errors instanceof Array && errors.length > 0) return errors.filter((item): item is string => typeof item === "string").join(" ");
      return "SCOA check found issues.";
    }
  }
  return action === "start" ? "Batch started." : action === "rollback" ? "Batch rolled back." : action === "finish" ? "Batch finished." : "Batch action completed.";
}
...
export async function runBatchAction(formData: FormData) {
  ...
  if (session.status !== "authenticated") redirect(resultPath("batch-management", "error", "The sign-in service is temporarily unavailable. Please try again."));
  if (!hasFinanceRole(session.roles)) redirect(resultPath("batch-management", "forbidden", "Your account does not have permission to manage batches."));

  const rawAction = text(formData, "action").toLowerCase();
  if (!ACTIONS.includes(rawAction as BatchAction)) redirect(resultPath("batch-management", "error", "The requested batch action is invalid."));
  const action = rawAction as BatchAction;
  const endpoint = `api/finance/batch/${action}`;
  const batchDate = text(formData, "batchDate");
  if (action === "start" && !/^\d{4}-\d{2}-\d{2}$/.test(batchDate)) redirect(resultPath(action, "error", "Select a valid batch date."));

  let result: unknown;
  try {
    result = await runFinanceAction(endpoint, action === "start" ? { batchDate: `${batchDate}T00:00:00`, financialSystemCode: 1 } : {});
  } catch (error) {
