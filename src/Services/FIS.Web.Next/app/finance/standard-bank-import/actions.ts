...
import { hasHeadOfficeFinanceAccess } from "@/app/finance/_components";
import { FinanceApiError, importStandardBankFile } from "@/lib/api-finance";
...
function responseMessage(value: unknown) {
  if (value && typeof value === "object" && !Array.isArray(value)) {
    ...
    if (typeof imported === "number" && typeof failed === "number") {
      return failed > 0 ? `Imported ${imported} transaction(s); ${failed} row(s) failed.` : imported > 0 ? `Imported ${imported} transaction(s) successfully.` : "No transaction rows were imported.";
    }
...
export async function importStandardBankAction(formData: FormData) {
  ...
  if (!hasHeadOfficeFinanceAccess(session.siteCode, session.email, session.roles)) redirect(resultPath("forbidden", "Your account does not have permission to import Standard Bank transactions."));

  const fileValue = formData.get("file");
  if (!fileValue || typeof fileValue !== "object" || !("arrayBuffer" in fileValue) || typeof fileValue.arrayBuffer !== "function") redirect(resultPath("error", "Select a CSV file before uploading."));
  const file = fileValue as Blob & { name?: string; size?: number };
  if (!file.name?.toLowerCase().endsWith(".csv")) redirect(resultPath("error", "Only CSV files can be uploaded."));
  if (!file.size || file.size <= 0 || file.size > 20 * 1024 * 1024) redirect(resultPath("error", "The CSV file must be between 1 byte and 20 MB."));
