...
import {
  getTrackingAllVehiclesReport,
  TrackingApiError,
} from "@/lib/api/fleet-operations/api-tracking";

async function TrackingAllVehicleReportPageContent({
  searchParams,
  ...
  try {
    ...
    return (
      <TrackingShell
        ...
        description="View tracking records across all vehicles."
      >
        <TrackingNotice query={query} />
        <form className="vehicle-status-maintenance-panel" method="get">
          <div className="form-field">
            <span className="form-label">Tracker type</span>
            <div className="contract-search-radio">
              <label className="form-radio-label">
                <input
                  ...
                  name="trackerType"
                  value="New"
                  defaultChecked={trackerType === "New"}
                />{" "}
                New
              </label>
              <label className="form-radio-label">
                <input
                  ...
                  name="trackerType"
                  value="Reused"
                  defaultChecked={trackerType === "Reused"}
                />{" "}
                Re-used
              </label>
              <label className="form-radio-label">
                <input
                  ...
                  name="trackerType"
                  value="All"
                  defaultChecked={trackerType === "All"}
                />{" "}
                All
              </label>
            </div>
    ...
    return (
      ...
      >
        <section className="vehicle-status-card" role="alert">
          ...
          <Link className="button button-primary" href="/tracking/reports/all-vehicle">
            Try again
          </Link>
        </section>
      </TrackingShell>
...
export default function TrackingAllVehicleReportPage(
  props: Parameters<typeof TrackingAllVehicleReportPageContent>[0],
) {
  return (
    <Suspense fallback={<RouteLoading />}>
      <TrackingAllVehicleReportPageContent {...props} />
    </Suspense>
  );
}
