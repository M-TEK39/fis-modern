...
async function renderFmlAddLeasePageContent({
  ...
  if (session.status === "expired")
    return (
      <main className="page-shell vehicle-page-shell">
        <SessionRecovery returnPath="/full-maintenance-lease/add-lease" />
      </main>
    );
  if (session.status === "unavailable")
    return (
      <main className="page-shell vehicle-page-shell">
        <ApiUnavailable message="The FML tariff form could not be opened." />
      </main>
    );
  ...
  try {
    ...
    return (
      <FmlFrame
        title="Add Lease Tariff"
        description="Capture a new tariff period for an in-service lease vehicle."
      >
      ...
      if (!(error instanceof FmlApiError && error.reason === "not-found")) {
        return (
          <FmlFrame
            ...
            description="Capture a new tariff period for an in-service lease vehicle."
          >
            <ApiUnavailable message="The existing lease tariff could not be loaded." />
          </FmlFrame>
        );
      }
    }
  ...
  return (
    ...
    >
      ...
      <div className="notice notice-info" role="note">
        Select a vehicle first. The legacy workflow starts the new tariff from the selected
        vehicle’s latest tariff end date.
      </div>
        ...
        <div className="form-card-body">
          <form method="get" className="form-grid">
            <div className="form-field">
              <label className="form-label" htmlFor="fml-add-search">
                Search number
              </label>
              <input
            ...
            <div className="form-field">
              <label className="form-label" htmlFor="fml-add-mode">
                Number type
              </label>
              <select className="form-select" id="fml-add-mode" name="mode" defaultValue={mode}>
      ...
      {selectedVehicle ? (
        <section className="form-card">
          ...
          <div className="form-card-body">
            <form action={createLeaseTariffAction} className="form-grid">
              <input name="vmfCode" type="hidden" value={selectedVehicle.vmfCode} readOnly />
              <div className="form-field">
                <label className="form-label" htmlFor="fml-add-start">
                  Start date
                </label>
                <input
                  className="form-input"
              ...
              <div className="form-field">
                <label className="form-label" htmlFor="fml-add-end">
                  End date
                </label>
                <input
                  className="form-input"
              ...
              <div className="form-field">
                <label className="form-label" htmlFor="fml-add-fixed">
                  Fixed tariff
                </label>
                <input
                  className="form-input"
                  ...
                  min="0"
                  step="0.01"
                  defaultValue={latest?.fixedTariff ?? ""}
                  required
                />
              </div>
              <div className="form-field">
                <label className="form-label" htmlFor="fml-add-excess">
                  Excess kilo tariff
                </label>
                <input
                  className="form-input"
                  ...
                  min="0"
                  step="0.01"
                  defaultValue={latest?.excessKiloTariff ?? ""}
                />
              </div>
              <div className="form-actions form-group-full">
                ...
                <Link className="button button-secondary" href="/full-maintenance-lease/add-lease">
                  Clear
                </Link>
              </div>
            </form>
            {latest ? (
              <p className="muted-copy">
                Latest tariff: {formatDate(latest.startDate)} to {formatDate(latest.endDate)} ·{" "}
                {formatCurrency(latest.fixedTariff)}
              </p>
            ) : (
              <p className="muted-copy">No existing tariff was found for this vehicle.</p>
            )}
          </div>
        </section>
      ) : (
        <section className="vehicle-status-card" role="status">
          ...
          <p className="muted-copy">Search above, then select the matching in-service vehicle.</p>
        </section>
      )}
      <div className="button-row">
        <Link className="button button-secondary" href="/full-maintenance-lease/tariffs">
          Open capture queue
        </Link>
