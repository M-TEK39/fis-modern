...
import { getQueryValue, parsePositiveInteger } from "@/app/(administration)/drivers/access";
import {
...
function DetailTable({
  ...
  siteName,
  licenceType,
}: Readonly<{ driver: DriverManagementDriver; siteName: string; licenceType: string }>) {
  const details = [
  ...
  return (
    <section
      ...
      aria-labelledby="driver-licence-results-title"
    >
      <div className="vehicle-form-section-header">
        <div>
          <p className="eyebrow">Driver record</p>
          <h2 id="driver-licence-results-title">Licence details for {displayName(driver)}</h2>
        </div>
        <span className="form-hint">{details.length} fields</span>
      </div>
...
function ApiUnavailable() {
  return (
    <section className="vehicle-status-card" role="alert">
      ...
      <h2>Driver licence details could not be loaded.</h2>
      <p className="muted-copy">Retry when the FIS API is available.</p>
      <Link className="button button-primary" href="/trips/driver-licence-details">
        Try again
      </Link>
    </section>
  );
...
export default async function DriverLicenceDetailsPage({
  ...
  const selectedCode = parsePositiveInteger(
    getQueryValue(query.siteDriverCode) ?? getQueryValue(query.SiteDriverCode),
  );
  ...
  try {
    ...
    return (
      <main className="page-shell vehicle-page-shell">
        <section className="vehicle-card" aria-labelledby="driver-licence-details-title">
          <header className="vehicle-page-header">
            <div>
              <p className="eyebrow">Trip tools</p>
              <h1 id="driver-licence-details-title">Driver Licence Details</h1>
              <p>View driver licence details by site driver ID.</p>
            </div>
          ...
          <form className="vehicle-status-maintenance-panel" method="get">
            <div className="form-grid">
              <div className="form-field">
                <label className="form-label" htmlFor="driver-licence-driver">
                  Site Driver ID
                </label>
                <select
                  className="form-select"
                  id="driver-licence-driver"
                  name="siteDriverCode"
                  defaultValue={selectedCode?.toString() ?? ""}
                >
                  <option value="">Select driver</option>
                  {drivers.map((item) => (
                    <option key={item.siteDriverCode} value={item.siteDriverCode}>
                      {displayName(item)} ({item.siteDriverCode})
                    </option>
                  ))}
                </select>
              </div>
            ...
            <div className="button-row">
              ...
              <Link className="button button-secondary" href="/trips/driver-licence-details">
                Clear
              </Link>
            </div>
          </form>
          {driver === null ? (
            <section className="vehicle-empty-state" aria-live="polite">
              <p className="eyebrow">No record selected</p>
              <h2>No driver licence details found.</h2>
              <p className="muted-copy">Select a site driver and fetch the details.</p>
            </section>
          ) : (
            <DetailTable
              driver={driver}
              siteName={
                site?.description
                  ? `${site.description} (${driver.siteCode})`
                  : String(driver.siteCode)
              }
              licenceType={
                licenceType?.description || licenceType?.code || String(driver.driverLicenceTypeId)
              }
            />
          )}
    ...
    if (error instanceof DriverManagementApiError && error.reason === "unauthorized")
      return (
        <main className="page-shell vehicle-page-shell">
          <SessionRecovery returnPath="/trips/driver-licence-details" />
        </main>
      );
    console.error(
      "FIS driver licence details request failed",
