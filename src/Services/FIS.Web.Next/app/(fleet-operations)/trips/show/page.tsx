...
import { closeTripAuthorityAction } from "@/app/(fleet-operations)/trips/actions";
import {
...
import { getDepartment, type DepartmentRecord } from "@/lib/api/reference-data/api-departments";
import { getSite, type SiteRecord } from "@/lib/api/reference-data/api-sites";
import {
  getTripAuthorityDetails,
  TripAuthorityApiError,
  type TripAuthorityDetails,
  type TripAuthorityRoute,
...
function selectedTab(value: string): Tab {
  return value === "trip" || value === "drivers" || value === "passengers" || value === "routes"
    ? value
    : "vehicle";
}
...
function FactsTable({
  caption,
  rows,
}: Readonly<{ caption: string; rows: readonly [string, ReactNode][] }>) {
  return (
...
function VehicleInformation({
  ...
  details: TripAuthorityDetails;
  contract: ContractRecord | null;
  vehicle: VehicleStatusVehicle | null;
        ...
        rows={[
          ...
          [
            ...
            site
              ...
              : valueOrDash(contract?.siteCode ?? trip.siteCode),
          ],
...
function TripInformation({
  details,
  contract,
}: Readonly<{ details: TripAuthorityDetails; contract: ContractRecord | null }>) {
  const trip = details.trip;
  return (
    <section className="vehicle-form-section" aria-labelledby="trip-information-title">
      <div className="vehicle-form-section-header">
        <div>
          <p className="eyebrow">Authority record</p>
          <h2 id="trip-information-title">Trip Information</h2>
        </div>
      </div>
      <FactsTable
        caption="Trip authority information"
        rows={[
...
function DriverInformation({ details }: Readonly<{ details: TripAuthorityDetails }>) {
  return (
    <section className="vehicle-form-section" aria-labelledby="trip-driver-information-title">
      <div className="vehicle-form-section-header">
        <div>
          <p className="eyebrow">
            {details.drivers.length} record{details.drivers.length === 1 ? "" : "s"}
          </p>
          <h2 id="trip-driver-information-title">Driver Information</h2>
        </div>
      </div>
      {details.drivers.length === 0 ? (
        <p className="muted-copy">No driver records found for this trip.</p>
      ) : (
        <div className="vehicle-table-wrapper">
          <table className="vehicle-table">
            ...
            <tbody>
              {details.drivers.map((driver) => (
                <tr key={driver.tripDriverCode}>
                  <td>{valueOrDash(driver.name)}</td>
                  <td>{valueOrDash(driver.identityNumber)}</td>
                  <td>{valueOrDash(driver.passportNumber)}</td>
                  <td>{valueOrDash(driver.licenceNumber)}</td>
                  <td>{formatBoolean(driver.isPrimary)}</td>
                  <td>{formatBoolean(driver.isActive)}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
function PassengerInformation({ details }: Readonly<{ details: TripAuthorityDetails }>) {
  return (
    <section className="vehicle-form-section" aria-labelledby="trip-passenger-information-title">
      <div className="vehicle-form-section-header">
        <div>
          <p className="eyebrow">
            {details.passengers.length} record{details.passengers.length === 1 ? "" : "s"}
          </p>
          <h2 id="trip-passenger-information-title">Passenger Information</h2>
        </div>
      </div>
      {details.passengers.length === 0 ? (
        <p className="muted-copy">No passenger records found for this trip.</p>
      ) : (
        <div className="vehicle-table-wrapper">
          <table className="vehicle-table">
            <caption className="sr-only">
            ...
            <tbody>
              {details.passengers.map((passenger) => (
                <tr key={passenger.tripPassengerCode}>
                  <td>{valueOrDash(passenger.name)}</td>
                  <td>{passenger.tripPassengerCode}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
function RouteInformation({ details }: Readonly<{ details: TripAuthorityDetails }>) {
  ...
  return (
    <section className="vehicle-form-section" aria-labelledby="trip-route-information-title">
      <div className="vehicle-form-section-header">
        <div>
          <p className="eyebrow">
            {details.routes.length} route{details.routes.length === 1 ? "" : "s"}
          </p>
          <h2 id="trip-route-information-title">Route Information</h2>
        </div>
      </div>
      {details.routes.length === 0 ? (
        <p className="muted-copy">No route records found for this trip.</p>
      ) : (
        <div className="vehicle-table-wrapper">
          <table className="vehicle-table">
            ...
            <tbody>
              {details.routes.map((route) => {
                ...
                previousEndOdometer = route.endOdometer ?? startOdometer;
                return (
                  <tr key={route.routeCode}>
                    <td>{route.routeCode}</td>
                    <td>{formatDate(route.startDate)}</td>
                    <td>{formatDate(route.endDate)}</td>
                    <td>{valueOrDash(startOdometer)}</td>
                    <td>{valueOrDash(route.endOdometer)}</td>
                    <td>{valueOrDash(route.distance ?? route.estimatedDistance)}</td>
                    <td>
                      {valueOrDash(route.startLocation)} → {valueOrDash(route.endLocation)}
                    </td>
                    <td>
                      Resp: {valueOrDash(route.responsibilityCode)}
                      <br />
                      Obj: {valueOrDash(route.objectiveCode)}
                      <br />
                      Project: {valueOrDash(route.projectNumber)}
                      <br />
                      Fund: {valueOrDash(route.fundCode)}
                    </td>
                  </tr>
                );
              })}
      ...
      <p className="muted-copy">
        Route end odometers are entered in the close form below. The API validates the persisted
        route sequence and required BAS fields before saving.
      </p>
    </section>
  );
}
...
function CloseTripForm({ details }: Readonly<{ details: TripAuthorityDetails }>) {
  ...
  return (
    <form action={closeTripAuthorityAction} className="vehicle-status-maintenance-panel">
      <div className="vehicle-form-section-header">
        <div>
          <p className="eyebrow">Open trip authority</p>
          <h2>Save and Close</h2>
          <p>
            Enter each route end odometer. The server calculates the route distances and trip end
            odometer from the persisted legacy records.
          </p>
        </div>
      </div>
      <input name="tripId" type="hidden" value={details.trip.tripId} />
      <input name="contractCode" type="hidden" value={details.trip.contractCode} />
      <input name="vmfCode" type="hidden" value={details.trip.vmfCode ?? ""} />
      {details.routes.length === 0 ? (
        <div className="form-field">
          <label className="form-label" htmlFor="trip-end-odometer">
            End ODO Meter
          </label>
          <input
          ...
          <table className="vehicle-table">
            ...
            <tbody>
              {details.routes.map((route) => (
                <tr key={route.routeCode}>
                  <td>
                    <input name="routeCode" type="hidden" value={route.routeCode} />
                    {route.routeCode}
                  </td>
                  <td>{valueOrDash(route.startOdometer)}</td>
                  <td>
                    <label className="sr-only" htmlFor={`route-end-odometer-${route.routeCode}`}>
                      End odometer for route {route.routeCode}
                    </label>
                    <input
                      className="form-input"
                      id={`route-end-odometer-${route.routeCode}`}
                      min={route.startOdometer ?? 0}
                      name={`routeEndOdometer-${route.routeCode}`}
                      type="number"
                      defaultValue={route.endOdometer ?? ""}
                      required
                    />
                  </td>
                </tr>
      ...
      <div className="button-row">
        ...
        <button className="button button-secondary" name="intent" value="renew" type="submit">
          Save and Renew
        </button>
        <Link
          className="button button-secondary"
          href={`/trips/show?tripId=${details.trip.tripId}&tab=routes`}
        >
          Review routes
        </Link>
      </div>
...
function resultMessage(result: string) {
  switch (result) {
    ...
    case "forbidden":
      return {
        tone: "error",
        text: "Your profile does not include Trip Authority access.",
      } as const;
    case "validation":
      return {
        tone: "error",
        text: "Enter a valid end odometer for every route before closing the trip.",
      } as const;
    case "missing-trip":
    ...
    case "unavailable":
      return {
        tone: "error",
        text: "The trip authority service is unavailable. Retry when the FIS API is available.",
      } as const;
    case "error":
...
function unavailableMessage() {
  return (
    <section className="vehicle-status-card" role="alert">
      ...
      <h2>Trip authority details could not be loaded.</h2>
...
export default async function ShowTripPage({
  ...
  if (tripId === null)
    return (
      <main className="page-shell vehicle-page-shell">
        <section className="vehicle-status-card" role="alert">
          <p className="eyebrow">Trip authority not selected</p>
          <h2>Choose a trip authority from the Trip Authority list.</h2>
          <Link className="button button-primary" href="/trip-authorities">
            Trip Authority list
          </Link>
        </section>
      </main>
  ...
  try {
    ...
    if (error instanceof TripAuthorityApiError && error.reason === "unauthorized")
      return (
        <main className="page-shell vehicle-page-shell">
          <SessionRecovery returnPath={returnPath} />
        </main>
      );
    if (error instanceof TripAuthorityApiError && error.reason === "not-found")
      return (
        <main className="page-shell vehicle-page-shell">
          <section className="vehicle-status-card" role="alert">
            <p className="eyebrow">Trip authority not found</p>
            <h2>Trip authority {tripId} could not be found.</h2>
    ...
    console.error(
      "FIS trip authority details failed",
  ...
  if (contract?.siteCode) {
    ...
    if (site?.departmentCode)
      department = await getDepartment(site.departmentCode).catch(() => null);
  }
  ...
  return (
    <main className="page-shell vehicle-page-shell">
      <article className="vehicle-card" aria-labelledby="show-trip-title">
        <header className="vehicle-page-header">
          <div>
            <p className="eyebrow">Trip Authority No: {details.trip.tripId}</p>
            <h1 id="show-trip-title">Trip Authority {details.trip.tripId}</h1>
            <p>
              Trip Status: <strong>{status}</strong>
            </p>
          </div>
          <div className="button-row">
            <Link className="button button-secondary" href="/trip-authorities">
            ...
            <PrintButton label="Print Trip Authority" />
        ...
        {details.trip.lockedForTransfer ? (
          <div className="notice notice-error" role="alert">
            This trip is locked for transfer and cannot be modified.
          </div>
        ) : null}
        <section className="vehicle-overview" aria-labelledby="trip-overview-title">
          <p className="eyebrow">Trip record</p>
          <h2 id="trip-overview-title">
            Trip Authority No: {details.trip.tripId} has the following information
          </h2>
          <TripTabs tripId={details.trip.tripId} activeTab={activeTab} />
          {activeTab === "vehicle" ? (
            <VehicleInformation
              details={details}
              contract={contract}
            ...
            <TripInformation details={details} contract={contract} />
          ) : activeTab === "drivers" ? (
            <DriverInformation details={details} />
          ) : activeTab === "passengers" ? (
            <PassengerInformation details={details} />
          ) : (
            <RouteInformation details={details} />
          )}
        </section>
        <CloseTripForm details={details} />
