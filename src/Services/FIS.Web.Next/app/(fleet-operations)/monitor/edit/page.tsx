...
export default async function MonitorEditPage({
  ...
  try {
    ...
    return (
      <MonitorShell
        title="Edit / Update an Existing Inquiry"
        description="Update a captured Call Centre monitoring inquiry."
      >
        <MonitorNotice query={query} />
        <form className="vehicle-status-maintenance-panel" method="get">
          <div className="vehicle-form-section-header">
            <div>
              <p className="eyebrow">Inquiry lookup</p>
              <h2>Enter a reference number</h2>
            </div>
          </div>
          <div className="vehicle-search-row">
            <label className="sr-only" htmlFor="monitor-reference-number">
              Reference number
            </label>
            <input
              className="vehicle-search"
              id="monitor-reference-number"
        ...
        {reference && !record ? (
          <section className="vehicle-status-card" role="alert">
            ...
            <p className="muted-copy">No monitor inquiry matched reference {reference}.</p>
          </section>
        ) : record ? (
          <MonitorForm
            record={record}
            vmfCode={record.vmfCode ?? 0}
            sites={sites}
            drivers={drivers}
            returnPath={`/monitor/edit?referenceNumber=${record.monitorCode}`}
          />
        ) : null}
    ...
    return (
      ...
      >
        <section className="vehicle-status-card" role="alert">
          <h2>
            {error instanceof MonitorApiError && error.reason === "not-found"
              ? "Inquiry not found."
              : "The Monitor service is temporarily unavailable."}
          </h2>
          <Link className="button button-primary" href="/monitor/edit">
            Try again
          </Link>
        </section>
      </MonitorShell>
