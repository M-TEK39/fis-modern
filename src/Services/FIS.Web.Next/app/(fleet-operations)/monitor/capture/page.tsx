...
import { getMonitorDrivers, MonitorApiError } from "@/lib/api/fleet-operations/api-monitor";
...
async function referenceData(): Promise<
  [SiteRecord[], Awaited<ReturnType<typeof getMonitorDrivers>>]
> {
...
export default async function MonitorCapturePage({
  ...
  try {
    ...
    const selected =
      Number.isInteger(requestedVmf) && requestedVmf > 0
        ? vehicles.find((vehicle) => vehicle.vmfCode === requestedVmf)
        : null;
    return (
      <MonitorShell
        title="Capture a New Inquiry"
        description="Capture a new Call Centre monitoring inquiry."
      >
        ...
        <form className="vehicle-status-maintenance-panel" method="get">
          <div className="vehicle-form-section-header">
            <div>
              ...
              <h2>Find a vehicle before capturing an inquiry</h2>
            </div>
          </div>
          <div className="vehicle-search-row">
            <label className="sr-only" htmlFor="monitor-lookup-mode">
              Lookup type
            </label>
            <select
              className="form-select"
              id="monitor-lookup-mode"
              name="mode"
              defaultValue={mode}
            >
            ...
            <label className="sr-only" htmlFor="monitor-lookup-search">
              Vehicle number
            </label>
            <input
              className="vehicle-search"
              id="monitor-lookup-search"
              name="search"
              defaultValue={search}
              placeholder={mode === "GP" ? "Enter registration number" : "Enter GG number"}
              required
            />
            <label className="sr-only" htmlFor="monitor-lookup-vehicle">
              Vehicle match
            </label>
            <select
              className="form-select"
              id="monitor-lookup-vehicle"
              name="vmfCode"
              defaultValue={selected?.vmfCode ?? ""}
            >
        ...
        {selected ? (
          <MonitorForm
            ...
            sites={sites}
            drivers={drivers}
            returnPath={`/monitor/capture?mode=${encodeURIComponent(mode)}&search=${encodeURIComponent(search)}&vmfCode=${selected.vmfCode}`}
          />
        ) : (
          <section className="vehicle-status-card">
            <p className="muted-copy">
              Search for a GG or registration number, select the vehicle match, then continue with
              the inquiry details.
            </p>
          </section>
        )}
    ...
    return (
      ...
      >
        <section className="vehicle-status-card" role="alert">
          <h2>
            {error instanceof MonitorApiError && error.reason === "unauthorized"
              ? "Your session has expired."
              : "Monitor reference data could not be loaded."}
          </h2>
          <p className="muted-copy">Retry after the FIS API is available.</p>
          <Link className="button button-primary" href="/monitor/capture">
            Try again
          </Link>
        </section>
      </MonitorShell>
