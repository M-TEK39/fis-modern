...
export async function FinancialAllocationRoute({ searchParams, action }: AllocationPageProps) {
  ...
  if (submitted) {
    try {
      if (normalizedAction === "activate-bas" || normalizedAction === "view-bas")
        segments = await getBasSegments(departmentCode, segmentType || undefined);
      if (normalizedAction === "invalid-journals" || normalizedAction === "fix-invalid-journals")
        rows = await getBasRows(
          `api/finance/bas/journals/invalid${departmentCode ? `?departmentCode=${departmentCode}` : ""}`,
        );
      if (normalizedAction === "allocate-fund-codes")
        rows = await getBasRows(
          `api/finance/bas/journals/uninvoiced${departmentCode ? `?departmentCode=${departmentCode}` : ""}`,
        );
      if (normalizedAction === "departments-no-bas")
        rows = await getBasRows("api/finance/bas/departments-without-bas");
      if (normalizedAction === "departments-missing-financial-system")
        rows = await getBasRows("api/finance/bas/departments-missing-financial-system");
    } catch (caught) {
      error =
  ...
  const count =
    normalizedAction === "activate-bas" || normalizedAction === "view-bas"
      ? segments.length
      : rows.length;
      ...
      {normalizedAction === "import-bas" ? (
        ...
        <>
          ...
          <form action={activateBasSegmentsAction} className="vehicle-status-maintenance-panel">
            <SegmentTable
              segments={segments.slice((page - 1) * 12, page * 12)}
          ...
          <Paginator action={normalizedAction} query={query} page={page} totalPages={totalPages} />
        </>
        ...
        <>
          ...
          <GenericTable action={normalizedAction} rows={rows.slice((page - 1) * 12, page * 12)} />
          <Paginator action={normalizedAction} query={query} page={page} totalPages={totalPages} />
        </>
      )}
