  ...
  if (session.status !== "authenticated")
    return (
      <FinanceFrame
        ...
        description="Import monthly Standard Bank transaction files."
      >
  ...
  if (!hasHeadOfficeFinanceAccess(session.siteCode, session.legacyUsername))
    return (
      <FinanceFrame
        ...
        description="Import monthly Standard Bank transaction files."
      >
        <FinanceRestricted message="Standard Bank imports are limited to the Finance head-office workflow." />
      </FinanceFrame>
    );
    ...
    >
      ...
      <section className="vehicle-status-maintenance-panel">
        <div className="vehicle-form-section-header">
          <div>
            <p className="eyebrow">Wesbank fuel card import</p>
            <h2>Upload Standard Bank Transactions File</h2>
          </div>
        </div>
        <div className="notice notice-info" role="note">
          Download the latest DBF file from Wesbank, save it as CSV, then upload the CSV file.
        </div>
        <form action={importStandardBankAction} encType="multipart/form-data">
          <div className="form-grid">
            <div className="form-field">
              <label className="form-label" htmlFor="standard-bank-file">
                CSV file
              </label>
              <input
                className="form-input"
                id="standard-bank-file"
                name="file"
                type="file"
                accept=".csv,text/csv"
                required
