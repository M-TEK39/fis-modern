...
export default async function TowingRequestPage({
  searchParams,
  ...
  try {
    if (searchQuery) {
      const [all, matches] = await Promise.all([
        getTowings(),
        searchTowingVehicles(searchType, searchQuery),
      ]);
      vehicles = matches;
    ...
    <main className="page-shell vehicle-page-shell">
      <section className="vehicle-card" aria-labelledby="towing-request-title">
        ...
        >
          ...
          <TowingRows rows={rows} searchType={searchType} searchQuery={searchQuery} />
        </section>
