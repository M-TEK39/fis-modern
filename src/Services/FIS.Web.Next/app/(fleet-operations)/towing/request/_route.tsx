...
function TowingRows({
  ...
  searchType,
  searchQuery,
}: Readonly<{ rows: TowingRecord[]; searchType: TowingSearchType; searchQuery: string }>) {
  if (rows.length === 0) {
    return (
      <div className="vehicle-empty-state">
        ...
        <h2>
          {searchQuery
            ? `No towing requests matched “${searchQuery}”.`
            : "Search for a vehicle to view towing requests."}
        </h2>
        <p className="muted-copy">
          Use the GG fleet number or GP registration number from the legacy request screen.
        </p>
      </div>
    );
  }
            ...
            <tr key={item.towingCode}>
              ...
              <td>
                <Link
                  ...
                  href={buildDetailHref(item.towingCode, searchType, searchQuery)}
                >
                  Mod
                </Link>
              </td>
...
function TowingPagination({
  page,
  searchType,
  searchQuery,
}: Readonly<{
  page: TowingPage;
  searchType: TowingSearchType;
  searchQuery: string;
}>) {
  ...
  const buildHref = (nextPage: number) => {
    ...
    return `/towing/request?${params.toString()}`;
  };

  return (
    <nav className="table-pagination" aria-label="Towing request pages">
      {page.page > 1 ? (
        <Link className="button button-secondary button-small" href={buildHref(page.page - 1)}>
          Previous
        </Link>
      ) : (
      ...
      <span aria-live="polite">
        Page {page.page} of {page.totalPages} ({page.total} requests)
      </span>
...
function ApiUnavailable({ routePath }: Readonly<{ routePath: string }>) {
  return (
    <section className="vehicle-status-card" role="alert">
      ...
      <h2>Towing requests could not be loaded.</h2>
      <p className="muted-copy">
...
async function TowingRequestPageContent({
  ...
  const searchQuery = (getQueryValue(query.searchQuery) ?? getQueryValue(query.xggnum) ?? "")
    .trim()
    .slice(0, 8);
  const rawPage = Number(getQueryValue(query.page));
    ...
    (getQueryValue(query.saved) === "1"
      ...
      : getQueryValue(query.updated) === "1"
        ...
        : getQueryValue(query.deleted) === "1"
          ? "Towing request deleted successfully."
          : "");
  const isError = Boolean(getQueryValue(query.error));

  let towingPage: TowingPage = {
    items: [],
    page: requestedPage,
    pageSize: 24,
    total: 0,
  ...
  try {
    if (searchQuery) {
      const matches = await searchTowingVehicles(searchType, searchQuery);
      vehicles = matches;
      if (matches.length > 0) {
        towingPage = await getTowingPage({
          vmfCodes: matches.map((vehicle) => vehicle.vmfCode),
          page: requestedPage,
          pageSize: 24,
        });
      }
    ...
    if (error instanceof TowingApiError && error.reason === "unauthorized")
      return (
        <main className="page-shell vehicle-page-shell">
          <SessionRecovery
            returnPath={`${routePath}?${new URLSearchParams({ searchType, searchQuery }).toString()}`}
          />
        </main>
      );
    console.error(
      "FIS towing request lookup failed",
      ...
      <section className="vehicle-card" aria-labelledby="towing-request-title">
        <header className="vehicle-page-header">
          <div>
            ...
            <h1 id="towing-request-title">Road Side Assistance Request Maintenance</h1>
            <p>
              Search by the same GG or GP vehicle identifier used by the legacy screen, then
              capture, edit, or delete a request.
            </p>
          </div>
          <Link className="button button-secondary" href="/towing">
            Towing Menu
        ...
        {vehicles.length > 0 ? (
          <section
            ...
            aria-labelledby="towing-vehicle-match-title"
          >
            <p className="eyebrow">Vehicle resolution</p>
            <h2 id="towing-vehicle-match-title">
              Matched vehicle{vehicles.length === 1 ? "" : "s"}
            </h2>
            <ul className="form-hint fis-list-pad">
              {vehicles.slice(0, 5).map((vehicle) => (
                <li key={vehicle.vmfCode}>
                  ...
                  <Link
                    className="button button-secondary button-small"
                    href={`/towing/request/detail?vmfCode=${vehicle.vmfCode}&searchType=${searchType}&searchQuery=${encodeURIComponent(searchQuery)}`}
                  >
                    New request
                  </Link>
                </li>
        ...
        <section
          className="vehicle-status-maintenance-panel"
          aria-labelledby="towing-request-results-title"
        >
          <div className="vehicle-form-section-header">
            <div>
              <p className="eyebrow">Existing requests</p>
              <h2 id="towing-request-results-title">Requests found</h2>
            </div>
          </div>
          <TowingRows rows={towingPage.items} searchType={searchType} searchQuery={searchQuery} />
          <TowingPagination page={towingPage} searchType={searchType} searchQuery={searchQuery} />
        </section>
        <div className="vehicle-footer-actions">
          <Link className="button button-secondary" href="/towing/help">
            Help
          </Link>
...
export default function TowingRequestPage(props: Parameters<typeof TowingRequestPageContent>[0]) {
  return (
