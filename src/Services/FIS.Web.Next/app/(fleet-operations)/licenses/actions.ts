...
import {
  captureLicenseVehicle,
  LicenseApiError,
  submitLicensePassword,
...
function date(formData: FormData, key: string, label: string, required = false) {
  ...
  if (
    ...
    throw new LicenseValidationError(`${label} is invalid.`);
  return value;
}
...
function positiveInteger(formData: FormData, key: string, label: string) {
  ...
  if (!value || !Number.isSafeInteger(parsed) || parsed <= 0)
    throw new LicenseValidationError(`${label} must be a positive whole number.`);
  return parsed;
}
...
function optionalInteger(formData: FormData, key: string, label: string) {
  ...
  if (!Number.isSafeInteger(parsed) || parsed < 0)
    throw new LicenseValidationError(`${label} must be a whole number.`);
  return parsed;
}
...
function boundedText(formData: FormData, key: string, label: string, maxLength: number) {
  ...
  if (value.length > maxLength)
    throw new LicenseValidationError(`${label} must be ${maxLength} characters or fewer.`);
  return value || null;
}
...
function errorMessage(error: unknown) {
  ...
  if (error instanceof LicenseApiError) {
    if (error.reason === "unauthorized")
      return "Your session has expired or the licence password was rejected.";
    if (error.reason === "not-found") return "The vehicle was not found.";
    if (error.reason === "unavailable")
      return "The Licence service is temporarily unavailable. Please try again.";
  }
...
function workflowPath(formData: FormData) {
  switch (text(formData, "workflow")) {
    case "multi-collection":
      return "/licenses/multi-collection";
    case "garage":
      return "/licenses/garage";
    default:
      return "/licenses/one-vehicle";
  }
}
...
export async function saveLicenseVehicleAction(formData: FormData) {
  ...
  if (!hasRole(session.roles, "Licence"))
    redirectWith(path, "error", "You do not have permission to maintain Licences.");
  ...
  try {
    ...
    if (expiry < new Date("2004-01-01T00:00:00.000Z"))
      throw new LicenseValidationError("Exp Date must be after 2004-01-01.");
    const nextDay = new Date(expiry);
    nextDay.setUTCDate(nextDay.getUTCDate() + 1);
    if (nextDay.getUTCMonth() === expiry.getUTCMonth())
      throw new LicenseValidationError("Exp Date must be the last day of the month.");
    ...
    if (collected < new Date("2003-01-01T00:00:00.000Z"))
      throw new LicenseValidationError("Date Collected must be after 2003-01-01.");

    const cofRequired = boundedText(formData, "cofRequired", "Cof Required", 1);
    const cofExpDate = date(
      formData,
      "cofExpDate",
      "Cof Exp Date",
      cofRequired?.toUpperCase() !== "N",
    );
    if (cofRequired?.toUpperCase() !== "N" && !cofExpDate)
      throw new LicenseValidationError("Cof Exp Date is required when Cof Required is not N.");
    const dayDifference = Math.abs((collected.getTime() - expiry.getTime()) / 86_400_000);
    if (dayDifference > 500)
      throw new LicenseValidationError(
        "Date Collected and Exp Date cannot differ by more than 500 days.",
      );
    ...
    revalidatePath(path);
    redirectWith(
      `${path}?${new URLSearchParams({ mode, number, lookup: "1" }).toString()}`,
      "saved",
