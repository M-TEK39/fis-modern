...
import { ModelApiError, updateModelLicenceFee } from "@/lib/api/reference-data/api-models";
...
function positiveCode(formData: FormData, key: string, label: string) {
  ...
  if (!Number.isSafeInteger(parsed) || parsed <= 0 || parsed > 32767)
    throw new ModelFeeValidationError(`${label} must be a positive code.`);
  return parsed;
}
...
function message(error: unknown) {
  ...
  if (error instanceof ModelApiError) {
    ...
    if (error.reason === "unavailable")
      return "The model licence fee service is temporarily unavailable. Please try again.";
...
export async function updateModelLicenceFeeAction(formData: FormData) {
  ...
  if (session.status !== "authenticated")
    redirect(
      `${path}?error=${encodeURIComponent("Your session has expired. Sign in again before continuing.")}`,
    );
  if (!hasRole(session.roles, "Licence"))
    redirect(
      `${path}?error=${encodeURIComponent("You do not have permission to maintain Licence Fees.")}`,
    );
  ...
  try {
    ...
    if (!(await updateModelLicenceFee(modelCode, licenceFeeCode)))
      throw new ModelApiError("invalid-response", "The model update response was invalid.");
  } catch (error) {
    redirect(
      `${path}?modelCode=${encodeURIComponent(String(modelCode))}&error=${encodeURIComponent(message(error))}`,
    );
  }
