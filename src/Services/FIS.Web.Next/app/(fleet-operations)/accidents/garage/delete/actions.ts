...
export type GarageDeleteActionState = {
  status: "idle" | "error";
  message?: string;
};

const initialStatus: GarageDeleteActionState = { status: "idle" };
...
function apiErrorMessage(error: AccidentApiError) {
  if (error.reason === "unauthorized") {
    return "Your session has expired or you are no longer allowed to delete accidents. Sign in again.";
  }
...
export async function deleteGarageAccidentAction(
  _previousState: GarageDeleteActionState = initialStatus,
  formData: FormData,
  ...
  if (!hasRole(session.roles, ACCIDENTS_ROLE)) {
    return { status: "error", message: "You do not have permission to delete garage accidents." };
  }
  ...
  revalidatePath("/accidents/garage/delete");
  redirect("/accidents/garage/delete?deleted=1");
}
