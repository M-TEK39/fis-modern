...
import GarageEditForm from "@/app/(fleet-operations)/accidents/garage/edit/garage-edit-form";
import {
  AccidentApiError,
  getAccidentForEdit,
...
async function GarageEditContent({ searchParams }: GarageEditPageProps) {
  ...
  if (session.status === "expired") {
    return <SessionRecovery returnPath="/accidents/garage/edit" />;
  }
  ...
  try {
    ...
    if (error instanceof AccidentApiError && error.reason === "unauthorized") {
      return <SessionRecovery returnPath={`/accidents/garage/edit?accidentId=${accidentId}`} />;
    }
