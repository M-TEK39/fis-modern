  ...
  return (
    <section className="vehicle-status-card" role="alert">
      ...
      <div className="button-row">
        <Link className="button button-primary" href="/accidents/hq/add">
          Try again
        </Link>
...
async function HqAddContent({ searchParams, locationCode }: HqAddPageProps) {
  ...
  if (!hasRole(session.roles, ACCIDENTS_ROLE)) {
    return (
      <section className="vehicle-status-card" role="alert">
        ...
        <h2>You do not have permission to add HQ accidents.</h2>
      </section>
    );
  }
  ...
  try {
    ...
    if (error instanceof AccidentApiError && error.reason === "unauthorized")
      return <SessionRecovery returnPath="/accidents/hq/add" />;
    console.error(
      "FIS HQ accident vehicle options failed",
...
export function HqAddRoute({ searchParams, locationCode }: HqAddPageProps) {
  return (
      ...
      <section className="vehicle-card" aria-labelledby="hq-add-title">
        <header className="vehicle-page-header">
          <div>
            ...
            <h1 id="hq-add-title">Accident Maintenance (HQ) - Add</h1>
            <p>Capture a new HQ accident for a fleet vehicle.</p>
          </div>
          <Link className="button button-secondary" href="/accidents/hq">
            Back to Search
          </Link>
        ...
        <Suspense fallback={<LoadingState />}>
          <HqAddContent searchParams={searchParams} locationCode={locationCode} />
        </Suspense>
      </section>
    </main>
  );
