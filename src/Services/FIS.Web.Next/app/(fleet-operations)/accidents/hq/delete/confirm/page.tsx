...
import HqDeleteConfirm from "@/app/(fleet-operations)/accidents/hq/delete/hq-delete-confirm";
...
function ErrorState({ notFoundState = false }: Readonly<{ notFoundState?: boolean }>) {
  return (
    <section className="vehicle-status-card" role="alert">
      ...
      <h2>
        {notFoundState
          ? "That accident record could not be found."
          : "The accident record could not be loaded."}
      </h2>
...
async function HqDeleteConfirmContent({ searchParams }: HqDeleteConfirmPageProps) {
  ...
  try {
      ...
      <>
        <header className="vehicle-page-header">
          <div>
            ...
            <h1 id="hq-delete-confirm-title">Delete HQ accident #{accident.accidentCode}</h1>
    ...
    if (error instanceof AccidentApiError && error.reason === "unauthorized")
      return (
        <SessionRecovery returnPath={`/accidents/hq/delete/confirm?accidentId=${accidentId}`} />
      );
    if (error instanceof AccidentApiError && error.reason === "not-found")
      return <ErrorState notFoundState />;
    console.error(
      "FIS HQ accident delete detail request failed",
  ...
  return (
    <main className="page-shell vehicle-page-shell">
      <section className="vehicle-card" aria-labelledby="hq-delete-confirm-title">
        ...
        >
          <HqDeleteConfirmContent searchParams={searchParams} />
        </Suspense>
      </section>
    </main>
  );
