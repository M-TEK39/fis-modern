...
async function LatestFuelCardReportPageContent({
  ...
  if (session.status === "expired" || session.status === "unavailable")
    return (
      <main className="page-shell vehicle-page-shell">
        <SessionRecovery returnPath="/fuel-cards/report/latest" />
      </main>
    );
  if (
    !session.roles.some(
  ...
  try {
    ...
    return (
      <main className="page-shell vehicle-page-shell">
        <section className="vehicle-card" aria-labelledby="latest-fuel-card-report-title">
          <header className="vehicle-page-header">
            <div>
              <p className="eyebrow">Fuelcard reports</p>
              <h1 id="latest-fuel-card-report-title">Latest Fuelcard Report for a GG Vehicle</h1>
              <p>Search for a vehicle to view its latest compatible fuelcard records.</p>
            </div>
          ...
          <VehicleSearchForm path="/fuel-cards/report/latest" type={type} search={search} />
          <section
            className="vehicle-status-maintenance-panel"
            aria-labelledby="latest-report-vehicles-title"
          >
            <div className="vehicle-form-section-header">
              <div>
                <p className="eyebrow">Vehicle lookup</p>
                <h2 id="latest-report-vehicles-title">Matching vehicles</h2>
              </div>
            </div>
            <VehicleResults
              vehicles={vehicles}
              path="/fuel-cards/report/latest"
              type={type}
              search={search}
          ...
          {selectedVehicle ? (
            <section
              className="vehicle-status-maintenance-panel"
              aria-labelledby="latest-report-cards-title"
            >
              <div className="vehicle-form-section-header">
                <div>
                  <p className="eyebrow">Latest records</p>
                  <h2 id="latest-report-cards-title">
                    Fuelcards for{" "}
    ...
    return (
      <main className="page-shell vehicle-page-shell">
        <ApiUnavailable path="/fuel-cards/report/latest" subject="Latest fuelcard report" />
      </main>
    );
  }
}

export default function LatestFuelCardReportPage(
  props: Parameters<typeof LatestFuelCardReportPageContent>[0],
) {
  return (
    <Suspense fallback={<RouteLoading />}>
      <LatestFuelCardReportPageContent {...props} />
    </Suspense>
  );
}
