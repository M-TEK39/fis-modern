  ...
  return (
    <div className="vehicle-create-form">
      ...
      <div className="notice notice-info" role="note">
        ...
        <span>
          GG numbers are allocated during authorization when a free number is available. This capture is saved to the
          pre-vehicle authorization workflow, including its legacy purchase, site, damage, notes, extras, and maintenance
          details.
        </span>
      </div>
      ...
      <section className="vehicle-form-section" aria-labelledby="vehicle-identity-title">
        ...
        <div className="vehicle-create-grid">
          <Field id="fleetNumber" label="Current GG number">
            <input id="fleetNumber" name="fleetNumber" type="text" autoComplete="off" maxLength={20} />
          </Field>

          <Field id="replacedGgNumber" label="Replace GG number">
            <input id="replacedGgNumber" name="replacedGgNumber" type="text" autoComplete="off" maxLength={20} />
          </Field>
          ...
          <Field id="colourPreset" label="Colour" required>
            ...
            >
              ...
              {[
                ...
                "Yellow",
                "White",
              ].map((colour) => <option key={colour} value={colour}>{colour}</option>)}
            </select>
            <label className="sr-only" htmlFor="colour">Specify colour</label>
            <input
              id="colour"
              name="colour"
              type="text"
          ...
          <Field id="engineNumber" label="Engine number" required>
            <input id="engineNumber" name="engineNumber" type="text" autoComplete="off" maxLength={60} required />
          </Field>

          <Field id="chassisNumber" label="VIN / chassis number" required>
            <input id="chassisNumber" name="chassisNumber" type="text" autoComplete="off" maxLength={60} required />
          </Field>
          ...
          <Field id="takeOnOdo" label="Take-on odometer (KM)" required>
            <input id="takeOnOdo" name="takeOnOdo" type="number" min="0" max="999999" step="1" required />
          </Field>
          ...
          <Field id="sourceCode" label="Hired from" required>
            <select id="sourceCode" name="sourceCode" defaultValue="" required>
              ...
              {referenceData.sources.map((source) => (
                <option key={source.code} value={source.code}>{source.name} ({source.code})</option>
              ))}
            </select>
          </Field>
          ...
          <Field id="purchaseAmount" label="Purchase amount (R)" required>
            <input id="purchaseAmount" name="purchaseAmount" type="number" min="5000" max="9999999" step="0.01" required />
          </Field>

          <Field id="purchaseFrom" label="Purchased from" required>
            <input id="purchaseFrom" name="purchaseFrom" type="text" autoComplete="organization" maxLength={60} required />
          </Field>

          <Field id="invoiceNumber" label="Invoice number">
            <input id="invoiceNumber" name="invoiceNumber" type="text" autoComplete="off" maxLength={60} />
          </Field>
      ...
      <section className="vehicle-form-section" aria-labelledby="vehicle-notes-title">
        ...
        <div className="vehicle-create-grid">
          <fieldset className="field vehicle-fieldset">
            ...
            <label className="vehicle-checkbox-label"><input name="damageStatus" type="radio" value="N" defaultChecked /> No</label>
            <label className="vehicle-checkbox-label"><input name="damageStatus" type="radio" value="Y" /> Yes</label>
          </fieldset>
      ...
      <section className="vehicle-form-section" aria-labelledby="vehicle-maintenance-title">
        ...
        <div className="vehicle-create-grid">
          <Field id="maintenanceTypeCode" label="Maintenance type">
            ...
            >
              <option value="">
                {referenceData.maintenanceTypes.length > 0 ? "No maintenance option" : "No options available"}
              </option>
          ...
          <Field id="maintenancePeriodMonths" label="Period (months)">
            <input id="maintenancePeriodMonths" name="maintenancePeriodMonths" type="number" min="0" step="1" />
          </Field>
          ...
          <Field id="maintenanceValue" label="Maintenance value">
            <input id="maintenanceValue" name="maintenanceValue" type="number" min="0" step="0.01" />
          </Field>
        </div>
      </section>
