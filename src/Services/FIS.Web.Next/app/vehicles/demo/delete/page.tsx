...
export default async function DemoDeletePage() {
  ...
  if (session.status === "expired")
    return (
      <main className="page-shell vehicle-page-shell">
        <DemoSessionRecovery returnPath="/vehicles/demo/delete" />
      </main>
    );
  ...
  if (!hasDemoVehicleRole(session.roles))
    return (
      <main className="page-shell vehicle-page-shell">
        <DemoAccessRestricted message="You do not have permission to delete demo vehicles." />
      </main>
    );

  return (
    <main className="page-shell vehicle-page-shell">
      <section className="vehicle-card" aria-labelledby="demo-delete-title">
        <header className="vehicle-page-header">
          <div>
            <p className="eyebrow">Vehicle Master / Demo Vehicles</p>
            <h1 id="demo-delete-title">Delete Demo Vehicle</h1>
            <p>Search by GG or GP number, then confirm deletion of the selected record.</p>
          </div>
