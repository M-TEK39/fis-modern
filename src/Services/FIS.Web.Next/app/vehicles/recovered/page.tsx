...
export type RecoveredVehicleRoutePath = "/vehicles/recovered" | "/Master-File/Update_Recovered_GG.aspx";
...
function StatusCard({ title, message, href }: Readonly<{ title: string; message: string; href: string }>) {
  return (
    <section className="vehicle-status-card" role="alert">
      ...
      <div className="button-row">
        <Link className="button button-primary" href={href}>Try again</Link>
...
async function loadInitialData(searchParams: RecoveredVehiclePageProps["searchParams"]): Promise<RecoveredVehicleInitialData> {
  ...
  const details = matches.length === 1 ? await getRecoveredVehicleDetails(matches[0].vmfCode) : null;
...
async function RecoveredVehicleContent({
  searchParams,
  routePath,
}: Readonly<{ searchParams?: RecoveredVehiclePageProps["searchParams"]; routePath: RecoveredVehicleRoutePath }>) {
  try {
    const initialData = await loadInitialData(searchParams);
    return <RecoveredVehicleClient initialSearchTerm={initialData.searchTerm} initialMode={initialData.mode} initialMatches={initialData.matches} initialDetails={initialData.details} />;
  } catch (error) {
    if (isUnauthorized(error)) {
      return <SessionRecovery returnPath={routePath} />;
    }

    console.error("FIS recovered vehicle initial load failed", error instanceof Error ? error.message : "unknown error");
    return <StatusCard title="API unavailable" message="Recovered vehicle information could not be loaded." href={routePath} />;
  }
}
...
export default async function RecoveredVehiclePage({
  ...
  if (session.status === "unavailable") {
    return <main className="page-shell vehicle-page-shell"><StatusCard title="API unavailable" message="The sign-in service is temporarily unavailable." href={routePath} /></main>;
  }

  if (!hasDemoVehicleRole(session.roles)) {
    return <main className="page-shell vehicle-page-shell"><StatusCard title="Access restricted" message="You do not have permission to update recovered vehicles." href="/vehicles" /></main>;
  }
    ...
    <main className="page-shell vehicle-page-shell">
      <section className="vehicle-card" aria-labelledby="recovered-vehicle-title">
        ...
        <div className="vehicle-footer-actions">
          ...
          <Link className="button button-secondary" href="/vehicles/renumbered-report">Renumbered report</Link>
        </div>
      </section>
    </main>
  );
