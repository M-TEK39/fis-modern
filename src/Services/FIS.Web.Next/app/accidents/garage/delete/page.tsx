...
function ApiUnavailable() {
  return (
    <section className="vehicle-status-card" role="alert">
      ...
      <Link className="button button-primary" href="/accidents/garage/delete">Try again</Link>
    </section>
  );
}
...
async function GarageDeleteContent({ searchParams }: GarageDeletePageProps) {
  ...
  try {
    ...
    console.error("FIS garage accident delete lookup failed", error instanceof Error ? error.message : "unknown error");
    return <ApiUnavailable />;
  }
        ...
        <>
              ...
              <tbody>
                {pageData.rows.map((row) => (
                  <tr key={row.accidentCode}>
                    ...
                    <td>
                      <Link className="button button-danger button-small" href={buildDeleteHref(searchType, searchTerm, row.accidentCode)}>
                        DELETE
                      </Link>
                    </td>
                  </tr>
          ...
          {pageData.totalPages > 1 ? (
            <nav className="vehicle-pagination" aria-label="Garage accident delete pages">
              {pageData.page > 1 ? (
                <Link className="vehicle-pagination-button" href={buildSearchHref(searchType, searchTerm, pageData.page - 1)}>Previous</Link>
              ) : (
              ...
              {pageData.page < pageData.totalPages ? (
                <Link className="vehicle-pagination-button" href={buildSearchHref(searchType, searchTerm, pageData.page + 1)}>Next</Link>
              ) : (
  ...
  return (
    <main className="page-shell vehicle-page-shell">
      <section className="vehicle-card" aria-labelledby="garage-delete-title">
        ...
        <Suspense fallback={<div className="loading-card" aria-busy="true"><span className="spinner" aria-hidden="true" /><p>Loading garage accidents...</p></div>}>
