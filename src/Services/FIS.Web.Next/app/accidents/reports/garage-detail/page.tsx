...
function ErrorState() {
  return (
    <section className="vehicle-status-card" role="alert">
      ...
      <h2>The garage-detail accident report could not be loaded.</h2>
      <p className="muted-copy">Retry when the FIS API is available.</p>
      <Link className="button button-primary" href="/accidents/reports/garage-detail">
        Try again
      </Link>
    </section>
  );
  ...
  if (shouldRun && !errorMessage) {
    try {
      ...
      if (error instanceof AccidentApiError && error.reason === "unauthorized")
        return <SessionRecovery returnPath="/accidents/reports/garage-detail" />;
      console.error(
        "FIS garage accident report failed",
  ...
  return (
    <>
      ...
      <form className="vehicle-status-maintenance-panel" method="get">
        <fieldset className="vehicle-search-options">
          ...
          <label className="vehicle-checkbox-label">
            <input name="mode" type="radio" value="jhb" defaultChecked={mode === "jhb"} /> JHB
          </label>
          <label className="vehicle-checkbox-label">
            <input name="mode" type="radio" value="pta" defaultChecked={mode === "pta"} /> PTA
          </label>
          <label className="vehicle-checkbox-label">
      ...
      <div className="vehicle-footer-actions">
        ...
        <Link className="button button-secondary" href="/accidents/reports/garage-detail">
          Clear
        </Link>
      ...
      <section className="vehicle-card" aria-labelledby="garage-accident-title">
        <header className="vehicle-page-header">
          <div>
            ...
            <h1 id="garage-accident-title">All Accidents - Garage Detail</h1>
            <p>Review the legacy garage report for JHB, PTA, or all garages.</p>
          </div>
          <Link className="button button-secondary" href="/accidents/reports">
            Report Menu
          </Link>
