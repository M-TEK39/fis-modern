...
function ApiUnavailable() {
  return <section className="vehicle-status-card" role="alert"><div className="status-icon status-icon-error" aria-hidden="true">!</div><p className="eyebrow">API unavailable</p><h2>The accident record could not be loaded.</h2><p className="muted-copy">Retry when the FIS API is available.</p><Link className="button button-primary" href="/accidents/hq">Return to search</Link></section>;
}

function AccidentNotFound() {
  return <section className="vehicle-status-card" role="alert"><p className="eyebrow">Accident not found</p><h2>That accident record could not be found.</h2><Link className="button button-secondary" href="/accidents/hq">Return to search</Link></section>;
}
...
async function HqEditContent({ searchParams }: HqEditPageProps) {
  ...
  if (!hasRole(session.roles, ACCIDENTS_ROLE)) {
    return <section className="vehicle-status-card" role="alert"><p className="eyebrow">Access restricted</p><h2>You do not have permission to edit HQ accidents.</h2></section>;
  }
  ...
  try {
    const [accident, referenceData] = await Promise.all([getAccidentForEdit(accidentId), getAccidentReferenceData()]);
    return (
      <>
        <header className="vehicle-page-header">
          <div><p className="eyebrow">Accident maintenance</p><h1 id="hq-edit-title">Edit HQ accident #{accident.accidentCode}</h1><p>Update the HQ accident record and its references.</p></div>
    ...
    if (error instanceof AccidentApiError && error.reason === "unauthorized") return <SessionRecovery returnPath={`/accidents/hq/edit?accidentId=${accidentId}`} />;
    if (error instanceof AccidentApiError && error.reason === "not-found") return <AccidentNotFound />;
    console.error("FIS HQ accident edit request failed", error instanceof Error ? error.message : "unknown error");
  ...
  return (
    <main className="page-shell vehicle-page-shell">
      <section className="vehicle-card" aria-labelledby="hq-edit-title">
        <Suspense fallback={<div className="loading-card" aria-busy="true"><span className="spinner" aria-hidden="true" /><p>Loading accident details...</p></div>}><HqEditContent searchParams={searchParams} /></Suspense>
      </section>
    </main>
  );
}
