...
export default async function TrackingAllDeviceReportPage({ searchParams }: Readonly<{ searchParams: Promise<Record<string, string | string[] | undefined>> }>) {
  ...
  try {
    ...
    return <TrackingShell title="Tracking Report for ALL Tracking Device" description="View tracker activity across all tracking devices."><TrackingNotice query={query} /><form className="vehicle-status-maintenance-panel" method="get"><DateRangeFields startDate={range.startDate} endDate={range.endDate} /><input name="run" type="hidden" value="1" /><div className="button-row"><button className="button button-primary" type="submit">Submit</button><Link className="button button-secondary" href="/tracking/reports">Report menu</Link></div></form>{run ? <TrackingReportTable records={rows} title="Tracking report for all devices" /> : <p className="muted-copy">Choose a date range, then submit the report.</p>}</TrackingShell>;
  } catch (error) {
    return <TrackingShell title="Tracking Report for ALL Tracking Device" description="View tracker activity across all tracking devices."><section className="vehicle-status-card" role="alert"><h2>{error instanceof TrackingApiError && error.reason === "unauthorized" ? "Your session has expired." : "The Tracking report service is temporarily unavailable."}</h2><Link className="button button-primary" href="/tracking/reports/all-device">Try again</Link></section></TrackingShell>;
  }
}
