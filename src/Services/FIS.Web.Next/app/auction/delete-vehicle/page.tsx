...
export type AuctionDeletePageProps = {
  ...
  return (
    <form className="vehicle-status-maintenance-panel" method="get">
      ...
      <div className="vehicle-search-row">
        <label className="sr-only" htmlFor="auction-delete-search">{searchType === "GG" ? "GG number" : "GP number"}</label>
        <input className="vehicle-search" id="auction-delete-search" maxLength={8} name="searchQuery" placeholder={searchType === "GG" ? "Enter GG number" : "Enter GP number"} defaultValue={searchQuery} />
      </div>
...
function AuctionRows({ auctions, searchType, searchQuery }: Readonly<{ auctions: AuctionRecord[]; searchType: AuctionSearchType; searchQuery: string }>) {
  if (auctions.length === 0) {
    return <div className="vehicle-empty-state"><p className="eyebrow">No records found</p><h2>{searchQuery ? `No auction records matched “${searchQuery}”.` : "No auction records are available."}</h2><p className="muted-copy">Try another GG or GP number.</p></div>;
  }
  ...
  return (
    <div className="vehicle-table-wrapper" aria-live="polite">
      <table className="vehicle-table">
        ...
        <thead><tr><th scope="col">Number</th><th scope="col">Auction Number</th><th scope="col">Auction Garage</th><th scope="col">Action</th></tr></thead>
        <tbody>
          {auctions.map((auction) => (
            <tr key={auction.auctionCode}>
              ...
              <td><Link className="button button-danger button-small" href={buildDetailHref(auction.auctionCode)}>DELETE</Link></td>
            </tr>
          ))}
        </tbody>
      </table>
...
function ApiUnavailable() {
  return (
    <section className="vehicle-status-card" role="alert">
      ...
      <h2>Auction records could not be loaded for deletion.</h2>
      <p className="muted-copy">The application is still running. Retry when the FIS API is available.</p>
      <Link className="button button-primary" href="/auction/delete-vehicle">Try again</Link>
    </section>
  );
}

export default async function AuctionDeletePage({ searchParams, routePath = "/auction/delete-vehicle" }: AuctionDeletePageProps) {
  await connection();
  ...
  try {
    ...
    return (
      <main className="page-shell vehicle-page-shell">
        <section className="vehicle-card" aria-labelledby="auction-delete-title">
          <header className="vehicle-page-header">
            <div><p className="eyebrow">Auction maintenance</p><h1 id="auction-delete-title">Delete a Vehicle on Auction</h1><p>Find an auction record by GG or GP number, review it, then confirm deletion.</p></div>
            <Link className="button button-secondary" href="/auction">Auction Menu</Link>
          </header>
          {deleted ? <div className="notice notice-success" role="status">Auction record deleted successfully.</div> : null}
          {error ? <div className="notice notice-error" role="alert">{error}</div> : null}
          <SearchForm searchType={searchType} searchQuery={searchQuery} />
          <section className="vehicle-status-maintenance-panel" aria-labelledby="auction-delete-results-title">
            <div className="vehicle-form-section-header"><div><p className="eyebrow">Johannesburg Garage</p><h2 id="auction-delete-results-title">Auction records found</h2></div></div>
    ...
    console.error("FIS auction deletion lookup failed", requestError instanceof Error ? requestError.message : "unknown error");
    return <main className="page-shell vehicle-page-shell"><ApiUnavailable /></main>;
  }
}
