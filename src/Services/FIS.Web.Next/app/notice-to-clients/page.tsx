...
function formatCreatedDate(value: string | null) {
  ...
  return new Intl.DateTimeFormat("en-ZA", { day: "2-digit", month: "2-digit", year: "numeric" }).format(date);
}
...
function PublicNotice({ notice, index, total }: Readonly<{ notice: Notice; index: number; total: number }>) {
  ...
  return (
    <article className="public-notice" aria-labelledby="public-notice-title">
      <dl className="public-notice-meta">
        <div><dt>Date</dt><dd>{formatNoticeDate(notice.noticeDate)}</dd></div>
        <div><dt>From</dt><dd>{notice.noticeFrom || "-"}</dd></div>
      </dl>
      ...
      <div className="public-notice-signoff">
        ...
        {notice.noticePersonTitle ? <p className="public-notice-person-title">{notice.noticePersonTitle}</p> : null}
      ...
      <div className="public-notice-actions">
        <Link className="button button-primary" href={nextPath}>{index + 1 < total ? "View next notice" : "Login to F.I.S"}</Link>
      </div>
    </article>
  );
}
...
function ErrorCard() {
  return (
    <section className="vehicle-status-card" role="alert">
      ...
      <div className="button-row"><Link className="button button-primary" href="/Notice_To_Clients.aspx">Try again</Link><Link className="button button-secondary" href="/login">Sign in</Link></div>
    </section>
  );
}

export default async function NoticeToClientsPage({ searchParams }: Readonly<{ searchParams: SearchParams }>) {
  await connection();
  let notices: Notice[];
  try {
    ...
    if (!(error instanceof NoticeApiError)) {
      console.error("FIS public notice request failed", error instanceof Error ? error.message : "unknown error");
    }
    return <main className="public-notice-shell"><ErrorCard /></main>;
  }
    ...
    <main className="public-notice-shell">
      <section className="public-notice-card" aria-labelledby="public-notice-heading">
        <header className="public-notice-header">
          <div className="public-notice-brand" aria-label="Fleet Information System">FIS</div>
          <div><p className="public-notice-brand-name">Fleet Information System</p><p className="public-notice-brand-caption">Gauteng Provincial Government</p></div>
        </header>
        <div className="public-notice-heading"><h2 id="public-notice-heading">Client Communication</h2></div>
        <PublicNotice notice={notice} index={index} total={notices.length} />
        <footer className="public-notice-footer">16 Boeing Road East, Bedfordview, Private Bag X1, Bedfordview 2008. Tel: +27 (0)11 372 8600. Fax: +27 (0)86 669 6926</footer>
      </section>
    </main>
  );
}
