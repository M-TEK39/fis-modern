...
import ForcePasswordForm from "@/app/users/force-password-change/force-password-form";
import { forceUserPasswordAction } from "@/app/users/force-password-change/actions";
...
function getMessage(result: string | undefined) {
  switch (result) {
    ...
    case "forbidden":
      return { tone: "error", text: "You do not have permission to reset user passwords." } as const;
    ...
    case "password-mismatch":
      return { tone: "error", text: "The Confirm Password must match the New Password entry." } as const;
    case "not-found":
      return { tone: "error", text: "The selected user could not be found." } as const;
    case "rejected":
      return { tone: "error", text: "The new password does not meet the FIS password policy." } as const;
    case "unavailable":
      return { tone: "error", text: "The password reset service is unavailable. Retry when the FIS API is available." } as const;
    case "unauthorized":
      return { tone: "error", text: "Your session is no longer authorized. Sign in again." } as const;
    default:
      return result ? { tone: "error", text: "The password reset could not be completed." } as const : null;
  }
}

export default async function ForcePasswordChangePage({ searchParams }: Readonly<{ searchParams: SearchParams }>) {
  await connection();
  const session = await getSession();

  if (session.status === "anonymous") redirect("/login");
  if (session.status === "expired") {
    return <main className="page-shell vehicle-page-shell"><SessionRecovery returnPath="/users/force-password-change" /></main>;
  }
  if (session.status === "unavailable") {
    return <main className="page-shell vehicle-page-shell"><section className="vehicle-status-card" role="alert"><p className="eyebrow">API unavailable</p><h2>Reset User Password could not be opened.</h2><p className="muted-copy">Retry when the FIS API is available.</p></section></main>;
  }
  ...
  try {
      ...
      <main className="page-shell vehicle-page-shell">
        <section className="vehicle-card" aria-labelledby="force-password-title">
          ...
          <section className="vehicle-overview" aria-labelledby="force-password-form-title">
            ...
            <ForcePasswordForm action={forceUserPasswordAction} users={users} username={username} alphabet={alphabet} />
          </section>
        </section>
      </main>
    );
    ...
    console.error("FIS force-password user lookup failed", error instanceof Error ? error.message : "unknown error");
    return <main className="page-shell vehicle-page-shell"><section className="vehicle-status-card" role="alert"><p className="eyebrow">API unavailable</p><h2>Reset User Password could not be loaded.</h2><p className="muted-copy">Retry when the FIS API is available.</p></section></main>;
  }
}
