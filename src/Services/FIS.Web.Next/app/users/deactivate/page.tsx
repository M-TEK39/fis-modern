...
function getMessage(result: string | undefined, operation: UserStatusOperation) {
  switch (result) {
    ...
    case "forbidden":
      return { tone: "error", text: "You do not have permission to manage user activation." } as const;
    ...
    case "rejected":
      return operation === "deactivate-expired"
        ? { tone: "error", text: "The user cannot be deactivated because the password is still valid." } as const
        : { tone: "error", text: "The user status change was rejected." } as const;
    case "unavailable":
      return { tone: "error", text: "The user status service is unavailable. Retry when the FIS API is available." } as const;
    case "unauthorized":
    ...
    default:
      return result ? { tone: "error", text: "The user status change could not be completed." } as const : null;
  }
}

export default async function DeactivateUserPage({ searchParams }: Readonly<{ searchParams: SearchParams }>) {
  await connection();
  const session = await getSession();

  if (session.status === "anonymous") redirect("/login");
  if (session.status === "expired") {
    return <main className="page-shell vehicle-page-shell"><SessionRecovery returnPath="/users/deactivate" /></main>;
  }
  if (session.status === "unavailable") {
    return <main className="page-shell vehicle-page-shell"><section className="vehicle-status-card" role="alert"><p className="eyebrow">API unavailable</p><h2>Activate or De-Activate User could not be opened.</h2><p className="muted-copy">Retry when the FIS API is available.</p></section></main>;
  }
  if (!hasUserAdministrationRole(session.roles)) {
    return <main className="page-shell vehicle-page-shell"><section className="vehicle-status-card" role="alert"><p className="eyebrow">Access restricted</p><h2>You do not have permission to manage user activation.</h2></section></main>;
  }
    ...
    <main className="page-shell vehicle-page-shell">
      <section className="vehicle-card" aria-labelledby="deactivate-user-title">
        ...
        <section className="vehicle-overview" aria-labelledby="deactivate-user-form-title">
          ...
          <h2 id="deactivate-user-form-title">Search for the user you want to de-activate or activate</h2>
          <DeactivateUserForm action={updateUserStatusAction} username={username} alphabet={alphabet} />
        </section>
      </section>
    </main>
  );
