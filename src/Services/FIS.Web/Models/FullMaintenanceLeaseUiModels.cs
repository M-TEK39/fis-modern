...
{
    public static string MapAuthorityStatus(int? status) =>
        status switch
        {
            1 => "Pending",
            2 => "Approved",
    ...
    public static string GetAuthorityBadgeClass(int? status) =>
        status switch
        {
    ...
    public static List<VehicleDto> FilterVehicleMatches(
        List<VehicleDto> matches,
        string keyword,
        string searchMode
    )
    {
            ...
            .Where(v =>
                isGp
                    ? !string.IsNullOrWhiteSpace(v.registration_number)
                        && v.registration_number.Contains(query, StringComparison.OrdinalIgnoreCase)
                    : !string.IsNullOrWhiteSpace(v.fleet_number)
                        && v.fleet_number.Contains(query, StringComparison.OrdinalIgnoreCase)
            )
            .OrderByDescending(v =>
                isGp
                    ? string.Equals(
