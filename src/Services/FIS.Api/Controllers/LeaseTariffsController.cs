...
public sealed class LeaseTariffsController : BaseApiController
{
    ...
    public LeaseTariffsController(ILeaseTariffRepository repository, ILogger<LeaseTariffsController> logger)
    {
    ...
    public async Task<ActionResult<LeaseTariff>> GetLatestByVehicle(int vmfCode)
    {
        try
        {
            ...
            var latest = tariffs.OrderByDescending(t => t.end_date).ThenByDescending(t => t.lease_tariff_code).FirstOrDefault();
            return latest is null ? NotFound() : Ok(latest);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error retrieving the latest lease tariff for vehicle {VmfCode}", vmfCode);
    ...
    public async Task<ActionResult<LeaseTariff>> Create([FromBody] LeaseTariff tariff)
    {
        try
        {
            ...
            return CreatedAtAction(nameof(GetById), new { id = created.lease_tariff_code }, created);
        }
