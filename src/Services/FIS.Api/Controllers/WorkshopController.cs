...
public class WorkshopController : ControllerBase
{
    ...
    [HttpPost]
    public async Task<ActionResult<Workshop>> Create([FromBody] Workshop item) { try { var created = await _repository.CreateAsync(item); return CreatedAtAction(nameof(GetById), new { id = created.ww_code }, created); } catch (Exception ex) { _logger.LogError(ex, "Error"); return StatusCode(500); } }

    [HttpPut("{id}")]
    public async Task<ActionResult<Workshop>> Update(short id, [FromBody] Workshop item) { try { if (id != item.ww_code) return BadRequest(); return Ok(await _repository.UpdateAsync(item)); } catch (Exception ex) { _logger.LogError(ex, "Error"); return StatusCode(500); } }
