...
public sealed class LicenseCertificatesController : BaseApiController
{
    ...
    public async Task<ActionResult> GetByVehicle(int vmfCode)
    {
        ...
        {
            _logger.LogError(ex, "Error listing licence certificates for vehicle {VmfCode}", vmfCode);
            return StatusCode(500, new { error = "Failed to retrieve licence certificates" });
        }
    }
    ...
    public async Task<ActionResult> GetMissing([FromQuery] string? location = null)
    {
        try
        {
            ...
            var vehicles = (await _vehicleRepository.GetActiveVehiclesAsync())
                ...
                .ToList();
            var certificateVehicleCodes = (await _repository.GetAllAsync()).Select(document => document.vmf_code).ToHashSet();
            var missing = vehicles
                ...
                .ToList();
            return Ok(new { location = normalizedLocation is "jhb" or "pta" ? normalizedLocation : "all", total_count = missing.Count, vehicles = missing });
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error listing vehicles without licence certificates");
            return StatusCode(500, new { error = "Failed to retrieve vehicles without licence certificates" });
        }
    }
        ...
        [FromForm] DateTime? periodEnd = null)
    {
        ...
        try
        {
            if (vmfCode <= 0) return BadRequest(new { error = "A valid vehicle is required." });
            if (periodBegin.HasValue && periodEnd.HasValue && periodEnd.Value < periodBegin.Value)
                return BadRequest(new { error = "The certificate end date must be on or after the begin date." });
            if (file is null || file.Length == 0) return BadRequest(new { error = "A scanned licence certificate is required." });
            if (!AllowedMimeTypes.Contains(file.ContentType ?? string.Empty))
                return BadRequest(new { error = "Only JPG, PNG, or GIF image scans are accepted." });
            ...
            if (await _repository.HasAnyForVehicleAsync(vmfCode))
                return Conflict(new { error = "A licence certificate is already uploaded for this vehicle. Delete the existing scan before uploading a replacement." });

            var source = await _repository.GetPreferredWriteSourceAsync();
            if (source is null) return StatusCode(503, new { error = "Neither the modern vehicle document table nor the legacy scan table is available." });

            var extension = Path.GetExtension(file.FileName).ToLowerInvariant();
            if (!new[] { ".jpg", ".jpeg", ".png", ".gif" }.Contains(extension, StringComparer.OrdinalIgnoreCase))
                return BadRequest(new { error = "The uploaded scan must use a JPG, PNG, or GIF file extension." });
            ...
            if (source.Equals("modern", StringComparison.OrdinalIgnoreCase))
            {
                ...
                var relativePath = Path.Combine(vmfCode.ToString(), "Licence", storedName).Replace('\\', '/');
                absolutePath = ResolveSafePath(basePath, relativePath);
                if (absolutePath is null) return StatusCode(500, new { error = "The document storage path is invalid." });
                Directory.CreateDirectory(Path.GetDirectoryName(absolutePath)!);
                await using (var stream = new FileStream(absolutePath, FileMode.CreateNew, FileAccess.Write, FileShare.None))
                    await file.CopyToAsync(stream);

                var document = await _repository.CreateAsync(new LicenseCertificateDocument
                {
                    Source = "modern",
                    vmf_code = vmfCode,
                ...
                return CreatedAtAction(nameof(GetByKey), new { source = document.Source, documentKey = document.DocumentKey, vmfCode }, await MapDocumentAsync(document));
            }

            var legacyBasePath = _configuration["LegacyScanStorage:BasePath"] ?? "wwwroot/scandocs/scan_img";
            absolutePath = ResolveSafePath(legacyBasePath, storedName);
            if (absolutePath is null) return StatusCode(500, new { error = "The legacy scan storage path is invalid." });
            Directory.CreateDirectory(Path.GetDirectoryName(absolutePath)!);
            await using (var stream = new FileStream(absolutePath, FileMode.CreateNew, FileAccess.Write, FileShare.None))
                await file.CopyToAsync(stream);
            var legacyDocument = await _repository.CreateAsync(new LicenseCertificateDocument
            {
                Source = "legacy",
                vmf_code = vmfCode,
                image = storedName,
            ...
            return CreatedAtAction(nameof(GetByKey), new { source = legacyDocument.Source, documentKey = legacyDocument.DocumentKey, vmfCode }, await MapDocumentAsync(legacyDocument));
        }
        catch (Exception ex)
        {
            if (absolutePath is not null && System.IO.File.Exists(absolutePath)) System.IO.File.Delete(absolutePath);
            _logger.LogError(ex, "Error uploading licence certificate for vehicle {VmfCode}", vmfCode);
            return StatusCode(500, new { error = "Failed to upload licence certificate" });
        }
    }

    [HttpGet("{source}/{documentKey}")]
    public async Task<ActionResult> GetByKey(string source, string documentKey, [FromQuery] int vmfCode)
    {
        var document = await _repository.GetByKeyAsync(source, vmfCode, documentKey);
        return document is null ? NotFound(new { error = "Licence certificate not found" }) : Ok(await MapDocumentAsync(document));
    }

    [HttpGet("{source}/{documentKey}/file")]
    public async Task<ActionResult> Download(string source, string documentKey, [FromQuery] int vmfCode)
    {
        var document = await _repository.GetByKeyAsync(source, vmfCode, documentKey);
        if (document is null) return NotFound(new { error = "Licence certificate not found" });
    ...
    public async Task<ActionResult> Delete(string source, string documentKey, [FromQuery] int vmfCode)
    {
        try
        {
            var document = await _repository.GetByKeyAsync(source, vmfCode, documentKey);
        ...
        {
            _logger.LogError(ex, "Error deleting licence certificate {Source}/{DocumentKey}", source, documentKey);
            return StatusCode(500, new { error = "Failed to delete licence certificate" });
        }
    }

    private async Task<IReadOnlyList<object>> AddVehicleDetailsAsync(IEnumerable<LicenseCertificateDocument> documents)
    {
    ...
    private object MapDocument(LicenseCertificateDocument document, FIS.Core.Domain.Entities.Vehicle? vehicle) => new
    {
    ...
    {
        var begin = periodBegin?.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture) ?? string.Empty;
    ...
    private string? ResolveDocumentPath(LicenseCertificateDocument document)
        => document.Source.Equals("modern", StringComparison.OrdinalIgnoreCase)
            ? ResolveSafePath(_configuration["DocumentStorage:BasePath"] ?? "uploads/documents", document.stored_file_path)
            : ResolveLegacyPath(document.image);

    private string? ResolveLegacyPath(string? image)
    {
        if (string.IsNullOrWhiteSpace(image) || !string.Equals(Path.GetFileName(image), image, StringComparison.Ordinal)) return null;
        var configured = ResolveSafePath(_configuration["LegacyScanStorage:BasePath"] ?? "wwwroot/scandocs/scan_img", image);
        if (configured is not null && System.IO.File.Exists(configured)) return configured;
        var historical = ResolveSafePath("wwwroot/ScanDocs/scan_img", image);
        return historical is not null && System.IO.File.Exists(historical) ? historical : configured;
    }
