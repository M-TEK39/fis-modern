...
public class VehicleAssessmentController : BaseApiController
{
    ...
    [HttpPost]
    public async Task<ActionResult<VehicleAssessment>> Create([FromBody] VehicleAssessment item) { try { var created = await _repository.CreateAsync(item, GetCurrentUserId()); return CreatedAtAction(nameof(GetById), new { id = created.vehicle_assessment_code }, created); } catch (Exception ex) { _logger.LogError(ex, "Error"); return StatusCode(500); } }

    [HttpPut("{id}")]
    public async Task<ActionResult<VehicleAssessment>> Update(int id, [FromBody] VehicleAssessment item) { try { if (id != item.vehicle_assessment_code) return BadRequest(); return Ok(await _repository.UpdateAsync(item, GetCurrentUserId())); } catch (Exception ex) { _logger.LogError(ex, "Error"); return StatusCode(500); } }
