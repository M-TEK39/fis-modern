...
public sealed class TaxiScanDocsController : BaseApiController
{
    ...
    public async Task<ActionResult> GetByVehicle(int vmfCode)
    {
        ...
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "Error listing taxi requisition scans for vehicle {VmfCode}",
                vmfCode
            );
            return StatusCode(500, new { error = "Failed to retrieve taxi requisition scans" });
        }
    ...
    )
    {
        try
        {
            ...
            if (file is null || file.Length == 0)
                return BadRequest(new { error = "A scanned requisition certificate is required." });
            if (!AllowedMimeTypes.Contains(file.ContentType ?? string.Empty))
                return BadRequest(
                    new { error = "Only JPG, PNG, GIF, or WEBP image scans are accepted." }
                );
            ...
            if (string.IsNullOrWhiteSpace(extension) || extension.Length > 10)
                return BadRequest(new { error = "The uploaded scan has an invalid file name." });
            ...
            return CreatedAtAction(
                nameof(GetById),
                new { scanDocCode = created.taxi_scandoc_code },
                await MapDocumentAsync(created)
            );
        }
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "Error uploading taxi requisition scan for vehicle {VmfCode}",
                vmfCode
            );
    ...
    {
        var document = await _repository.GetByIdAsync(scanDocCode);
        if (document is null || string.IsNullOrWhiteSpace(document.image))
            return NotFound();

        var path = ResolveStoredPath(document.image);
        if (path is null || !System.IO.File.Exists(path))
            return NotFound(new { error = "The scan file is not available." });
    ...
    private async Task<IReadOnlyList<object>> AddVehicleDetailsAsync(
        IEnumerable<TaxiScanDoc> documents
    )
    {
    ...
    private static object MapDocument(
        ...
        new
        {
            ...
            date_created = document.date_created == default
                ? (DateTime?)null
