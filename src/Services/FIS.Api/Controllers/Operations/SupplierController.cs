...
public class SupplierController : BaseApiController
{
    ...
    [HttpGet("active")]
    public async Task<ActionResult<IEnumerable<Supplier>>> GetActive()
    {
        try
        {
    ...
    [HttpPost]
    public async Task<ActionResult<Supplier>> Create([FromBody] Supplier item)
    {
        try
        {
            var created = await _repository.CreateAsync(item, GetCurrentUserId());
            return CreatedAtAction(nameof(GetById), new { id = created.supplier_id }, created);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error");
    ...
    [HttpPut("{id}")]
    public async Task<ActionResult<Supplier>> Update(short id, [FromBody] Supplier item)
    {
        try
        {
            if (id != item.supplier_id)
                return BadRequest();
