...
public class LeaseContractTermsController : BaseApiController
{
    ...
    [HttpPost]
    public async Task<ActionResult<LeaseContractTerms>> Create([FromBody] LeaseContractTerms item) { try { var created = await _repository.CreateAsync(item, GetCurrentUserId()); return CreatedAtAction(nameof(GetById), new { id = created.VehicleContractTermID }, created); } catch (Exception ex) { _logger.LogError(ex, "Error"); return StatusCode(500); } }

    [HttpPut("{id}")]
    public async Task<ActionResult<LeaseContractTerms>> Update(int id, [FromBody] LeaseContractTerms item) { try { if (id != item.VehicleContractTermID) return BadRequest(); return Ok(await _repository.UpdateAsync(item, GetCurrentUserId())); } catch (Exception ex) { _logger.LogError(ex, "Error"); return StatusCode(500); } }
