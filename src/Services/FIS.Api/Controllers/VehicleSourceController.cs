...
public sealed class VehicleSourceController : BaseApiController
{
    ...
    public async Task<IActionResult> GetAll()
    {
        ...
        catch (Exception ex)
        {
            ...
            return StatusCode(StatusCodes.Status503ServiceUnavailable, new { message = "The vehicle source service is unavailable." });
        }
    }
    ...
    public async Task<IActionResult> GetByCode(byte sourceCode)
    {
        ...
        try
        {
            ...
            return source is null
                ? NotFound(new { message = $"Vehicle source with code {sourceCode} was not found." })
                : Ok(Map(source));
        }
        catch (Exception ex)
        {
    ...
    public async Task<IActionResult> Create([FromBody] VehicleSourceRequest request)
    {
        ...
        try
        {
            ...
            return CreatedAtAction(nameof(GetByCode), new { sourceCode = created.SourceCode }, Map(created));
        }
        catch (VehicleSourceFieldUnavailableException ex)
        {
            return Conflict(new { message = $"This database cannot store the legacy vehicle source {GetFriendlyFieldName(ex.FieldName)} field." });
        }
        ...
        catch (Exception ex)
        {
            ...
            return StatusCode(StatusCodes.Status503ServiceUnavailable, new { message = "The vehicle source could not be saved." });
        }
    }

    [HttpPut("{sourceCode:int}")]
    public async Task<IActionResult> Update(byte sourceCode, [FromBody] VehicleSourceRequest request)
    {
        if (!HasVehicleManagementPermission())
        {
            return Forbid();
        ...
        try
        {
            ...
            var updated = await _repository.UpdateAsync(sourceCode, ToInput(request), GetCurrentUserId());
        ...
        {
            return NotFound(new { message = $"Vehicle source with code {sourceCode} was not found." });
        }
        catch (UnauthorizedAccessException)
        {
            return Unauthorized();
    ...
    private static string? Validate(VehicleSourceRequest? request, VehicleSourceCapabilities capabilities)
    {
        if (request is null)
        {
        ...
        var missing = requiredFields.FirstOrDefault(field => string.IsNullOrWhiteSpace(field.Value));
        ...
        return request.Name!.Trim().Length > 60 ? "The vehicle source name cannot exceed 60 characters."
            : request.PhysicalAddress!.Trim().Length > 60 ? "The physical address cannot exceed 60 characters."
            : request.PostalAddress!.Trim().Length > 60 ? "The postal address cannot exceed 60 characters."
            : request.TelephoneNumber!.Trim().Length > 20 ? "The telephone number cannot exceed 20 characters."
            : request.FaxNumber!.Trim().Length > 20 ? "The fax number cannot exceed 20 characters."
            : request.EmailAddress?.Trim().Length > 40 ? "The e-mail address cannot exceed 40 characters."
            : request.ContactPerson?.Trim().Length > 40 ? "The contact person cannot exceed 40 characters."
            : null;
    }
