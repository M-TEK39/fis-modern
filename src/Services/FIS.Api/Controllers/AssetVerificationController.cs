...
public class AssetVerificationController : ControllerBase
{
    ...
    public AssetVerificationController(IAssetVerificationRepository repository, ILogger<AssetVerificationController> logger) { _repository = repository; _logger = logger; }

    [HttpGet]
    public async Task<ActionResult<IEnumerable<AssetVerification>>> GetAll() { try { return Ok(await _repository.GetAllAsync()); } catch (Exception ex) { _logger.LogError(ex, "Error"); return StatusCode(500); } }

    [HttpGet("{id}")]
    public async Task<ActionResult<AssetVerification>> GetById(int id) { try { var item = await _repository.GetByIdAsync(id); return item == null ? NotFound() : Ok(item); } catch (Exception ex) { _logger.LogError(ex, "Error"); return StatusCode(500); } }

    [HttpGet("vehicle/{vmfCode}")]
    public async Task<ActionResult<IEnumerable<AssetVerification>>> GetByVehicle(int vmfCode) { try { return Ok(await _repository.GetByVehicleAsync(vmfCode)); } catch (Exception ex) { _logger.LogError(ex, "Error"); return StatusCode(500); } }

    [HttpGet("site/{siteCode}")]
    public async Task<ActionResult<IEnumerable<AssetVerification>>> GetBySite(int siteCode) { try { return Ok(await _repository.GetBySiteAsync(siteCode)); } catch (Exception ex) { _logger.LogError(ex, "Error"); return StatusCode(500); } }
