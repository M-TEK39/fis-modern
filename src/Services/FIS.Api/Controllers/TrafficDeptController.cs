...
public class TrafficDeptController : BaseApiController
{
    ...
    [HttpPost]
    public async Task<ActionResult<TrafficDept>> Create([FromBody] TrafficDept item) { try { var created = await _repository.CreateAsync(item, GetCurrentUserId()); return CreatedAtAction(nameof(GetById), new { id = created.Traffic_dept_code }, created); } catch (Exception ex) { _logger.LogError(ex, "Error"); return StatusCode(500); } }

    [HttpPut("{id}")]
    public async Task<ActionResult<TrafficDept>> Update(short id, [FromBody] TrafficDept item) { try { if (id != item.Traffic_dept_code) return BadRequest(); return Ok(await _repository.UpdateAsync(item, GetCurrentUserId())); } catch (Exception ex) { _logger.LogError(ex, "Error"); return StatusCode(500); } }
