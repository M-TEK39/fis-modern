...
- When a route calls `connection()`, reads cookies/session state, checks permissions, awaits `searchParams`, or fetches protected data, its **default page export must be a synchronous static shell with its own local `<Suspense>` boundary** and generic, non-sensitive fallback. Put every request-time operation in an async child rendered by that boundary. Do not rely only on a route-segment `loading.tsx`: a canonical route may be re-exported by a compatibility alias outside that segment, and the local boundary must travel with the page. A shared-layout session boundary must contain only authenticated chrome, never the layout's `{children}` tree.
- Keep the outer page/layout static where practical and stream the async authenticated child. Do not suppress the Cache Components diagnostic with `export const instant = false`, remove `cacheComponents`, or cache session/authorization data. Redirects and access checks stay inside the streamed server child; the fallback must never reveal protected labels, records, roles, or identifiers. `page.tsx` is a constrained Next route module: do not export reusable runtime route components, route helpers, or custom route-prop types from it. Put reusable compatibility shells in a sibling non-route module and import that from both legal route defaults. Before handoff, audit every `page.tsx` containing `connection()` or protected session/adaptor work—not only the reported route—and use browser navigation to verify that no Cache Components uncached-data diagnostic remains.
